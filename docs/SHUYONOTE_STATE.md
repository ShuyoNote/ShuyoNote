...
| 里程碑 | 主题 | 状态 |
|---|---|---|
| M1–M25 | Markdown/加密/主题/仪表盘/PDF/数据库/多空间/插件/网盘/跨平台/AI/绘图/PDF批注/帮助/公式 | ✅（详见客户端 `docs/roadmap.md`） |
| M26 | 公式（数学） | ✅ |
| M27 | 团队版（自建协作） | 🔶 部分（服务端 S1–S8 已落地；客户端 per-workspace `sync_profiles` + 账户 UI（U1–U4）+ E2E 加密已落地；**实时协同后置**） |
...
4. **M11.3 UI 插件 / M11.4 市场 / M23.5 协同 / 移动端（M6）**：已评估延后。
