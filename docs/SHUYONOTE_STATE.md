...
4. **插件体系进化 M11.5 前置硬化**：先补运行时限、**资源上限**（已核实 Boa 无分配预算 API → 进程内用"超预算 panic 的限流分配器"闭合分配炸弹）与故障可见性（死循环不挂窗口/不泄漏线程、失败可见、`enabled` 后端强制），再冻结 ABI 与权限。**定位=做第一不做更大**：做**第一个「有权限模型 + 作用在 E2EE 可自托管数据上」的可信插件体系**（不比能力条数）——见[插件体系进化方案](plans/2026-09-10-plugin-evolution-plan.md)（M11.5–M11.13，顺序刚性；M11.13 进程级隔离为 M11.11 分发硬前置）。
