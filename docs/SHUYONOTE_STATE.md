...
8. **跨机器多端同步测试（Windows ⇄ Mac，服务器放 Mac）—— 工具已就绪，等 Mac 侧执行**（2026-09-15）：
   会合协议 `scripts/sync-multidevice.mjs`（两端各跑一次、不需要约定先后：写标记 → 等对方 →
   **互改对方的页**再等回改 ⇒ 证明"就地更新也双向到达"，不只是"新页能看见"）。
   本机双角色自测通过（两端各 6/0，`peerSeen`/`bidirectional` 都 true）；手册
   [sync-multidevice-test.md](sync-multidevice-test.md) 含 Mac 侧起服务端/发设备密钥/**真客户端肉眼确认**、
   四种不通过的原因、以及"没覆盖的三项（同页冲突 / 跨机附件 / 断网重连）"。
   **设备密钥那套（服务端 K1 + 客户端 K2）就是这条测试的凭据来源**：服务端 schema v14 可签发/作废
   `sk_` 密钥（只存指纹），客户端「高级：手动填令牌 / 设备密钥」直接粘贴即可、无需注册。
   两台机器不通时用 SSH 反向隧道（公网服务器回环 + 8799；Windows 侧这半已验通）。
   **交接与回报区在服务端仓 `docs/SESSION_CONTINUE.md` §13**（Mac 执行后填 §13.3）。
