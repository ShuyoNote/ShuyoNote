...
4. **插件体系进化 M11.8 触发面与事件**：M11.5/M11.6/M11.7 均已落地（时限与资源上限、ABI v1 + 能力注册表 + 权限与写中介、20 条能力 + 与 AI 工具层合并，**以及 M11.6 收口的作者工具链**——应用内校验/热重载/`pnpm plugin:validate`/示例插件/类型包 globals）；**M11.8 已落地三档**（命令参数 → 宿主渲染表单、结构化返回、事件钩子 v1 含 `page.saved`、**触发面 v1：编辑器 `/` 菜单**）；下一步做 M11.8 剩余部分：其余触发面（页面/附件右键、编辑器工具栏）、插件设置、其余事件发射点；另有一处**信任缺口**待办（插件更新后新增 events/权限不重新征求同意，见路线图与 M11.11b）。见[插件体系进化方案](plans/2026-09-10-plugin-evolution-plan.md)（**定位=做第一不做更大**：做**第一个「有权限模型 + 作用在 E2EE 可自托管数据上」的可信插件体系**，不比能力条数）。
