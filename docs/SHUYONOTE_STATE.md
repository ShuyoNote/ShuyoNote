...
- **版本**：对外交付线最新 **`1.91.26`**（2026-09-23）；**`main` 已包含 `dev` 的全部内容** ——
...
- **1.91.25 修两个"只在打包产物里坏"的 bug**（dev/CI 全绿、装出来的包才崩，值得记住这一类）：
  ...
  （单测抓不到：ESM 下永远绿）。
- **1.91.26 修"社区帖存成笔记后属性区不及时刷新"**：属性区自己拉 `getPageProps` 存本地 state，
  而这条路**先建页、后写属性**（走 `set_page_prop` 命令，绕过属性区写路径）⇒ 面板挂载时拿到的是
  "没属性"那份，要重开这一页才刷新。修法：`propertyUi` 加 `propsRev`/`bumpProps()`，面板把它放进
  加载依赖，`savePostAsNote` 写完属性就 bump。**约定：任何绕过属性区写属性的地方都要 bump**
  （插件 API / 模板套用那类入口目前还没接，属已知遗留）。
