...
- **块级 CRDT**：**阶段 1 已落地**（块身份 `blockId` 归属、**块版本 `blockRev`** 纯函数 Rust ↔ TS 双份判据、写层与就绪度盘点）；**阶段 2 客户端两侧已落地**（每页状态落盘 ＋ 编辑器绑定 ＋ 推/收两侧都消费 `crdt_state` ＋ 桌面也收发状态；服务端开算见[决策稿](plans/2026-09-23-s5-server-merge-decision.md)：**建议暂缓**，格式层已由 [yrs 对拍](plans/2026-09-23-yrs-interop-spike-conclusions.md) 证可行），见 §3.5 与[全上线冲刺](plans/2026-09-23-crdt-full-launch-sprint.md) §11–§14。
- **社区与插件分发**：`plugin-index.json` 规范 ＋ 索引签名 ＋ 撤回（两级、离线生效）＋ 发布者公钥固定（TOFU）＋ 多源订阅 ＋ **事实清单（只摆事实不评分）**；社区侧另有「一键发布到社区」的客户端方案。
- **近实时协作**：同页冲突提示、presence（谁在编辑）、评论/@/通知、SSE 推送——服务端 `collab.rs` 与客户端命令/UI 已落地。⚠️ **如实记**：SSE **桌面与 Web 都有**（2026-09-23 第 48–49 轮补齐桌面：Rust 订流 ＋ `app.emit` ⇒ 前端 `listen` 后照旧拉，**拉取仍由前端发起** ⇒ 照旧过 C2 闸门/防重入/状态行）；桌面侧**判据 7 的真行为与真机双设备仍未验**。定位随 CRDT 更新（冲突提示降级、SSE 升级为 CRDT 体验的延迟环节）。见 [实时协同分析](realtime-collab-analysis.md) §9。
...
| **P0** | **团队版（自建协作）** | Notion 团队版 / 语雀 / 飞书知识库 | 多用户 + 权限（协同后置）；不接外部通讯 App，全自建；**团队空间放弃零知识**（个人空间保留 E2E）—— 口径以[数据可见边界](sync-server-data-boundary.md) §0.5 为准 | 🔶 **M27 部分**（服务端 S1–S8 + 客户端 per-space 同步 + 账户 UI（U1–U4）+ 本地加密（E1）已落地；协同后置，[身份与隐私子路线图](identity-privacy-roadmap.md)） |
...
| **P1** | **块级协同地基（阶段 1：块版本 ＋ 冲突提示）** | 思源/语雀的近实时编辑体验 | 页级 LWW 会把同页并发编辑整页覆盖；阶段 1 先让"谁改了哪块"可见、可合 | ✅ **阶段 1 已落地**（块身份 ＋ `blockRev` ＋ 写层）／**阶段 2 客户端两侧已落地**（每页状态落盘、编辑器绑定、推/收两侧消费 `crdt_state`、桌面也收发状态；服务端开算**建议暂缓**）；见[阶段 1 就绪度](plans/2026-09-19-stage1-block-lww-readiness.md)、[blockRev 写层](plans/2026-09-22-block-rev-write-layer.md)、[阶段 2 开工](plans/2026-09-23-crdt-stage2-kickoff.md) 与[全上线冲刺](plans/2026-09-23-crdt-full-launch-sprint.md) §11–§15 |
...
- **M11.8 触发面与事件** 🚧（**第一档已落地：命令参数 + 结构化返回**）：
  ...
  - ✅ **第二批一方插件（继续用"真的写插件"压 API）**：`stale-triage`（陈旧页 → 批量打标签，**多条草稿汇总成一次确认**）、`orphan-pages`（逐页查反链 → 孤立页清单）、`csv-table`（CSV/TSV 导入 → Markdown 表格，带引号与换行的容错解析）。压出并修掉第二个缺口：**`api.pages.list()` 不返回 `created_at`**（"刚建的页面不算孤岛""这几天新建了几篇"这类判断做不了）——加字段是非破坏性的，比新加一条能力便宜得多，所以直接补上（`weekly-review` 也顺势报出"其中 N 篇是新建的"）。
  - ✅ **第三批一方插件**：`page-status`（当前页 → 状态属性，**没有属性定义时用标签优雅降级**）、`search-collect`（检索 → 清单草稿）。这一批**没有再压出需要改 API 的缺口**（说明能力面到了"够写有价值的插件"）。
  - ✅ **顺带补齐作者文档的空白格子**：22 条能力里有 **13 条没有 `desc`**——作者文档的能力表就是 13 个空格子，而那份文档的定位是"只看这一份就能写出插件"。现在全部补齐，并把门禁从"只要求 `ai:true` 的能力有 desc"扩到**全部能力**（缺描述 = 门禁失败）——这类"没人报错但文档是空的"缺口，只能靠门禁兜。
  - ⚠️ **已知缺口（能力面，有证据）**：**插件拿不到当前页的 id / 标题**——`page.current` 只给 content_json；`blocks.list` 省略 pageId 会回退到当前页，但也只给块的文本。写一方插件时撞到的具体后果：`page-to-md` 只能用日期给导出文件命名（「用页面标题当文件名」做不到），想做「把这一页整理成大纲 / 改写这一页」的插件同样拿不到页标题。候选修法：加一条只读能力 `api.page.meta()` → `{ id, title, kind }`（**不**改 `page.current` 的返回类型——那是 ABI 变更）。等第二个插件也撞到同一个缺口再动，别为一条插件加能力。**第二个缺口**：插件**不能新建属性定义**（`properties.set` 要既有 `attrId`）——`page-status` 的应对是"退回标签并在结果里说清"，体验可以接受，所以还不到加能力的程度；若将来出现"插件要建自己的数据结构"的真实需求，再考虑 `properties.define`。
  - ✅ **信任缺口已闭合**（原记录：插件更新后新增 `events`/权限不会重新征求同意）：启用时把声明（权限 + 事件）记成**授权快照**（`plugin_install.approved_json`），之后只认**新增**——声明扩张则由**后端**拒绝执行（`approval_required`）并停止事件派发，直到用户在插件管理里点「重新确认」。存量插件首次扫描补记一次（不这么做等于升级后全部停摆）；只跟踪启用中的插件（避免作者改 manifest 时被反复要求确认）。作者文档 §4.5.1 写清了"新增权限＝用户会看到暂停"，因为这直接影响他怎么发新版。
  - ⏳ **未做**：`open`（让宿主导航到某页）——那是新的副作用面，属 ABI 决策，不夹带在参数这档。
  - ✅ **事件发射点补齐（7/7）**：`app.started`（启动，插件列表先加载完再播报）/ `page.opened` / `page.deleted` / `space.switched` / `page.saved` / **`import.finished`** / **`sync.completed`** 都已接。派发走**零依赖公告口** `lib/pluginEvents.ts`（避免 store 循环依赖），并带「没人订阅就不发 IPC」的快速路径。
    - 后两个是**后台**事件（用户没做任何动作）：同步由 `useAutoSync` 每 5 分钟一轮、附件导入可能由别的界面发起，用户看不到它们有没有发生。所以只在**单一咽喉点**播报——`api.syncNow` / `api.syncWorkspace`（全仓 5 处调用都从这里过）与 `api.importAttachmentFiles`（4 个界面 + 编辑器 4 处都从这里过）：逐个调用点加必然漂。
    - 播报规则都有测试：导入空数组（用户取消选择）不播报；同步**整次失败**不播报（那时叫醒插件、插件按「完成」处理更糟）、部分成功只累加成功那些；一次调用只播一次（多工作空间求和，不是每个空间叫一遍）。
    - 注册表口径跟着收紧：**不再允许 `hosted: false`**——要么接上，要么从注册表删掉（门禁测试钉住）。保护本身（"宿主不发的事件不算订阅、且明确告知作者"）改用**合成的**声明继续覆盖，不会因为注册表里暂时没有这种事件就失去测试。
    - 升级影响（诚实说明）：**已装且启用**、又声明过这两个事件的插件，会在下次扫描时被判为「声明扩张」而暂停并显示「重新确认」——这是对的：以前那两个订阅其实从不触发，现在真的会跑代码了，用户该重新看一眼。示例 `activity-digest`（同步/导入摘要，零笔记权限）。
  ...
  - ✅ **触发面第二档：`page.context`（页面列表行菜单）**：`register({ menus: ["page.context"] })` 的命令出现在页面列表那一行的「⋯」菜单里（**在行上右键同样是它**），与内置动作之间有一道分隔。关键语义：这次运行的**"当前页"是用户点的那一页**，不是"现在打开的那一页"——`page-to-md` 因此不用改一行 JS 就能导出列表里随手点的一页（否则「导出这一页」导出的是别人，比没这个入口更糟）。入口渲染统一走 `PluginMenuItems`（启用状态过滤、带参数的命令转交命令面板、结果如实回显都在一处，各入口各写一份必然漂）。用户可见性：插件管理新增「命令会出现在：…」（标题取自注册表生成物 `lib/capabilities/menus.meta.ts`，前端不手抄）。
  - ✅ **触发面第三档：`file.context`（文件列表右键菜单）**：`register({ menus: ["file.context"] })` 的命令进入文件管理器右键菜单（与 `page.context` 共用同一份 `PluginMenuItems`）。关键是**入参由宿主给**、走已有的 `argsJson` 通道（与导入触发同一套）：`{ fileName, size, mime }`，并**刻意不给绝对路径**——插件本来就没有读文件的能力（能力表里只有 `files.list` / `files.export`），递 `path` 过去只会让人以为能读；"当前页"是**这个文件所在的那一页**。这类入口不渲染参数表单，所以声明了 `params` 会由校验器提醒（`menu_host_args_ignores_params`），组件层也据此**不再转交命令面板**（转交等于把刚拿到的文件信息丢掉）。示例 `attachment-report`（教学示例）。
  - ✅ **触发面第四档：`editor.toolbar`（编辑器顶部工具栏）**（M11.8 至此**全部落地**）：`register({ menus: ["editor.toolbar"] })` 的命令进入编辑器顶部工具栏的「⋯」菜单（与「导出 Markdown / HTML / PDF」同一处）——那里本来就有一排 28px 的图标按钮，而**插件给不出图标**，一排一模一样的 🔌 只会更难认，所以走文字项（注册表的 desc 把这条限制如实写给作者）。"当前页"是**正在编辑的这一页**，所以省略 pageId 的能力调用直接作用在它身上。示例 `page-to-md` 同时声明 `page.context` 与 `editor.toolbar`：同一份代码，列表里点哪页就导出哪页、工具栏里按当前页导出。
...
- **M11.9 声明式贡献面（无代码插件）** ✅（**视图 / 主题 / 参数化 / 导入触发 / 导出 / 视图落点均已落地**——这一档的目标「一个零 JS 插件即可安装并显示」已达成）：
  - ✅ **零代码插件**（`runtime: "declarative"`）：manifest 声明 `views`（`query` + `columns`），宿主查询/排序/格式化/渲染；视图入口在命令面板，点开是表格，点行打开页面；顶部可显示汇总。`columns`/`sort`/`kind` 都是宿主白名单；`runtime` 不认识的值直接拒载。
  - ✅ **两档能力边界写清且被检查**：声明式不申请权限、不收事件、读不了设置（写了会被如实提醒）；`logic` 档声明 `views` 同样提醒不生效。校验器走**独立路径**（不套用逻辑档的入口/语法/基线授权判定），作者 CLI 同步支持。
  - ✅ **示例 `examples/plugins/reading-board/`**：只有 manifest.json、没有一行 JS，被 `cargo test` / 作者 CLI / 示例检查三处钉住。
  - ✅ **主题插件（只出一组 token）**：manifest `theme.tokens`；白名单只含 24 项外观变量（布局度量刻意不在内）；值校验挡住 `url(` 这类会造成外部请求的写法（前后端各筛一遍）；单赢家 + 冲突报出；启用即应用、停用即恢复；示例 `warm-night`（零代码）。
  - ✅ **主题检查进校验器**（两档共用同一个检查函数）：白名单外的变量名给提醒、会让样式变成外部请求的值给**错误**——此前这些只在加载/应用时被静默丢弃，作者得不到反馈。
  - ✅ **导入触发**（manifest `triggers`：`kind` + `extensions` + `command`）：命令面板多出「导入：用「插件名」打开 .md」入口，点击 → 选文件 → **宿主**用现成的 `readTextFile` 读内容 → 把 `{ fileName, content }` 当 `argsJson` 交给 `run_plugin_command`。**没有新能力、也没有新命令**——它与既有的「参数」通道同源，所以权限与写中介原样成立：插件拿到内容后仍然只能通过 `api.*` 产出（写能力照样出草稿、照样要用户确认）。`kind` 只认注册表里的值（目前只有 `import`，写别的会在校验器里报错而不是静默不触发）；扩展名规范化成 `.md` 这种小写带点形式（`"MD"` / `".MD"` 都等于 `.md`）。同时把 `MAX_ARGS_BYTES` 16 KiB → 1 MiB，并把它注释的语义改成「**行为的界**」（够装常见文本/表格文件，又不让单次调用携带不可控数据量），而不是「参数必须是短值」——**数据通道仍然只有 `api.*`**。校验走**独立函数、两档各显式调一次**（与主题检查同一个理由）：`kind` 不认识 / `command` 为空 = 错误，`extensions` 为空或不合法 = 提醒；另在 discovery 之后单独查一次「触发指向的命令是否真的注册了」（命令 id 写错时宿主什么都不报，用户点下去才看到「命令不存在」）。插件管理里如实列出「会接住这些文件」；示例 `examples/plugins/md-outline/`（宿主读文件 + 写能力照样出草稿）。
  - ✅ **声明式视图的参数化（M11.9 收口）**：查询字段可以写成 `{ "fromSetting": "key" }`，由宿主在渲染时按用户设的值解析——**「用户可配」不再必然意味着「必须有代码」**（设置表单、视图渲染本来两端都在宿主手里）。默认值只有一处（设置声明的 `default`）；值不可用时按「没给」处理、绝不抛错，而写错的地方（引用的设置不存在、类型产不出值、`select` 候选项不在白名单里）校验器逐条指出。顺带修掉三个静默失效的坑：查询字段的 camelCase（`updatedWithinDays` / `titleContains`）此前被 serde 静默丢弃、声明式校验缺少「加载器会不会拒」的兜底、`select` 候选项的短写法（文档给的那种）会被拒载。示例 `reading-board` 改成真实的参数化用法。
  - ✅ **导出与 `kind: "export"` 触发**：新能力 **`api.files.export(fileName, content)`**（权限 `export:files`，风险 medium）——它**不直接写盘**：只登记请求，命令跑完后宿主**逐个弹系统保存对话框**，用户点保存才写、点取消就什么都没写（与草稿确认同一个中介思路：插件申请、用户决定）。**插件给不出路径**（`fileName` 只是建议名，目录会被去掉），所以它不是「写任意路径」——没有那个自由度，也就没有那个风险。上限：单文件 4 MiB、一次运行最多 4 个；事件里无效（没有对话框可弹，请求被忽略并写进日志）。ABI 选择上**没有**走「给结构化返回加 `export` 副作用面」那条路（那会让「导出」成为一个特例通道），而是走能力注册表：声明权限、逐次校验、进审计、进类型包与作者文档——写盘本来就该受权限与中介约束。命令面板多出「导出：用「插件名」保存为 .md」；插件管理里列出它。校验器对导出触发额外提醒一句：显式声明了权限却没写 `export:files`，用户点下去只会看到权限不足。示例 `index-export`（读页面清单 → 生成 Markdown 索引 → 用户选位置保存）。
  - ✅ **第一批一方参考插件**（分发策略里「一方先行」那一级）：`weekly-review`（参数表单 + 设置 + 草稿）、`page-to-md`（读当前页 + 导出）、`eye-care-theme` / `high-contrast-theme`（零代码主题）、`reading-board`（参数化声明式视图）——都能直接装来用，同时被 `cargo test` / 作者 CLI / 示例类型检查三处钉住。**写它们的过程本身就是 API 验收**：`blocks.list` 省略 pageId 回退到当前页就是这一步撞出来的（此前「能写当前页、读不到当前页」），顺手修掉。
  - ✅ **视图的两种落点（面板的更多形态）**：`views[].placement` —— 省略或 `"overlay"` 是**浮层**（"看完了就关"），`"rail"` 是**右侧常驻面板**（"一边看正文一边看着它"：点行**不关面板**、页面列表变了表格跟着更新，右侧竖条上占一个按钮、与 AI/目录/评论互斥）。两种形态**共用同一张表**（`PluginViewTable`：查询/列/汇总/点行打开那一页全都一样），差别只在开在哪里、关不关——这是 M11.10 那道启动闸门里"多数「我要一个插件面板」应当由声明式渲染器满足"的正面回答。口径与既有白名单一致：取值不认识**不拒载**（按浮层处理），校验器给出提醒并说明按什么处理；形态写错（不是字符串）会让整份 manifest 解析失败，所以是错误。作者可见性做全三处：作者 CLI 的视图清单印出「→ 浮层 / 右侧常驻面板」、应用内插件管理列出「会加上这些视图」、命令面板入口文案区分「插件视图：/ 插件面板：」。示例 `reading-board` 两种落点各示范一个。
  - 原始条目（保留备查）：manifest 即可声明面板/视图（复用数据库透镜 query+columns）、主题 token、导入导出触发、命令参数、菜单与斜杠项。**验收**：一个「阅读统计面板」插件**零 JS** 即可安装并显示。
...
- **M11.11 分发与信任**（原 M11.4，重编号；**形态与供给侧策略见[插件分发策略](plans/2026-09-10-plugin-distribution-strategy.md)**）🚧（拆三小档，**a 已落地**）：**a. 索引协议 + zip/URL 安装** ✅ 已落地——`plugin-index.json` 静态索引规范（任意位置可托管）+ 索引签名（复用 updater 的 minisign 习惯）+ `install_plugin` 支持 zip/URL 且先校验后落盘，**不做商店 UI**（这是复用既有设施，边际成本≈0）。**落地口径**：索引/包只走 https（例外本机回环）+ 体积上限（读取途中也守）+ `sha256` 校验 + 解包到临时目录后才动插件目录 + 给了公钥就必须验签通过（取不到签名也算失败）+ 解包侧 zip-slip/绝对路径/符号链接/文件数/解压体积全拒；界面显示来源（拥有者 + 域名）与签名状态，**不内置任何"官方索引"**（信谁由用户决定）；「从索引安装（给 URL）」（含「装 zip 包」）已进插件管理。样例索引 [docs/plugin-index.example.json](plugin-index.example.json)、发布流程见[插件配方 § 发布](plugin-recipes.md)；**b. 治理与信任**——publisher 签名 + 撤回两级（索引侧 + **应用侧离线撤回列表**）+ 自动扫描 + 安全评分卡 + 开发者政策 + 漏洞披露 + `license` 必填；**c. 应用内市场 UI（最后做）**——订阅**多个**索引（自托/社区/企业内网）+ 搜索/兼容提示/更新/撤回。**闸门（硬性）**：作者文档 + ≥3 个真实第三方插件（**c 尤受约束**）；**硬前置：M11.13 进程级隔离须先于 a 完成**；并与「绝不跟踪」对齐——禁下载计数/埋点排行/竞价位，「评价」走社区帖子（`discussionUrl`）。
- **M11.12（可选）插件数据同步** 🗓（规划）：`sync:data` 声明 + 空间级 `plugin_data` 增量同步（E2EE 兼容）。默认不同步。
- **M11.13 隔离强度：插件宿主子进程化 + OS 级资源限制** 🚧（**方案已拍板，阶段 1+2 已落地——应用已真正跑在子进程上**：[插件宿主子进程化方案](plans/2026-09-10-plugin-host-isolation-plan.md)，6 个决定见 §8.1、进展见 §8.2。**阶段 1**：`--plugin-host` 分流（在 Tauri/单实例之前）、长度前缀 JSON 帧、握手、子进程主循环、父进程 `HostClient`（含"drop 不留孤儿"）；CI 已从 `cargo test --lib` 扩到 `cargo test`。**阶段 2**：能力调用走 IPC 回父进程（`Cap`/`CapResult` 两帧 + 父进程 `run_with_server` 服务）；命令与事件两条路都切到 `run_command_via_host` / `run_event_via_host`；`cap_stub`/进程内执行路径已删除（D7），14 处测试全部迁到生产路；实测启动 ~5 ms/次、能力 IPC ~0.1 ms/次。**阶段 1 + 2 已完成——应用现在真的跑在子进程上**（协议 + 帧 + `HostClient`；能力调用走 IPC 回父进程服务；命令与事件两条路都已切流；`cap_stub` 与进程内执行路径已删除，14 处测试全部迁到生产那条路；实测进程启动 ~5 ms、每次能力 IPC ~0.1 ms）；**阶段 3a 已落地**：超时 = **真杀进程**（到点 `kill`，错误码 `timeout`）、崩溃语义（异常退出 → `plugin_crash` + 退出码/信号）、错误码归一到注册表并新增 `plugin_crash`；**3b 主体已落地**：RSS 看门狗（默认 256 MiB，超限即杀，Linux/macOS 可读；实测空闲子进程 10.5 MiB、吃 4 MB 的插件峰值 30.4 MiB）+ 子进程环境白名单（应用变量一律不递）；**取消已接前端**（`runId` + `cancel_plugin_run`，点了就真杀，文案改成"已终止插件"）、**进程内测试面已清零**；**崩溃与峰值内存已进审计**（命令每次都记、事件只在失败时记）；**仍待做**：Windows RSS、内核强制上限（rlimit / Job Object）、打包产物手工验收）：Boa **无分配预算 API**（0.21.1 与最新 0.22.0 均已核实，上游 issue 亦不涉内存）→ 内存炸弹由 M11.5 的限流分配器闭合，但**原生崩溃 / 段错误 / UB / OS OOM-killer / 进程级资源账目**只有 OS 进程边界能兜。交付：宿主挪到独立子进程 + 私有 IPC + **OS 级内存/CPU 限制** + 崩溃只终结该次调用 + 跨进程可中止（顺带彻底解决 M11.5 遗留的"超时只是遗弃线程"）。**排期**：**不阻塞 M11.7**（紧迫性下降），改为 **M11.11 分发的硬前置**——第三方插件进场前必须完成；**保留升级条款**：一旦观察到 Boa 段错误与插件相关，立即升 P0。**不接受降级为不做**（有存量插件后再改宿主 = 重做 `RunState`/命令路由/超时取消）。依据 [方案 §3.11](plans/2026-09-10-plugin-evolution-plan.md)。**方案要点**：子进程是**纯解释器**（不碰 DB、不拿解密密钥、没有路径；能力全部 RPC 回父进程——今天的现状是密钥与不可信代码同进程，这条边界同时买到崩溃隔离与「不可信进程里没有秘密」）；IPC 用同二进制 re-exec + stdin/stdout（不新增二进制，打包签名不变）；**取消/超时 = 真的杀进程**（顺带关掉 M11.5 的线程空转）；OS 上限 = Linux rlimit / Windows Job Object / 三平台统一 RSS 看门狗；分 4 阶段约 9–10 天，每阶段独立可合、任何时刻只有一个后端。
...
### M27 — 团队版（自建协作，不接外部通讯 App，方案已移入私有仓库 `shuyonote-sync-server`）
> 把 ShuyoNote 从「单用户 · 多设备 · 本地优先」演进为「**可自建团队知识库**」——**不接微信/企业微信/钉钉**，账号、通知、讨论、采集全自建。两根柱子：**账号/认证（多用户）、权限模型**；**实时协同后置（P2）**。四项关键决策已拍板——**团队空间放弃零知识**（个人空间保留 E2E；口径见[数据可见边界](sync-server-data-boundary.md) §0.5）、协同后置 P2、部署私有化、暂不建实时聊天。
> **进度**：✅ **服务端 S1–S8** 已落地（认证/空间/成员/角色/同步隔离/**空间级附件分桶+SHA-256 校验+鉴权**/审计日志/Docker 部署）；✅ 客户端 **per-workspace `sync_profiles`**（一个客户端持多个身份，各空间同步到各服务器）+ 按 profile 同步 + SyncPanel 每空间一行；同步 E2E（见 M2）；✅ **K1 设备密钥**（2026-09-15，schema v14）——「个人/无账户」部署的机主拿一把 `sk_` 钥匙即拥有该空间，服务端只存指纹、可签发可作废，第一把由 CLI 发。
...
| **块级 CRDT（阶段 1 ✅ / 阶段 2 客户端双侧已上、服务端开算暂缓）** | 阶段 1：块身份归属已拍板；`blockRev` 纯函数（Rust 14 条 ↔ TS 15 条判据）＋ 18 类节点声明字段全接入；写层已落地。阶段 2：**Slice A**（`content_json` ⇄ `ydoc` 唯一实现 ＋ 6 条判据）＋ **Slice B**（开关接进 `docContent` 两条出入口，默认关 ⇒ 逐字节不变，＋ 9 条路径级判据）都在 dev 上 | **两条红已消**（`52bf9010`：① 初始化环 `docContent → crdt/plane → yDocBridge → editor/config` ⇒ **平面实现改成注入**（那一层只认签名，`src/main.tsx` 启动时注册；开着未注册 ⇒ 如实抛错），那一层不再拖编辑器节点表；② 层模块改名 `contentJsonYDoc.ts → yDocBridge.ts` ＋ `plane.ts` 改措辞 ⇒ 门禁命中 **5 → 0**，基线 562 处未动）。**Slice B 的 5 条判据已全绿**（`src/lib/crdt/plane.path.test.ts` 9 条：④ 引用完整性用**产品自己**的 `topLevelBlockIds` 读；⑤ 派生口径如实降级成"落后**有痕**（`text_stale`／补算队列）＋ 补算器能收口"），并补了一条**实测撞出来的**第 6 条：平面开着时**存量未补种块身份的页面会炸在读路径上**（`readAllContents` 是全库扫描）⇒ "开关成为设置项"的前提**从三条变四条**（[开关策略](plans/2026-09-23-crdt-slice-b-workorder.md) §0.6）。★ **当前卡点：平面住哪条边界还没拍** —— 它今天钉在**磁盘边界**（`docContent` 两读一写），而**磁盘边界只有一个版本可过 ⇒ 合并不了任何东西**（开着只是把已落盘 JSON 归一化一次），且**桌面 Rust 侧根本没接**（唯一生产 importer 是 `platform/web.ts`）⇒ 补种块身份**只服务"每页持久化 ydoc 状态"那条路**，边界没拍之前不做（[决策简报](plans/2026-09-23-crdt-plane-boundary-decision.md)：**2026-09-23 owner 已拍 = 服务端合并** ⇒ 补种块身份**不需要**（施工单 §0.6 前提 4 划掉）、前提 3 改成"wire 载荷带版本标记 ＋ 老客户端如实降级"，且磁盘边界那三处接线**应当撤出**）。★ **冲刺全上线已开**（owner：无客户 ⇒ 放弃向后兼容）：**S1** 实测红线（"从 JSON 新建的状态**不可合**"⇒ 一块变两块且 `blockId` 重复）✅ → **S2a** 活会话 ✅ → **S2b** 状态落盘（新表 `page_crdt`）✅ → **S3a–S3b-2e** 编辑器真绑定（含**浏览器级判据**：打字 ⇒ 刷新 ⇒ 字还在，并用变异实测证明它咬人）✅ → **S4a** 远端状态并入本机（次序无关、幂等）✅ → **S4b-0/1a** wire 载荷带版本标记 ＋「推」那一侧接上 ✅ → **S4b-1b**「收」那一侧接上（`applyChange` 按版本解、**注入**落地实现，`remoteApply.test.ts` 3 条）✅ → **S5-0** 勘察（服务端是**哑中转** ⇒ 阶段 1「只存不算」免费成立；**阶段 2 开算的真实成本是 Rust 侧要有一份与 JS `yjs` 互操作的实现 ＋ 对拍**，建议推迟）✅ → **S6a/S6b** 派生落后**有痕** ＋ 块身份穿过合并 ✅ → **S7-1/S7-2** 桌面侧 `page_crdt` 存取（Rust 三条函数 ＋ Tauri 命令接上，顺带修掉桌面每次开页弹「绑定失败」）✅ → **S8** 血统护栏（两条**独立创建**的状态拒绝合并，有痕）✅。**第 41–45 轮收口（实况以[全上线冲刺](plans/2026-09-23-crdt-full-launch-sprint.md) §11–§15 为准）**：S9 服务端首写者裁定**已实现并已发版**（探针 `/lineage-claim`=401）＋ 桌面侧 claim **已接**（`WEB_ONLY` 登记已撤）＋ claim 的**两套 id 修正**（本地 `workspace_id` ≠ 远端 `space_id`；403 ⇒ `unavailable`）＋ **桌面侧也消费 `crdt_state`**（收进 `page_crdt_pending`、打开页面时合并/承接；推的载荷也挂状态）＋ **yrs 对拍已做**（格式层可行）＋ **状态体积读数**（~158 B/块）。**只剩**：**真机双设备验收**（要人手）／**真账号端到端 claim 探针**（要凭据）／**S5 阶段 2**（[决策稿](plans/2026-09-23-s5-server-merge-decision.md)：**建议暂缓**）。之后按[层清单决策树](plans/2026-09-18-doc-content-layer-inventory.md)逐个判 Slice C/D 的新文件；**三线联合验收**（与国密、AI 覆盖一起测）见 [JOINT-ACCEPTANCE](JOINT-ACCEPTANCE.md) | [块 ID 归属](plans/2026-09-18-crdt-block-id-ownership.md) ＋ [阶段 1 就绪度](plans/2026-09-19-stage1-block-lww-readiness.md) ＋ [blockRev 写层](plans/2026-09-22-block-rev-write-layer.md) |
...
| **近实时协作** | 同页冲突提示 / presence / 评论@通知 / SSE 推送均已落地（服务端 `collab.rs` ＋ 客户端命令/UI；**SSE 桌面与 Web 都有**，桌面侧判据 1–6 有读数、7 的真行为未验） | **块级真协同已不再是缺口**（第 41–43 轮：客户端两侧消费状态、块级合并在 dev 上；服务端开算见[决策稿](plans/2026-09-23-s5-server-merge-decision.md)：**建议暂缓**）；**明确不做**的是实时聊天；「真·实时」（WebSocket/光标）仍后置 | [实时协同分析](realtime-collab-analysis.md) ＋ [SYNC](SYNC.md) |

> ★ **三条战役相交的格子**（国密 × 全库 AI 覆盖 × 块级 CRDT）由 [三平面联合验收](JOINT-ACCEPTANCE.md) 统一登记：
> 就绪面板 `node scripts/joint-acceptance.mjs`（三态：就绪 / 未就绪 / **未实查**，未实查不算红）＋ **格子 j1–j8**
> （每格写清跨哪几个平面、谁跑、判据与下限）。口径是那句话：**分开绿 ≠ 一起绿** —— 三条线各自绿，交界的那些格子
> （加密库上的新表、导出的快照、平面开着时的全库扫描与两设备合并）此前没有任何人负责。
...
| 同页并发编辑 | 页级 LWW ⇒ 同页并发可能整页覆盖 | **块级 CRDT**：阶段 1 ✅（`blockRev` ＋ 冲突提示）／**阶段 2 客户端两侧已落地**（每页状态 ＋ 推/收两侧消费状态；服务端开算暂缓）；见 [§3.5](#35-2026-09-在飞战役按方案-p-阶段跟踪不进-m-编号) |
