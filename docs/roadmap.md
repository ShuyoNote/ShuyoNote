...
- **M11.6 ABI v1 冻结 + 能力注册表 + 权限模型** 🚧（**契约层已落地**，其余待做）：
  ...
  - ⏳ **未完成**：`plugin_install`/`plugin_data` 两张表与迁移（现仍是 `plugin_state` 泛 KV）；§3.9 的跨空间/`space_locked`（v1 能力全是 `current-space`/`app`，暂无跨空间能力，故尚未触发）与 `plugin_data` 按 scope 落库；§3.10 能力调用审计轨迹；作者 CLI（`validate` / `dev` 热重载）；§6.3 正名（内置命令组重命名 + 删死代码）。
