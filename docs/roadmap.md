...
- **M11.5 前置硬化（时限 · 可见性 · 一致性）** 🚧（**大部分已落地**，[进化方案](plans/2026-09-10-plugin-evolution-plan.md)）：
  ...
  - ⏳ **未完成**：**插件日志环缓冲 + `__toast` 接通 UI**（`__toast` 目前仍只写 stderr）；**长任务运行态可见 + 用户可随时取消**（对齐 Figma 的 "Running" 提示与取消入口）。
  - ✅ **测试与 CI**：新增 16 条 Rust 单测（沙箱逃逸回归 / 循环预算 / discovery 快失败 / with_timeout / id 白名单 / `main` 矩阵 / `read_manifest` 矩阵 / 启停往返 / 卸载清理 / **分配炸弹只终结该次调用** / 分配器自身 5 条）与 8 条前端单测；**`cargo test --lib` 已进 CI**（`ci.yml` 新增 `rust-tests` job，依赖与工具链照抄 `release.yml`）。
  - **验收状态**：死循环插件不再挂调用方、分配炸弹只终结该次调用（均有测试断言）；失败路径 UI 可见；禁用插件 IPC 不可执行 —— 已达；**「运行中的插件有可见状态且能取消」未达**（见上「未完成」）。
