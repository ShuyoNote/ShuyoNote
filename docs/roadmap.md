...
> 把 ShuyoNote 从「单用户 · 多设备 · 本地优先」演进为「**可自建团队知识库**」——**不接微信/企业微信/钉钉**，账号、通知、讨论、采集全自建。两根柱子：**账号/认证（多用户）、权限模型**；**实时协同后置（P2）**。**服务端已实现 S5**（认证/空间/成员/同步隔离），设计见 sync-server 仓库；本仓库聚焦客户端侧（登录 + Bearer + 空间绑定 + 成员 UI）。四项关键决策已拍板——E2E 团队空间放弃零知识（决策 A：传输加密 + 服务端信任边界，个人空间保留 E2E）、协同后置 P2（先共享+权限+评论）、部署私有化、暂不建实时聊天。
