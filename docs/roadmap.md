...
| **全库 AI 覆盖** | 派生文本 ✅ / 块 ✅ / 嵌入 ✅；抽取器注册表按 mime 分派 ＋ 跨实现 conformance；**本机端点红线**（抽取不得走远程 provider） | 面板侧"消费抽取结果"那一层未落地（触发点在导入/附件那条路）；真模型读数受本机服务是否在跑影响 | [覆盖方案](plans/2026-09-17-knowledge-base-ai-coverage-plan.md)（总账 §8.0） |
