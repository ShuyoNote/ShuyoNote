...
- **M11.7 能力扩容：读 + 受控写（与 AI 工具层合并）** 🚧（**读能力已落地**，写能力与 AI 工具层合并待做）：
  ...
  - ⏳ **未完成**：`properties.set` / `tags.add`（需要给 `applyDraft` 加两种新载荷 + 对应的写 api）；**把 `src/lib/ai/tools.ts` 重构为消费同一注册表**（AI 宿主与插件共用一个能力层；需要给每条能力加 `desc` 供 LLM 用 + 一份"能力 id → 前端 api 调用"的绑定表，并由门禁校验绑定覆盖）。
