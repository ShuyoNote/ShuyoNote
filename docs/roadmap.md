...
- **M11.8 触发面与事件** 🚧（**第一档已落地：命令参数 + 结构化返回**）：
  ...
  - ⚠️ **已知缺口（能力面，有证据）**：**插件拿不到当前页的 id / 标题**——`page.current` 只给 content_json；`blocks.list` 省略 pageId 会回退到当前页，但也只给块的文本。写一方插件时撞到的具体后果：`page-to-md` 只能用日期给导出文件命名（「用页面标题当文件名」做不到），想做「把这一页整理成大纲 / 改写这一页」的插件同样拿不到页标题。候选修法：加一条只读能力 `api.page.meta()` → `{ id, title, kind }`（**不**改 `page.current` 的返回类型——那是 ABI 变更）。等第二个插件也撞到同一个缺口再动，别为一条插件加能力。
