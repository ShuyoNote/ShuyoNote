...
- **M12.3c md 应用内打开** 🗓（[分析](plans/2026-08-30-md-in-app-open-plan.md)）：点 `.md` 在文件管理器中**应用内只读渲染**（复用 `readTextFile` + `mdToHtml`）+ 一个明确的「转为笔记」按钮（复用 `MarkdownImportDialog`）；不默认自动转页面，避免破坏「文件=网盘」心智。
