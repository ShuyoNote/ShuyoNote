...
| **P3** | **内联 AI 起草**（就地写/扩/改 + 高亮待定块 + 快捷动作） | wolai / FlowUs / Notion AI | 把 M17 底座变成「所见即所得」的文档内创作；与侧边栏分工 | 🗓 M18（[方案](plans/2026-08-24-inline-ai-draft-plan.md)） |
| **P1** | **Wiki 织网增强**（未链接提及 + 双链别名 + 精确块链） | Obsidian 双链 / wiki | 把「双链」做深，让笔记真正织成网 | 🗓 M19 |
| **P1** | **模板变量 + 语义检索（RAG）** | Notion 模板 / 思源 AI | 复用模板中心；搜索从「关键词」升级为「语义」 | 🗓 M20 |
| **P2** | **静态 wiki 导出 + 关系图探索** | 独立 wiki 站点 / FlowUs 图谱 | 「本地优先 + wiki」的终局表达；图谱从能看变能探索 | 🗓 M21 |
...
- **M18.1 内联起草条 + 模板下拉** 🗓：`/` 或空行唤起起草条（「告诉 AI 你想写什么…」）+「用 AI 起草」模板（文章大纲 / 内容简介 / 社交媒体帖子 / 电子邮件 / 广告文案 / 短篇故事 / …更多），模型选择 + 发送。
- **M18.2 流式写入 + 待定块高亮** 🗓：内容流式写入当前页一个 `ai-pending` 高亮块（**不落库**）；状态条「AI 正在创作···」+ `R` 重新生成 / `ESC` 停止。
- **M18.3 生成后动作菜单** 🗓：完成（落库 + 去高亮）/ 关闭（丢弃）/ 续写 / 扩写 / 重新生成 / 创建新页面并插入内容；附「AI 回复可能有偏差，仅供参考。回顾思考过程」。
- **M18.4 安全与验收** 🗓：预览→完成才写库；`scripts/smoke-web.mjs` 新增断言且原断言无回归；`tsc`/`vite build`/`cargo check` 通过。
> 状态：**规划（建议）**。最小闭环先行（M18.1 → M18.3 的「完成/关闭」）；续写/扩写/重新生成/新建页二期。

### M19 — Wiki 织网增强（P1）
> 把「双链」从"能链接"做深为"真织网"，对标 Obsidian 双链 / wiki。
- **M19.1 未链接提及（Unlinked Mentions）** 🗓：扫描正文里出现但**未打 `[[ ]]`** 的页面标题，页面底部提示"是否改为链接"，一键转链。
- **M19.2 双向链接别名** 🗓：`[[标题|别名]]` 显示别名、仍指向目标页；`[[页面#块]]` 直接链到具体块（复用现有 `blockId`）。
- **M19.3 链接建议增强** 🗓：输入 `[[` 时按当前页上下文/最近编辑智能排序；粘贴标题自动识别可链接。

### M20 — 模板变量 + 语义检索（P1）
> 模板复用 + 搜索从「关键词」升级为「语义」。
- **M20.1 模板变量** 🗓：模板支持 `{{date}}` / `{{title}}` / `{{selected}}`，建页时自动填充。
- **M20.2 语义检索（embedding）** 🗓：本地/云端 embedding 向量化页面，支持"意思相近"的语义搜索（保留 FTS 关键词作为兜底）。
- **M20.3 语义检索接入 AI** 🗓：侧边栏 AI 可检索语义结果，提升"问知识库"能力。

### M21 — 静态 wiki 导出 + 关系图探索（P2）
...
| 语义检索 | 仅 FTS 关键词搜索 | **M20 语义检索（embedding）+ 接入 AI 问答** |
| wiki 导出 | 无「把你的知识库变成可浏览网站」能力 | **M21 静态 wiki 导出** |
