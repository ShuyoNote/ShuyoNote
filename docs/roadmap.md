...
- **近实时协作**：同页冲突提示、presence（谁在编辑）、评论/@/通知、SSE 推送——服务端 `collab.rs` 与客户端命令/UI 已落地。⚠️ **两条如实记**：SSE **只有 Web 端**（桌面走轮询 ⇒ "近实时"在桌面上等于轮询间隔）；定位随 CRDT 更新（冲突提示降级、SSE 升级为 CRDT 体验的延迟环节）。见 [实时协同分析](realtime-collab-analysis.md) §9。
...
| **近实时协作** | 同页冲突提示 / presence / 评论@通知 / SSE 推送均已落地（服务端 `collab.rs` ＋ 客户端命令/UI；**SSE 仅 Web**） | **块级真协同已不再是缺口**（第 41–43 轮：客户端两侧消费状态、块级合并在 dev 上；服务端开算见[决策稿](plans/2026-09-23-s5-server-merge-decision.md)：**建议暂缓**）；**明确不做**的是实时聊天；「真·实时」（WebSocket/光标）仍后置 | [实时协同分析](realtime-collab-analysis.md) ＋ [SYNC](SYNC.md) |
