...
- **M11.1 插件底座** ✅（v1.14.0）：插件目录扫描 + `manifest.json` 校验 + **boa 受限运行时 + 白名单 API**（`register`/`__get_current_page`/`__pages`/`__toast`）+ 命令并入命令面板 + `enabled` 持久化；内置示例插件 + Rust 单元测试。**补记（2026-09-10，代码核实）**：白名单实为 4 个（`__insert` 见 M11.2b）；`__pages()` 返回**总数**而非页面列表；`__toast()` 只写 stderr、**未接 UI**。
...
- **M11.5 前置硬化（时限 · 可见性 · 一致性）** 🗓（规划，[进化方案](plans/2026-09-10-plugin-evolution-plan.md)）：给 Boa 设 `RuntimeLimits`（死循环变可捕获错误，而非永久空转线程）；discovery 走带超时的工作线程、`list_plugins` 改 `async`；卸载清状态行、`demo` 用播种标记不再复活；安装**先校验后落盘**并清理半残目录；`run_plugin_command` 后端强制 `enabled`；插件失败对用户可见（接 toast/confirm）、`__toast` 接通 UI；`main` 校验修正（当前误拒 `./main.js`、放行 `..`）；**资源上限：已核实结论 + 进程内防线**（Boa `RuntimeLimits` 0.21.1 与最新 0.22.0 均无堆/分配预算 API，上游 issue 亦不涉内存 → **不能设**；但**进程内可行**：装 **thread-local 限流分配器，超预算时 panic 让它 unwind**，由既有 `Disconnected` 分支转成干净错误、应用存活；**注意不能返回 null**，stable Rust 下那等于 abort 应用）；同时按"内存"重算 `loop_iteration_limit`（峰值 ≈ 迭代数 × 每次迭代分配，原 1e7 过大 → run 降到 1e6 量级）；**长任务运行态可见 + 用户可随时取消**（对齐 Figma 的 "Running" 提示与取消入口）；补测试（逃逸回归/超时/拒绝矩阵/持久化/**分配炸弹只终结该次调用**）并让 `cargo test` 进 CI。**验收**：死循环插件既不挂窗口也不泄漏线程；**分配炸弹插件应用存活**；所有失败路径 UI 可见；禁用插件 IPC 不可执行；运行中的插件有可见状态且能取消；**资源结论已落地（方案 §3.11）**。
- **M11.6 ABI v1 冻结 + 能力注册表 + 权限模型** 🗓（规划）：建 `capabilities/capabilities.json` **单一事实源**（每条能力必须声明 `scope`）+ 生成（JS shim / Rust 绑定 / 作者文档）+ `check-capabilities` 门禁；Boa 只留 `__cap`/`__log` 两个私有原语，插件只见生成的 `api.*`（**传输无关**，为 M11.10 复用）；`apiVersion` 必填且**语义化**（`"1.0.0"`，对齐 Figma manifest）；manifest `permissions` 为**带 `reason` 的对象数组**，安装界面显示「权限 + 理由」（对齐 Figma `networkAccess.reasoning`）+ 后端逐调用校验；**权限 × 加密边界**落地（能力上下文只含活动空间、**锁定空间返回 `space_locked` 且不隐式解锁**、`plugin_data` 按 scope 落库：space 级进空间库随 SQLCipher 加密、app 级才进 `meta.db`）；`plugin_install`/`plugin_data` 两表替代泛 KV；插件 id 收紧为单段 `[a-z0-9-]`；**生成并发布 `@shuyonote/plugin-types` 类型包**（作者侧 IDE 补全/编译期检查，对齐 `@figma/plugin-typings`/`@types/vscode`）；**审计与可见性落地**（权限使用审计轨迹 + 文档化错误码表）；**作者 CLI 与热重载从 M11.11 前移到本档**（开发循环与 API 同时交付）；新建 `docs/plugin-api.md` 作者文档 + 示例插件。**验收**：只看文档不读源码的人能写出可安装可运行的插件（示例插件在类型包下 `tsc` 零错误）；未声明权限的调用被拒；锁定空间返回 `space_locked`（有测试）；空间级插件数据随空间加密/导出（有测试）；设置页可查插件的权限使用记录。**这是「可信」定位的技术落点**（[方案 §3.9/§3.10](plans/2026-09-10-plugin-evolution-plan.md)）。
- **M11.7 能力扩容：读 + 受控写（与 AI 工具层合并）** 🗓（规划）：读能力（当前页/列表/搜索/单页/标签/反链/文件元数据）+ 写能力三档（`insertText` 即时；建页/追加块/改属性/加标签**走草稿确认**）；**把 `src/lib/ai/tools.ts` 重构为消费同一注册表**——AI 宿主与插件共用一个能力层，仓库内不留第二份语义工具实现。
