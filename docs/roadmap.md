...
| **P0** | **团队版（自建协作）** | Notion 团队版 / 语雀 / 飞书知识库 | 多用户 + 权限（协同后置）；不接外部通讯 App，全自建；团队空间放弃零知识（个人空间保留 E2E） | 🔶 **M27 部分**（服务端 S1–S8 + 客户端 per-space 同步已落地；账户 UI / 本地加密待，[身份与隐私子路线图](identity-privacy-roadmap.md)） |
| **P0** | **多账户切换 UI**（空间身份标签 / 当前目标 pill / SyncPanel 登录+空间下拉） | 多账号工作流 | 一人多服务器×多空间时看清「我在同步到哪」；低成本 | 规划（M27-UI，U1–U3） |
| **P1** | **本地静置加密**（口令→Argon2id→密钥→加密 DB+附件，默认关） | Notion 本地加密 / 思源 | 个人无服务器也想私密；与同步 E2E（M2）互补 | 规划（M27-E，E1–E2） |
...
> 把 ShuyoNote 从「单用户 · 多设备 · 本地优先」演进为「**可自建团队知识库**」——**不接微信/企业微信/钉钉**，账号、通知、讨论、采集全自建。两根柱子：**账号/认证（多用户）、权限模型**；**实时协同后置（P2）**。四项关键决策已拍板——E2E 团队空间放弃零知识、协同后置 P2、部署私有化、暂不建实时聊天。
...
> **仍待（身份与隐私，详见 [身份与隐私子路线图](identity-privacy-roadmap.md)）**：多账户切换 UI（空间切换器身份标签 / 当前同步目标 pill / SyncPanel 登录+空间下拉绑定 / 账户中心看板）；个人「密钥」鉴权（服务端签发/作废 device key）；**本地静置加密（at-rest，口令→Argon2id→密钥→加密 DB+附件）**；可用性加固（备份 + `/health/db` 深探）。
