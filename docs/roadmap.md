...
- **M10.2b 空间增删改补全** ⏳（[规划](plans/2026-08-22-workspace-crud-plan.md)）：`create_workspace` 种子默认首页；`rename_workspace(id, name)` 按 id 重命名（替代仅活动空间）；删除前导出/备份提醒；软删保留、物理清理归 M14。
...
- **M10.3b 空间级设置落地** ⏳（[规划](plans/2026-08-22-workspace-crud-plan.md)）：`WorkspaceMeta` 增 `icon/sort_order/theme`；侧栏空间项显示图标 + 排序；`set_workspace_settings`；每空间主题（可选覆盖）。
...
- **M14.1 空间统计面板** ⏳：`storage_stats`（数据库/附件/回收站/版本历史/软删空间/临时文件各分项占用）+ 前端「存储」面板展示。
- **M14.2 清空回收站** ⏳：`clear_trash` 物理删除软删页面树 + 级联释放附件引用；二次确认 + 用量预览 + 结果提示。
- **M14.3 清理孤立附件** ⏳：`cleanup_orphan_attachments` 删除「零引用」磁盘字节（内容寻址 hash 差集）。
- **M14.4（可选）**：清理版本历史（超阈值）/ 临时文件 / 软删工作空间。
