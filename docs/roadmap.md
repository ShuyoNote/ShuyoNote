...
- **M11.8 触发面与事件** 🚧（**第一档已落地：命令参数 + 结构化返回**）：
  ...
  - ⏳ **未做**：manifest 声明斜杠菜单/页面右键/文件右键/编辑器菜单入口；事件钩子 v1（`app.started`/`space.switched`/`page.opened`/`page.saved`/`page.deleted`/`import.finished`/`sync.completed`）；插件设置（manifest schema → `plugin_data`）。
