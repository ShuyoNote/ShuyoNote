...
- **M11.6 ABI v1 冻结 + 能力注册表 + 权限模型** 🚧（**契约层 + 状态表 + 审计 + 正名已落地**，仅剩作者 CLI）：
  ...
  - ⏳ **未完成**：作者 CLI（`validate` / `dev` 热重载，前移到 M11.11 前再补）；§3.9 的跨空间与 `space_locked`（v1 能力全是 `current-space`/`app`，**暂无跨空间能力所以尚未触发**；表结构与落库约定已按 §3.7 就位，等 M11.7 加跨空间能力时生效）。
...
- **M11.7 能力扩容：读 + 受控写（与 AI 工具层合并）** 🚧（**读能力已落地**，写能力与 AI 工具层合并待做）：
  ...
  - ⏳ **未完成**：受控写能力（`pages.create` / `blocks.append` / `properties.set` / `tags.add`，走草稿确认——拟把草稿随 `PluginRunResult` 回传、前端复用既有 `applyDraft` 预览面板）；**把 `src/lib/ai/tools.ts` 重构为消费同一注册表**（AI 宿主与插件共用一个能力层；需要给每条能力加 `desc` 供 LLM 用 + 一份"能力 id → 前端 api 调用"的绑定表，并由门禁校验绑定覆盖）。
