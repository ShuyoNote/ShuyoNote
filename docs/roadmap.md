...
- **M11.9 声明式贡献面（无代码插件）** 🚧（**第一档已落地：声明式视图**）：
  ...
  - ⏳ **未做**：导入导出触发（扩展名 → 调用哪个命令）；面板的更多形态（侧栏常驻面板 / 自定义视图位置）；`runtime: "declarative"` 的**参数化**（让视图的查询条件可由用户设置——目前查询固定在 manifest 里，需要用户可配就得写 logic 档）。
