...
- **M16.6 web 能力补齐（P0）** 🗓（规划，[建议清单](plans/2026-08-24-web-polish-backlog-plan.md)）：web 端空桩命令让桌面/web 不对称——**附件移动/批量删除、存储统计精确化、全文搜索**是影响真实使用的最优先项。
- **M16.7 web 体验优化（P1）** 🗓（规划，同上）：侧边栏拖拽补完（自动展开/滚动/插入线）、大媒体内存、自动化测试补强。
- **M16.8 web 数据安全（P1）** 🗓（规划，同上）：写库失败回滚与提示（整库快照 `persist()` 无失败回滚，会静默丢数据）。
