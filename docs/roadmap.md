...
> **MVP 切割（阶段 1）**：pdf.js 分页渲染 + 批注 overlay（高亮/荧光笔/画笔/便签，坐标归一化、不写回源 PDF）+ 内容寻址持久化 + 「摘录成块」进反链/搜索。**阶段 2**：写回 PDF / OCR / 文本层精确划词（Tauri 专属、很贵、长尾）。**阶段 3**：AI 帮读（复用 M17/M18 薄 Agent）。
