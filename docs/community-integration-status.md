...
| **界面那一段**（订阅 → 安装） | ⏳ **等有 GUI 的机器** | 代码级端到端已有测试（真夹具 + 真环回 HTTP + 真解包 + 真 discovery + 真记账）；差"在真实应用里点一次" |

## 五、还差什么（一张清单）

1. **社区侧**：帖子页支持 `Accept: application/json`（一条分支）→ 应用这边**一行不用改**；
   Web 版另需 `Access-Control-Allow-Origin`（桌面走原生、没有 CORS，别混为一谈）。
2. **界面验收**：有 GUI 的机器上跑一次「订阅 → 安装」（用社区那份索引），并贴回确认框原文。
3. **`compose`**：已裁定降到 P1（要的是"未保存的编辑器内容"，不是先落库再删）。
4. **托管交接**：发版后把 `plugin-index.fragment.json` 交给索引托管方，**先传包后传索引**（见 [plugin-hosting.md](plugin-hosting.md)）。
