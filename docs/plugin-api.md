...
| `export:files` | 把内容保存成文件：存到哪、存不存都由你在系统保存对话框里定（插件只能给一个建议文件名，给不了路径）。**导出的文件是明文**，不随空间的加密一起保护 | medium |
...
| 工具 | 返回 | 自 |
|---|---|---|
| `coverage.report` | object | 1.92.0 |

### `coverage.report` — 索引覆盖报告

- **调用方**：只有 AI 宿主（无 `api.*` 入口）
- 权限：`read:files`
- scope：`current-space`
- 返回：**一行摘要**（可直接念给用户）＋ 结构化明细：`pages`/`attachments`/`derived`/`chunks` 四组计数与 `gaps` 明细。`attachments.partial` ＝ **搜得到但没抽全**的数量（§15.10）；`gaps[].reason` 是分类：`no_extractor`（没人认领这种格式）/ `no_content`（认领了但抽出来是空）/ `not_chunked`（抽到了没切块）/ `page_empty`（页面正文空）/ `partial`（已索引但抽取器报了缺口）。**缺口只列前若干条**：`gapsTotal` 是总数、`gapsTruncated` 说明是否被截断
- 说明：检查**整个库**的索引覆盖：哪些内容真的进了检索面、哪些没进、哪些**进了但没抽全**。参数: 无。返回 {summary, report}：`summary` 是一行中文摘要（可直接展示）；`report.attachments` 给出 已索引/没抽全/未索引 的计数与分类，`report.gaps` 给出**明细**（每条含 `reason` 与一句「该怎么办」）。⚠️ 三个必须分清的口径：① **没抽到 ≠ 文件里没有**（`no_content` 可能是空文件/加密/纯图）；② **页面正文空 ≠ 这页内容没被索引**（图片/附件/数据库块由附件侧负责）；③ **`partial` 是「搜得到，但只覆盖了一部分」**（典型：混合 PDF 只抽到正文页）—— 别把「已索引 N/N」读成「内容全在检索面里」。**缺口列表可能被截断**（看 `gapsTotal` 与 `gapsTruncated`）。 ★ **只有 AI 宿主**能实现它（`host: frontend`）：判「没人认领这种格式」必须以 TS 侧的抽取器注册表为准，Rust 侧再长一份就是两份实现、而漂移不会报错 ⇒ 它不进插件 shim／插件类型包／Rust 绑定表，插件调不到。
...
### 4.5.1 更新插件时：新增权限/事件要用户重新确认

**这一条会直接影响你怎么发新版**：用户启用插件的那一刻，宿主会把他当时看到的那份声明（权限 + 事件）记成一条**授权快照**。之后你把插件目录换成声明更大的版本时——

- 宿主**暂停**这个插件（命令不执行、事件也不派发），并在插件管理里说清**新增了哪几项**；
- 用户点了「重新确认」之后才恢复运行。

所以：

- **别把「新增一项权限」当成小改动**——它会让插件停下等用户确认，用户没注意到就会以为插件坏了（插件管理里有明确的提示条与按钮，这是设计，不是故障）；
- **收敛声明（删掉权限/事件）不需要确认**，也不会暂停；只有新增会；
- 重新安装（卸载再装）等于重新开始：新装的插件默认未启用，用户启用时才记快照。
...
| `page.context` | 页面右键菜单：页面列表里那一行的「⋯」菜单（在行上右键同样是它）：你的命令会拿到**被点的那一页**作为当前页 | ✅ 已实现 |
| `file.context` | 附件右键菜单：文件（附件）列表里右键那一项：你的命令会拿到**被点的那个文件**的信息作为入参——`{ fileName, size, mime }`（**没有绝对路径**：插件本来也读不了文件，给路径只会让人以为能读），并且"当前页"是**这个文件所在的那一页** | ✅ 已实现 |
| `editor.toolbar` | 编辑器工具栏：编辑器顶部工具栏的「⋯」菜单里（与导出 Markdown/HTML/PDF 同一处）：适合「边写边跑」的命令，省掉打 `/` 或开命令面板那一步；当前页就是正在编辑的这一页。插件给不出图标，所以不会变成工具栏上那排图标按钮 | ✅ 已实现 |
...
- **零代码插件也能用设置**：视图的查询字段可以写成 `{ "fromSetting": "key" }`，由宿主在渲染时解析（见 §4.9.1）——那种情况下「读设置的是宿主」，你依然不需要代码。
...
**零代码也能「用户可配」**：查询字段除了写死，还可以指向一个设置——用户改了设置，视图下次打开就按新条件查。
...
- **设置表单还是宿主渲染的**：你在 `settings` 里声明什么，用户在「插件管理 → 设置」里就看到什么；你依然读不到它（没有代码），是**宿主**在渲染视图时把值解析进去；
- **默认值只有一处**：就是设置声明里的 `default`——不要在两边各写一个，那必然漂。用户没设过、你也没给 `default` 时，该字段按「没给」处理（即宿主的默认行为）；
- **类型要对得上**：数字字段（`updatedWithinDays` / `limit`）← `number` 设置（或候选项都是数字的 `select`）；`sort` / `kind` ← 候选项都在白名单里的 `select`（用 `string` 会提醒你：用户可能填出白名单外的值，筛选会静默回到默认）；`titleContains` ← `string`）；
- **值不可用时按「没给」处理**，绝不报错：视图永远打得开。但这几种写法会被校验器明确指出来（尤其是 `select` 的候选项不在白名单里——界面看着完全正常，筛选却永远按默认来，最阴的一种）；
...
## 4.11 导入触发（manifest 声明 + 宿主读文件）

想让用户「选一个文件 → 你的命令拿到它的内容」，在 manifest 里声明 `triggers`：

```json
"triggers": [
  { "kind": "import", "extensions": [".md", ".csv"], "command": "my-plugin.importMd" }
]
```

```js
register({
  id: "my-plugin.importMd",
  title: "把选中的文件读一遍",
  run: function (args) {
    // args.fileName —— 用户选中的**文件名**（不含目录）
    // args.content  —— 文件内容（宿主读的，插件没有任何文件权限）
    return args.fileName + "：" + args.content.length + " 字符";
  }
});
```
...
- **读文件的是宿主，不是插件**：插件仍然没有文件读写能力，内容只是**这一次调用的入参**；
  插件要产出笔记依旧只能走 `api.*`（写能力照样出草稿、照样要用户确认）——触发方式不改变这条链路；
- 入口出现在命令面板里（「导入：用「插件名」打开 .md」）：点击 → 选文件 → 读内容 → 执行你的命令；
  **用户取消选择就不执行**（不会拿空内容去跑一次）；
- `extensions` 怎么写都会被规范化成 `.md` 这种**小写带点**形式（`"MD"` / `".MD"` 都等于 `.md`）；
  空数组或不合法的项（含空格 / 斜杠 / 目录分隔符）会被忽略，校验器会明确提醒；
- 触发只能调用**你自己注册的命令**：命令 id 写错或忘了 `register` 会提醒——
  否则用户点下去只会看到「命令不存在」；
- 命令如果还声明了 `params`，**导入触发不会渲染参数表单**（入参就是 `{ fileName, content }`）；
...
- 需要权限 `export:files`（能力注册表里的写能力，风险等级 medium）。用户在安装/启用时看到的是「把内容保存成文件」，理由里会写清**导出的文件是明文**——它落在笔记之外，不随空间的加密一起保护。这一点请你在 `permissions[].reason` 里如实说，别让用户以为导出件也受加密保护。
