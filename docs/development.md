...
- **两条全量 `vitest` 并发 ⇒ live 判据"假红"**（2026-09-23，AMD 侧实测；macOS 侧无模型服务，**我未能复现**，只记其结论与边界）：
  现场是"自己的全量 `vitest` 与**同步环触发的**全量 `vitest` 同时在跑"，两条 live 判据各红一条
  （`image.localVlm` 的 caption 报 `provider_error: …timeout`、`librarySummary.e2e.live` 报错），
  而**两条单跑都绿**（各 3 passed）⇒ 本机模型服务扛不住两条全量并发。
  **处置**：读数之前**先停环**（与"取门禁读数前先停监听器"同一条纪律）——**别改断言**；
  "live 假红"的成因至少有两个（5 s 默认超时 / 资源并发），先看是哪一种再动手。
  ⚠️ 边界：这条的**读数在 AMD 那台**（有本机模型服务的那台）；这台 macOS 上那条族是 skipped，
  所以我记的是"**成因与处置**"，不是"我量过"。
- **Rust 判据里别用 `std::env::set_var` 传递夹具状态 —— cargo 测试是并行多线程的**（2026-09-23，我自己踩的）：
  我为 `convert_legacy_office` 写假 `soffice` 脚本时，用环境变量把"记参数的文件路径"传给脚本 ⇒
  **同一组判据时红时绿、每次红的条数还不一样**（三次跑：红 3 条 / 红 1 条 / 全绿），
  因为两个用例在互相覆盖同一个进程级环境变量，子进程读到的可能是**别人的**路径。
  **处置**：夹具的路径**写死进假脚本正文**（每个用例生成自己的脚本），别走进程环境；
  推广一句：**判据之间不许有共享可变状态** —— 它造出来的假红比真 bug 更难查（红的位置与原因都对不上）。
  分辨特征：**同一组判据的失败集合随执行顺序/并发变**，而单跑某一**条**永远是绿的。
...
- **`src/lib/extract/schema.ts` 注释里的反引号 —— 真实规则比"一个都不要写"窄得多**（2026-09-23；AMD 侧踩到、我第一版记档记**错**了，Windows 侧复核后我按实测改写）：
  `db::tests::derived_schema_matches_the_ts_source_of_truth` 的取法是**按反引号切分**（`ts.split` 用反引号当分隔符）取**奇数下标**段当"模板字符串体"，
  再**按前缀过滤**出 `CREATE TABLE` / `CREATE INDEX` 开头的段、要求恰好 6 条并与 Rust 侧逐字相同
  （`src-tauri/src/db.rs:1201-1210`，**两头都在**：过滤是 `starts_with`，不是"奇数段就一定是 DDL"）。
  ⇒ 实测四种改动的结果（我在本机按判据的口径复刻过）：

  | 在注释里写什么 | 结果 |
  |---|---|
  | 一对普通反引号（如 `` `exec()` ``、`` `db.rs` ``） | ✅ **绿**（成对 ⇒ 奇偶不变；段不以 `CREATE` 开头 ⇒ 不进过滤） |
  | 两对、三对普通反引号 | ✅ **绿**（同上 —— 当前文件注释里本来就有 3 对） |
  | 一段**以 `CREATE TABLE`/`CREATE INDEX` 开头**的反引号段 | ❌ 红（多出一条 DDL 段 ⇒ 「解析出 7 条」） |
  | 一**个落单**的反引号（左右不配对） | ❌ 红，且**报错很误导**：奇偶被翻转 ⇒ 6 条 DDL 全被跳过 ⇒ 「解析出 0 条」 |

  **处置**：注释里**成对的**反引号随便写；只要不写出「以 `CREATE TABLE` / `CREATE INDEX` 开头的反引号段」、
  也别写落单的反引号（后者是笔误，正是它把 6 条变成 0 条）。
  ⚠️ 我第一版写成"注释里一个反引号都不要写"是**过宽**的 —— 那种口径会让人为了过门禁去删掉本来有用的注释；
  真实触发条件上面这张表就是全部。若哪天把解析改成"**取全部段再按前缀过滤**"，规则会更简单（与奇偶、位置彻底无关）——
  那是 `db.rs` 里判据自己的改动，走它自己的线。
...
先说清一件容易混的事：`docs/plans/` 里那七十多份**不都是待办清单**。
能被"照着做完"的是带**施工单**三个字的那几份
（[PDFium Linux 字体修复施工单](plans/2026-09-20-pdfium-linux-font-backend-workorder.md)、
[CRDT spike 施工单](plans/2026-09-18-crdt-spike-workorder.md)），其余多数是**分析与判断**
...
[AI 覆盖 · 交接件](plans/2026-09-17-ai-coverage-handoff-patches.md) 的开头原话就是
「Windows 跑不了 `cargo test`，于是 `pages.get` 分页 / `files.read`+`files.search` / `db.rs` 派生表
这三项**不该由它做**」。于是要**另补一份交接件**，而且此前"每次都要重新解释一遍为什么绕开"——
**缺的是切法，不是决心。**

⇒ **切法**：每个任务在**写下的时候**就要能回答"做完之后，在**这台**（或明确指派的另一台）机器上
跑**哪一条命令**、看到**什么读数**"。跑不出来的不算一个任务，只能算一次讨论——
而讨论应当在写施工单**之前**结束。

（外部实践把这条写作"2–5 分钟一个任务"。本仓只取它**可自验**的那一面，
**不引用那个时长数字**——时长是别人的经验值，这里能钉住的只有"有没有一条命令"。）

#### ② 复核分两端：**先对规格，再对质量**（顺序不能反）

两端问的是两个不同的问题：① **做的是不是计划里说的事**？② 代码本身好不好读、有没有更简单的写法？
合成一遍就出事：一份写得很漂亮的实现，很容易让人**跳过第 ① 问**。

本仓恰好有一整族事故长这个样子——`AGENTS.md` §8 那批门禁的共同点原文写着
「违规**不炸、不报错、测试全绿**，只是用户那边出问题」：整店订阅、提前 `return` 越过 hooks、
没登记的浮层（**安卓返回键直接退出应用**）……**它们单看代码都挑不出毛病，出事出在"干的不是那件事"**。

⇒ 第一端**只拿读数**（计划里那几条判据是否**逐条**成立），说得再好听也不算；
第二端才谈实现质量。**反过来先做第二端，就会被"写得好"说服。**

#### ③ 改了计划里的一处判断，要**顺带做一次一致性复核**（本仓最贵的一类错）

**真实事故（2026-09-26 实测）**：那句「`/traffic/` 的 nginx 行仍在主 vhost 里……
**那个缺口还开着**」写于 **2026-09-24 13:34**，而服务器侧 **13:47 就落地了**——
**它在被写下 13 分钟后就过期了，然后两天没人再看它。**
同族的还有：`docs/plans/` 到 **71 篇**时 **4 篇**没进索引（§8.1）、
别仓一份 `AGENTS.md` 改了正文没改标题（标题与正文互相打脸）。
⇒ **两条动作**：

1. **改一处判断时，先问"同样的话还有谁在读"**：`git grep -n '<原话>'` 把副本找全，一个个改；
   确实改不动的那份，**就地写明"以谁为准"**。
2. **写下一句会过期的话时，就地写上它的判据**（那条命令），别只写结论。
   与其写"镜像没落后"，不如写"跑 `mirror-github.ps1 -DryRun`，看到 in-sync 才算"。
   **带日期的快照只代表那一刻**——要判据就写命令。

> 这三条**都不是"再多写点文档"**，而是**减少只能靠人记得的东西**。
> §8.1 那三条机器判据守的是"文档能不能被找到"；这三条守的是"计划能不能被执行、执行完能不能被认出来"。
> 后者**现在还没有机器能判**（"做得对不对"暂时写不出判据），所以更要写在这里——
> 而一旦某一条将来能变成门禁（像 §8.1 那样），**就应当把它从本文档挪进 `scripts/lib/gates.mjs`**。
