...
- **在 `src/lib/extract/schema.ts` 的注释里写「反引号」⇒ Rust 那条两侧一致性判据当场变红**（2026-09-23，AMD 侧踩到、我记档）：
  `db::tests::derived_schema_matches_the_ts_source_of_truth` 的取法是**把那个 TS 文件按反引号切段**、取奇数段当"模板字符串体"，
  再要求恰好 6 条 DDL 与 Rust 侧逐字相同；于是**注释里**多一对反引号就会打乱奇偶
  ⇒ 报「解析出 7 条」，第一条是 `CREATE TABLE IF NOT EXISTS` 这种碎片（判据红得对，它守的就是"两侧逐字相同"）。
  **处置**：该文件里**只有 DDL 用反引号，注释里一个都不写**（文件顶部已用醒目注释写明）；
  不要为了这条去放宽判据（与下面"判据可以粗，但不许按目录豁免"同源：改写法，不改判据）。
