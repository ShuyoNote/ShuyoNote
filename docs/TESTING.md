...
<!-- facts:begin -->
门禁 44 条（contract 20 / smoke 3 / sync 1 / plugin 3 / browser 3 / mobile 3 / rust 8 / artifact 3）· 能力 25 条 · 命令 Rust 244 / web 245 / CommandMap 246
<!-- facts:end -->
...
- ★ **计时类判据在全量并行下会"假红"**（2026-09-23 一天内**三次**实测，同一条判据）：
  `src/lib/graphLayout.test.ts` 里"250 节点收敛 ≤ **250ms**"那条是**墙钟预算**，于是在**全量
  `pnpm vitest run`**（两百多个文件并行、CPU 打满）时会超：实测全量里 **260 / 276 / 310ms** ⇒ 红，
  而**同一台机、同一棵树单独跑** `pnpm vitest run src/lib/graphLayout.test.ts` **3/3 全绿**
  （该文件全部用例 140–232ms）。
  ...
  （2026-09-23 三次里两次都是这么过的），并把这次假红如实记在提交里，而不是当成代码问题去改产品代码。
