...
<!-- facts:begin -->
门禁 44 条（contract 20 / smoke 3 / sync 1 / plugin 3 / browser 3 / mobile 3 / rust 8 / artifact 3）· 能力 25 条 · 命令 Rust 247 / web 245 / CommandMap 249
<!-- facts:end -->
...
- ★ **计时类判据在全量并行下会"假红"**（2026-09-23 一天内**五次**实测，同一条判据）：
  `src/lib/graphLayout.test.ts` 里"250 节点收敛 ≤ **250ms**"那条是**墙钟预算**，于是在**全量
  `pnpm vitest run`**（两百多个文件并行、CPU 打满）时会超：实测全量里
  **260 / 276 / 310 / 315 / 347ms** ⇒ 红，而**同一台机、同一棵树单独跑**
  `pnpm vitest run src/lib/graphLayout.test.ts` **3/3 全绿**（该文件全部用例 140–232ms）。
  ⚠️ **五次都发生在同一天的同一条判据上，且每次"再跑一次全量"就过** ⇒ 这已经不是偶发，而是
  **这条判据在满负载下不可靠**。判读方法见下，但**它本身该单独治一片**（见下）。
  ⇒ **判读方法**：看到它红，先**单独重跑该文件**；单独也红才是回归。**不要**因此去调大那个 250
  （那是"把门槛改松让它变绿"——本仓明确反对；真要治就单独一片：把预算换成**相对**口径，
  或让这条判据跑在**独占**的 worker 里）。
  ⚠️ 同时记住：**推送前要求"当轮 tip 全绿"** ⇒ 遇到这种假红，正确动作是**再跑一次全量**取到全绿 tip
  （2026-09-23 五次都是这么过的），并把这次假红如实记在提交里，而不是当成代码问题去改产品代码。
  **建议的治本方向（单独一片，且要带变异实测）**：把"单次墙钟 ≤250ms"换成**同一棵树内的相对口径**
  ——例如"取 3 次里最快的一次"或"与同一进程内另一次基准计算比"，并要求**故意把布局拖慢 3×**
  时它仍然红（否则就是把判据改瞎了）。
