...
| 门禁 | `pnpm verify`（本机 **25 条**全绿）/ `node scripts/test-report.mjs --group rust`（**7 条**，含 `rust-test`（默认即国密）、`rust-no-sm-crypto`（**回滚通道**）、**`rust-sm-wired`（库级国密接线：打补丁 ＋ `--features sm-library` 全量单测，2026-09-22 新增）**、`check-crypto-backend`、`gm-conformance`） | 全绿（`rust-sm-wired` 在无 SM 版 OpenSSL 前缀的机器上自报跳过；本机实测：`OPENSSL_DIR=<Tongsuo> node scripts/check-gm-wired.mjs` ⇒ **440 passed / 0 failed**） |
