...
|---|---|---|---|---|---|
| **macOS** | ✅ | ✅ **`sm-library` 构建：P2 已接线 ＋ P3（SM4 页）已落地** ⇒ 页加密 SM4、页 MAC/KDF SM3（本机实测：XOR 判据打印「本构建的**页加密** = SM4 页」—— ⚠️ **更正**：那条判据**只判页加密**，夹具是裸钥默认参数写的）。<br>⚠️ **【历史口径 · 已被 2026-09-22 取代】** 本格原写「**默认包仍是 CommonCrypto（AES ＋ SHA512）**」—— 那是 **2026-09-19** 的拍板（"macOS 不切默认、国密版另发"）。**2026-09-22 owner 改判「单一口味＝国密」**：所有平台**发出去的包**一律 SM4 页。<br>★ **但"发出去的"要按发版矩阵读**（`release.yml` 的 `matrix.include` 就是权威）：今天**只有** `ubuntu-24.04 (deb,appimage)` ＋ `windows-latest (nsis)` 两项，紧跟一行注明「**macOS 暂缓**：待 Apple 签名/公证 secrets 到位后再加回（dmg,app）」，另有单独的 `android` job ⇒ **macOS 今天一个包都不发**，"macOS 默认包 vs 国密版"这个选择**根本不在发生**。<br>★★ **证据形态必须分开写**（AMD 2026-09-25 复核了 workflow，本节照此收口）：**Linux／Windows ＝ 流水线断言** ✅（`check-crypto-backend`：页加密＝SM4，`v1.91.24/25/26` 三次 tag run 全绿）· **Android ＝ 产物三条断言** ✅（无 `libcrypto.so`／无 `DT_NEEDED`／补丁字面量在包里）· **macOS ＝ 只有本机手工实测**，流水线一处断言都没有（`macos.yml` 里 `sm-library` / `check-crypto-backend` 命中 0）⇒ **交付材料里 macOS 只能写"本机实测"，不许写"已发"**；**加回 macOS 发版 job 的那一天，必须同时带上 `check-crypto-backend`**（否则就是"口径领先于机器"） | Tongsuo **已在本机构建**（commit `540603a3`）；`sm-library` 打开时 `build.rs` **fail-fast**（不给 `OPENSSL_DIR` 就当场失败） | 本侧 | 应用层 ✅；**三段自证在本机全绿**（见 §四）；库级 ✅（`security::` 22/0、`gm_provider::` 9/0，接线构建实测）；⚠️ 有一条 macOS-only 风险见 §五 |
...
| ★★★ **读数怎么读（2026-09-25 收口，AMD 复核后写死）** | 一律写成 **`<命令> @ <平台> ⇒ passed / failed / ignored`**，并指定**门禁口径＝`check-gm-wired`**（`node scripts/check-gm-wired.mjs`：下限 **`passed ≥ 380 且 failed == 0`**、**win32 上显式 `--skip plugins::`**，是唯一"空跑即红"的那条） | ⚠️ 本表里那些"同一件事好几个数"（`427/0` · `427/1` · `security::` **21** · **22**）**不是互相矛盾**，而是**三种口径 ＋ 不同时点**：<br>① **门禁口径**（`check-gm-wired`）只覆盖国密各组、带下限与平台 skip 规则；<br>② **全量口径**（裸 `cargo test`）把 `plugins::` 那一族也算进来 ⇒ 与①**天生不同数**（win32 上那族 34 条必然红，与国密无关）；<br>③ **模块子集**（`security::` / `gm_provider::`）**只用于定位**，不是"国密读数" —— 它在一天里增删一条判据就会 ±1（22 → 21 就是这么来的）。<br>⇒ **要引用一个数，就带命令、带平台、带时点**；单说"国密是 427/0"没有信息量 |
...
| ★★ **本机静态前缀那一格：已闭合**（2026-09-23，owner 拍「做」后 AMD 当天做完） | **AMD ✅** | 走的是**第二条**（自编静态 Tongsuo）—— 这台机**没有 vcpkg**，而 vcpkg 只能从 `github.com` 取（本机 DNS 不通）⇒ 自编 `no-shared`，与 macOS 侧同一条思路。**读数**：`libcrypto.lib` **49,708,370 字节**（静态）＋ `bin` 下 crypto DLL **0 个**；用它跑 `node scripts/check-gm-wired.mjs` ⇒ **403 passed / 0 failed**（下限 380）＋ **③ link-search 实际链的目录 `…\.third\tongsuo-static-out\lib` = 钉的前缀** ⇒ `GATE_EXIT=0`，跑完还原补丁并重建默认特性（不留混态）。⚠️ **踩到的坑（值得留给后来人）**：只加 `no-shared` **不够** —— 链接报 `LNK2001: unresolved external symbol OPENSSL_UplinkTable`（`libcrypto-lib-bss_file.obj`）。原因：Configure 把 uplink 挂在 `shared` 上，但 **VC 目标定义了 `uplink_arch` ⇒ `no-shared` 下 uplink 仍默认开着**，而 uplink 表是 DLL 时代要**使用方**提供的东西（`openssl/applink.c`）。修法：Configure 再加 **`no-uplink`**（Configure 自带该选项）。vcpkg 的静态 openssl 正是这么做的。⚠️ 与 CI 的差别如实记：CI 用 vcpkg 的 **OpenSSL**，这里是自编 **Tongsuo**（同族；SM3/SM4/PBKDF2-SM3 都在）——**判据要的属性一致**（无 DLL ＋ 静态 `libcrypto.lib` ＋ 链到钉的前缀） |
