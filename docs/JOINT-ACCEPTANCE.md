...
- **缺口**：j2/j3/j4 三格的施工单已写，判据还没落地。前提侧的好消息：CRDT 冲刺的 **S4b-1b**（「收」那一侧）、
  **S7-1/S7-2**（桌面侧 `page_crdt`）与**桌面侧消费 `crdt_state`**（第 43 轮）都已落地 ⇒ j3/j4 **现在可以施工**了；
  剩下的外部件是**真机双设备验收**（S7-3）。
- ⚠️ **一条必须记档的"该做没做"**（2026-09-23 第 45 轮核对）：本文 §5 与上面那句提到的
  **磁盘边界平面开关（`VITE_CRDT_PLANE=1`）今天仍然存在**（`src/lib/crdt/plane.ts` ＋ `docContent.ts`
  的两读一写三处仍调 `throughCrdtPlane`）—— 而[边界决策](plans/2026-09-23-crdt-plane-boundary-decision.md)
  §6.2 已经拍了"**那三处接线应当撤出**"。它现在**既不推进合并**（口径 1 作废后它只是"把已落盘 JSON
  归一化一次"），也不再是任何判据的前提 ⇒ 建议**单独一片撤掉**（撤的时候那 9 条 `plane.path.test.ts`
  要按新边界重写，**不许删判据不写替代**）。在那之前，本文里凡是用它的步骤（j6 剧本第 2 步）都照旧可用。
