...
## 2b. 三端合练（mac / windows / amd）：`scripts/sync-3way.mjs`

上面 §2 的 `sync-multidevice.mjs` 验的是**两台**机器的会合。第三台（AMD 工作站）加入之后，
"前面两台还能不能互相看见对方的新页"没人验过 —— 这条脚本就是补这个洞：**三个角色各自推一页，
再从每个角色的视角全量拉取，必须都看得见三份**。

```bash
# 服务端就是 §1 起的那个（同一个 ~/mdtest/sync.db）
cd ~/zhai/ShuyoNote
node scripts/sync-3way.mjs --server http://127.0.0.1:8787 --token 'sk_……'
```

期望结尾（12 条判据）：

```
  · 本轮 runId=rmu54ck5d-nene（每轮唯一，避免被服务端的 (device_id, device_seq) 去重挡掉）
  ✓ mac 推了一页 / windows 推了一页 / amd 推了一页        accepted=1
  ✓ mac 视角 / windows 视角 / amd 视角 全量拉取看得见三页   看见 mac/windows/amd
  ✓ 后加入的设备一次全量拉取即补齐三页
  ✓ 重推同一 device_seq 被去重                          accepted=0
  ✓ 同页并发写：服务端保留两条变更（不做合并）              共 2 条
[三端合练] 全部通过 ✅
```

**⚠️ 它是可复现的，而"可复现"这条本身踩过一次坑**（2026-09-17）：第一版把 `device_id` 写死成
`dev-mac`/`dev-windows`/`dev-amd`、`device_seq` 也从 1 开始 ⇒ **第一次跑 12/12 全绿，第二次跑 8 条红**。
服务端按 `(device_id, device_seq)` 去重**是正确的**，错的是脚本：它把"被正确去重"读成了"推失败"。
⇒ 现在每轮带一个唯一 `runId` 后缀，并且**连跑三次都 12/12**才敢说这条合练是可复现判据。

**这条脚本能证明什么、不能证明什么**（写在脚本头部，这里重复一遍免得误读）：

- 能证明：三个设备各自的变更都进了空间、任一设备全量拉取都看得见三份、后加入的设备能补齐历史、
  同一 `(device_id, device_seq)` 重推是幂等的、同页并发写时**服务端存了两条**（不合并）。
- **不能**证明：客户端如何解决同页冲突 —— 那是客户端测试面（见 §6）。
