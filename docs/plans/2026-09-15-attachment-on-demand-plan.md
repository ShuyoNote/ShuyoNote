...
### 7. P6.3 续：把"可操作"补到**所有应用内打开路径**（2026-09-15）

上一轮只在文件管理器给了入口（「未下载」标记 + ☁ 按钮）。但用户不一定从文件管理器打开文件——
他可能从**页面树 / 附件面板 / 内联附件引用 / PDF 引用 / 插件命令**点开，那些路原来仍会
落到一句死胡同文案上。本轮把它们一起修掉。

**关键判断：修在"必经之路"上，不修在 10 个调用点上。**

| 位置 | 内容 |
|---|---|
| **新增** `src/lib/attachmentBytes.ts` | `ensureAttachmentBytes(hash)` —— **判据与取字节的唯一实现**：先 `list_attachment_hashes`（走**附件目录**，不是数据库）判断在不在本机；不在就 `download_attachment` 取回来；三种失败都给人话提示。放在这一层的理由写在文件头：打开 PDF 有 **10 个调用点**，各写一遍＝同一件事抄十遍 |
| `store/pdfReader.ts` | `openPdf()` 是**所有 PDF 入口的必经之路**（文件管理器 / 页面树 / 附件面板 / 内联附件引用 / PDF 引用 / 插件命令…）。改成 **读 → 失败则按需取回 → 再读一次**；取不回来**抛最初那个错**（离线 / 服务端也没这份字节，那才是真原因） |
| `store/filePreview.ts` | markdown 预览的读取同样加"读→取回→再读" |
| `components/FilePreviewDialog.tsx` | 图片 / 视频 / 音频 / PDF 四个分支原先各有一句一模一样的"文件内容缺失（可能未同步到本机，或已被删除）"——**文案没错，但没有任何动作**。现在：① 读取失败先**自动按需取回**再读一次；② 真失败才落到统一的 `missingBlock`，**并给一个「从服务器下载」按钮**（点它重取并重跑读取） |

**顺手发现并修掉的一处死状态**：`FilePreviewDialog` 的 `asset.missing` **写了 3 处、一处都没读**
（`grep missing` 只有赋值没有使用）⇒ 那个"缺失"状态在界面上从来没有被区分过。
现在它真的被用来渲染上面那个可操作状态。

**仍未覆盖**：**页面内嵌的图片 / 视频**（`ImageNode` / `VideoNode` 的 `src` 解析）不从这条
必经之路走 —— 字节不在本机时那里还是一张坏图。修它要动编辑器节点的 src 解析，
是另一类改动，**本轮不做**（记在这里，别当成已经修了）。

**验证**：`tsc` 0；`pnpm test` 656 全过；`smoke-web` 350 通过；`check-panel-layout` 25 通过
（真实 Chromium，量了几何）；`check:web-commands` 通过。⚠️ 未做真机/真机模拟验收。
