...
2. ★★ **轮换"正开着"的那个空间跑不通** —— 而这恰恰是最常见的用法（用户就待在这个空间里点它）。
   现有判据拿的是**别的空间**的连接（`open_space_conn_at("rot-b")`）⇒ 这条真实路径**没人守**。
   我试着照 `enable_space` 那套（swap 成内存库 → 转换 → 开回来）修，当场红了：
   `file is not a database` ＋ `ERROR CORE sqlite3Codec: error decrypting page 1 data: 1`。
   **根因定位清楚了**：函数里是"**先转换、最后换盒子**"，于是"重新打开"那一步拿着**旧盒子**的钥匙
   ...
   a. 取旧钥匙 ＋ 先备份；
   b. 若连接正开着它 ⇒ swap 成内存库；
   c. 两次转换（失败 ⇒ 尽量回退到旧钥匙那一版）；
   d. **失败**：先把连接按旧状态开回来（此时盒子里还是旧钥匙 ⇒ 对得上），再返回那句带备份路径的错误；
      **成功**：先把新盒子**只放进内存**（需要一个"只更新内存、不写 meta"的生产 setter —— 此刻 `conn`
      是内存库，写 meta 会 `no such table`），再重新打开（这一步才拿得到新钥匙），
      最后用真正的连接写 meta（`store_keyring` 幂等）＋ 打标记；
   e. 判据：对着**正开着**的空间轮换 ⇒ 成；数据没丢；连接被开回来且查得动；
      失败路径 ⇒ 库与盒子都还在旧钥匙那一版、连接照样能用。
...
> 这一轮我**没有**把上面这两件硬塞进当轮提交：第二件的正确修法要重排轮换的顺序并回滚盒子，
> 在剩余余量里做不完 ⇒ **回退**、把根因与修法写在这里，树保持全绿。
