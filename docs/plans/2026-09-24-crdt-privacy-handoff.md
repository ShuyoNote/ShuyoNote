...
| **C** | **1**（存量迁移：**报错** ＋ 说清怎么办） | ✅ **第一半已做**：`space_crypto::migrate_legacy_space_into_keyring` —— 把**旧的应用级钥匙原样装进袋子**（空间钥匙 == 旧钥匙 ⇒ **库文件一个字节都不动**，所以"先备份再重写"这一步在**这一半**里不必要）；四条出口都**响亮**（明文空间 ⇒ Err／没解锁或没袋子 ⇒ Err／成功 ⇒ `Ok(true)`／已有盒子 ⇒ `Ok(false)` 幂等）。⏳ **第二半待做**：换成**真随机的空间钥匙**（`rotate` ＋ 一次重加密 —— **那一步才要"先备份"**，且要"可重跑、幂等、失败有痕"） |
