...
| **A=3** | owner 拍板"分类由入口决定" ⇒ 本地新建空间标 `personal`（`workspaces::insert_new_local_space`）＋ **D 的 KDF 实测读数**（8 秒大头在 SM3 那条腿） | `4fa9d6d4` ✅ **已在 `origin/dev`**（原来这里写着"只在本地、推送被 TLS 挡住" —— 那件事当天就恢复了，见 §6；本条 2026-09-25 复核过） |
...
✅ **2026-09-25 更正（本节原先的两条理由都已过期，结论本身仍成立）**：

- 原写「`encryption_enabled` 与启动闸门也还没按空间」—— **已经按空间了**（`63fc5506`，见 §1 表里的 1b-2b）。
- 原写「因为今天**全是未分类**」—— 不准确：**本地新建/导入的空间今天都会标成 `personal`**（A=3，§7 A 行）。

⇒ **结论不变、要记的缺口换成两条更准的**：① **团队空间没有自动分类来源**（本仓只有手动
`set_space_kind`，团队流程在另一个仓）⇒ 它落在"未分类"上，而闸门对未分类**放行**；
② **存量空间一律 `''`**（owner 拍了不批量迁移）。⇒ **"个人空间密文 / 团队空间明文同时成立"
这句话，今天仍然不能写成"已经做到"**（机制齐了、**团队的输入**没齐）。
...
### ✅ 2026-09-25：MIX 2 上"**开启加密后「会话锁定」整节不出现、要重启才出现**"已修（AMD 落，Windows 定性）

- **现象**（真机 MIX 2）：给活动空间开启加密成功后，「设置 → 安全」那行「已加密 · 已解锁」＋
  **「立即锁定」按钮直到重启才出现** ⇒ 本轮那次锁定是**靠重启**拿到的，`[unlock] 742 ms` 也是那么量到的。
- **根因**（不是条件写窄）：`SpacePrivacySection.run()` 成功后只 `reload()` 了**本节自己的视图**，
  没有刷新状态中枢；而整节由 `SettingsDialog` 的 `useVault().enabled`（**内核读数**）控制
  ⇒ 中枢停在旧值 false。`runKeyring()` 的 ok 分支同病（"取回公开材料"也改变内核读数）。
- **修法**：两处都在 `reload()` 之后补 `await refreshVault()`（`vault.ts:50` 原本就有这个出口）。
- **判据**（★ 这条比修法重要）：`SpacePrivacySection.test.ts` 新增两条，形态选**行为**而非实现 ——
  「开启加密成功后 `vaultState().enabled` 必须变成 true」（＝那一节会不会出现）；关掉那条同理反向。
  修前实测**红**（`encryptionStatus` 从没被调用过），修后 **16/16 绿**（`tsc` exit 0）。
- ⚠️ 这一族的通病值得记：**"改了数据、忘了刷新一个状态中枢"** 既有判据结构上抓不到
  （它们直接打桩 api、不经过 UI 的 store 刷新路径）⇒ 抓它的唯一办法就是**从 UI 走一遍并断言中枢**。
...
2. ~~**服务端补丁还没落进那个仓**~~ ✅ **2026-09-25 更正：已经落进去了**（`d391be9`，是那个仓 `main` 的
   HEAD 且在 `origin/main` 上；`git branch -r --contains` 复核过）。原来那句说的是更早的状态 ——
   与 §7.0 那条"已 `git am` 落进他们仓"是同一件事，**以 §7.0 为准**。
