...
> 真服务端**16ms** 收到推送）・6/7 ✅**部分**（"连不上有痕"有判据；"连上时关掉立刻断"与
> "轮询不受影响"的**真行为**要跑起来才算）・Rust `sync_stream` **12 条全绿**。
> ⇒ **实现完成、判据 1–5 有读数**；剩下的两条半（判据 6/7 的真行为、真机双设备那条完整链路）
...
| 6 | **可关**：关开关 ⇒ `status.running=false`、不再重连 | 单测（状态机）＋ 文本级接线判据 | 🔶 部分：`stop` 幂等 + 开关两处共用的接线有判据；"连上时关掉 ⇒ 立刻断"的真行为仍要跑起来才算 |
| 7 | **不静默**：连不上/被代理掐 ⇒ 有痕，且**不影响**轮询 | 单测 ＋ 人工 | 🔶 部分：`unreachable_server_surfaces_an_error` 证明"连不上有痕"；轮询不受影响是**结构性的**（两条路独立）但没跑起来验 |
...
**⚠️ 仍然没验的**（如实写，别读强）：① 判据 6/7 的"真行为"那一半（要 `tauri dev` 或真机）；
