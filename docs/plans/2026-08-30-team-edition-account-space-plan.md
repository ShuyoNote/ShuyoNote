...
> 📦 **迁出又迁回**：本篇 2026-09-13 曾以「属公司运作（商业化团队版）范围」为由**迁出到私有仓**；
> **2026-09-24 复核后迁回本仓**。复核结论三条：
> ① 它讲的是**已经上线**的功能（`team_*` 命令，2026-09-01 落地）的**实现细节**；
> ② **不含价格 / 客户 / 合同**（逐项扫描确认）；
> ③ 它原本就自我定位为「**落地仓库：ShuyoNote（本仓库，AGPL-3.0）**」（见下一段）。
> ⚠️ 它的**姊妹篇《团队版总方案》仍留在私有仓** —— 那一份含对外策略论证与**未做部分**的 roadmap。
...
> 关联：**团队版总方案**（`2026-08-30-team-edition-plan.md` —— ⚠️ **暂未随本篇公开**：
> 那一份含对外策略论证与未做部分的 roadmap，等团队版正式对外时再一起放）。
...
> 🖥 **仅桌面**：`team_*` 命令在 Web driver 上一律抛「Web 版不支持团队版，请用桌面版」。原因（服务端不挂 CORS / 引擎在 Rust / 浏览器存储与凭证边界）见 [Web 同步能力边界](https://gitcode.com/shuyo-cn/ShuyoNote/blob/main/docs/web-sync-boundary.md)。
...
- **零破坏**：现有单用户数据 = 「个人空间」，不登录、不迁移、不回填；组织空间是「新建」才产生。
...
- **登录态与同步联动**：token 失效正确触发登出 + 暂停组织空间同步，避免用失效 token 反复请求。
