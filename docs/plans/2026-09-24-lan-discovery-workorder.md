...
⚠️ **同时暴露一条实现缺口（Android 专属，本片没修）**：两台手机**在同一个热点上**、
...
（这一类通常要 `WifiManager.MulticastLock`；`lan.rs` 现在只 `bind` ＋ `set_broadcast`，没有那一层）。
⇒ 结论：**地址分层这一半在真机上成立**；**"两台安卓靠广播互看"这一格仍不能算验过** ——
它需要一个小的 Android 侧补丁（拿 MulticastLock）＋ 两台真机复跑一次。


> ⚠️ **本片没做的**（别读成"局域网已经做完了"）：**不做内嵌接待窗口**（甲-2，§8 ③ 先不排）；
> **"两台安卓靠广播互看"没验成**（2026-09-25 真机实测：同热点、ping 通、广播收不到 ——
> 见上面那一节；地址分层本身已在真机上验到底，但那一步是**单播**喂进去的）；
