...
> **2026-09-23 增加第三个**：`src/lib/crdt/yDocBridge.ts` —— 阶段 2 Slice A 落的「`content_json` ⇄ `ydoc` **唯一实现**」
> （见 [2026-09-23-crdt-stage2-kickoff.md](2026-09-23-crdt-stage2-kickoff.md) §3）：它按定义同时提到两种形态，别的地方一律经它转。
>
> ### ★ 新增 `crdt/*.ts` 时的**决策树**（2026-09-23 Windows 侧裁决，macOS 侧记档）
>
> 每条新文件**逐个判**，三档，**没有第四档**：
>
> | 它是什么 | 处置 |
> |---|---|
> | **那一层**（唯一同时碰"落盘形态"与 CRDT/派生语义的地方） | 加进 `LAYER_FILES` ＋ **注释写明"为什么它是那一层"与"什么情况下该删掉这一行"** |
> | **不是那一层**（薄壳/开关/工具） | **改措辞**：参数名、变量名、注释、**模块名**都不要出现 `content_json` / `content_text` / `contentJson` 这三个字面量（本门禁既有处置第 2 条：注释里写中文描述，别写存储字段名） |
> | — | **不许按目录豁免**（`src/lib/crdt/**` 这种洞会无限扩大）；**不许用抬基线消音**（抬基线 = 把"合法的那一层"与"待收口的存量"混进同一张表，`--update` 也只允许变小） |
>
> ⚠️ **一条会反复撞的税：层模块的**名字**里带那三个字面量** ⇒ **每个 import 它的生产文件永久 +1**（`import … from "./contentJsonYDoc"` 这一行本身就被计数）。
> 今天只有 `src/lib/crdt/plane.ts` 一个 importer；Slice C/D 每多一个就多一处**清不掉**的命中。
> ⇒ 干净的解法是**把层模块改名**（例如 `src/lib/crdt/yDocBridge.ts`，同时 `plane.ts` 的参数与注释改措辞 ⇒ 命中归 0），
> **不要**为了它去把门禁口径改小 —— 门禁粗得有理由（见本文件 §口径修订 2：任何"聪明"的剥离规则都会多一种**漏算（假绿）**的形状）。
>
> ✅ **2026-09-23 已按此法消掉（`52bf9010`）**：`contentJsonYDoc.ts → yDocBridge.ts` ＋ `plane.ts` 参数与注释改措辞 ⇒
> 命中 **5 → 0**，基线 **562 处未动**；而且那一层不再**静态 import** 层模块（`plane.ts` 只留 `setCrdtPlaneImpl` 注入点，
> 生产由 `src/main.tsx` 注册）⇒ 上面那条"模块初始化环"一并断掉。⇒ **Slice C/D 每加一个 importer 不再自动 +1**，
> 但决策树照旧：新文件若真碰两种形态，它就是那一层，走 `LAYER_FILES`。
>
