...
> [!] **本次明确不做**：块级 CRDT / 富块合并（P2 后置，需付费信号）；个人空间不参与实时协作（保留 E2E）。
...
| 层 | 现有能力 |
|---|---|
| 服务端 | 认证、空间/成员/角色、`/push` `/pull`（服务端 `seq` 基准 + dirty 优先本地，v1.84.3）、附件（内容寻址）、审计、组织、`devices`/`server_config` |
| 客户端 | 同步面板、自动同步（10s~5min 可配）、成员管理、每工作空间独立库 |
| 冲突 | **服务端 `seq` 基准 + dirty 优先本地**（非静默覆盖：本地未同步改动优先保留；无 dirty 时按 seq LWW，非字段/块级合并） |
| 缺失 | 同页**冲突提示（提示+选择）** / 评论 / @ / 通知 / 在线状态 / 正在编辑提示 |
...
- [x] 空间成员列表显示在线/离线；页面显示「谁正在编辑本页」；离线在超时后消失。
      — `test:sync-collab`：presence 心跳 + 在线列表 + **超时后从列表消失**（用 `window_ms` 收紧窗口验证，不必真等 30s）。
- [x] 页面/块可评论；`@` 成员生成通知；被 @ 者收到未读通知并可跳转到目标。
      — `test:sync-collab`：评论增/列、`@email` → 生成通知、未读计数；线上同款断言通过。
- [x] 通知中心未读/已读正确，清除全部可用。
...
- [x] （若做 P1.5）另一设备改动后，本设备在秒级内收到推送并自动拉取。
      ...
      **线上实测 41ms（经 nginx/HTTPS）**；本地 3-5ms。这条是「近实时」唯一能证明服务端真的主动推的证据——
...
| 项 | 结论 |
|---|---|
| 块级 CRDT | [x] 不做（P2 后置，需付费信号） |
| 近实时第一步 | [x] 同页冲突提示（软提示，不静默覆盖） |
| 在线/正在编辑 | [x] presence 心跳 |
| 评论/@/通知 | [x] P1（团队版规划范围内） |
| 传输层推送 | [x] 已做（P1.5 SSE `changes-stream`，浏览器端订阅后秒级刷新；桌面端保留轮询） |
| 个人空间 | 不参与实时协作（保留 E2E） |
