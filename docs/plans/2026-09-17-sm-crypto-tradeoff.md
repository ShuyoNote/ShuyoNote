...
| 传输层 | TLS 1.3 | 路径 2：**保持 TLS 1.3**；路径 3：**TLCP**（GB/T 38636-2020）**或** **RFC 8998**（TLS 1.3 ＋ SM2 单证书） | 路径 3 **分两形态**：TLCP 的框架是 TLS 1.1 时代的、**比 TLS 1.3 老一代**；**RFC 8998 与 TLS 1.3 同代**（国密单证书）。两形态都要换 TLS 栈 | 路径 3 = 1–2 周，且**服务端也要换**（Tongsuo 版 Nginx） |
