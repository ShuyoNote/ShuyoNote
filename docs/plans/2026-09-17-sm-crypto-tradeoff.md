...
| 应用层 AEAD | XChaCha20-Poly1305，**nonce 24B**（`crypto.rs:10,61`） | ✅ **已定（2026-09-17）：SM4-CBC ＋ HMAC-SM3（encrypt-then-MAC）**，不选 SM4-GCM | ↔️ 大致对等，且**已用 EtM 把这层的 nonce 管理压力消掉**（无"撞了就完"的风险）；SM4 密钥 **128 位** vs 现在 256 位仍在 | 中（密文格式必变 ⇒ **先做版本化**；格式头已定为 2 字节 magic＋版本） |
...
### 5.4 跨平台上的最小风险做法（四条）——**第 1、2 条已拍板（2026-09-17）**

1. ✅ **算法随空间记录**，不做全局开关：空间元数据里存算法标识（**已定：空间状态 ＋ 同步载荷都要带**，见[落地方案 §0-C](2026-09-16-sm-crypto-full-plan.md)）；
2. ✅ **老端遇到不认识的算法要「明确拒绝」**（同步带算法标识，不支持就拒收 ＋ 提示升级），而不是当成数据损坏 —— **已定为验收项**；
...
### 7.1 连带引出的设计决定：**默认编** 还是 **feature 门控**？→ ✅ **已定（2026-09-17）：feature 门控**
...
>
> ✅ **本条已拍板：走 (a) feature 门控 ＋ 一条常开的国密 CI job**（见[落地方案 §0-E](2026-09-16-sm-crypto-full-plan.md)）。
> 施工要求：主干加 `--features sm-crypto`；**那条常开 job 必须真的加**，否则国密路径就成了「没人编、坏了也没人知道」的死代码。
