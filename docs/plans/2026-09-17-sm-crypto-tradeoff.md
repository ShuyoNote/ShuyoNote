...
3. ~~**Tongsuo 的 MSVC spike 先做**~~ → **已降级（2026-09-17）**：路径 3 暂缓后，这个 spike **不再是主线前置**，
   只在"哪天要重启路径 3"时才做（第一关仍是 Perl ＋ Configure）。而且它的失败路径（退回路径 2）
   **现在就是既定形态** ⇒ 没有"必须成功"的压力，也不阻塞国密主线与 PDFium。
