...
| **页面级索引编排**（`indexPage`） | **Windows** | 两块入口各自都在，但**没有任何地方按"一个页面"把它们串起来** —— 而"页面保存后这一页可检索了吗"才是那个单位 | ✅ **已完成**（`src/lib/indexPage.ts`：页面正文分块 + 该页每个附件抽取分块 + 一句话摘要）。三条不变量有判据：**单附件失败不拖垮整页**、**重复调用很便宜**（cached/unchanged）、**作用域不越界**（只按页取，不把"未整理"算进来） |
