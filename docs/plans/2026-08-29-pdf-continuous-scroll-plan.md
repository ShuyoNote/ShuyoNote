...
## 6. 增量：缩放下拉 + 点击 PDF 直达阅读器（v1.59.188）

### 6.1 修复「缩放迟钝/抖动」
**根因**：连续模式下页块宽固定为视口内容宽（与 `scale` 无关），`scale` 只改栅格分辨率、不改变显示尺寸，也从不重新光栅化（数据加载只在 `meta` 缺失时拉页，加载完就不再动）。结果是：点 +/− 只把舞台滚回页顶（抖动），页面毫无视觉变化（迟钝/像"没反应"）。

**改法**：
- 页块显示宽改为缩放驱动的 `contentWidth = refW × scale`（`refW` = 基准页宽，取首页 meta），真正缩放。
- `scale` 变化后对**每个可见页**按当前缩放重新光栅化（缓存键 `${i}@${scale}`，缓存命中则跳过，LRU 淘汰跳过仍挂载页）；`meta`/文本层仍是缺失时一次性拉取（不随缩放重拉）。
- 连续模式页图像 `width: 100%`（铺满页块宽），否则被 `max-width:100%` 卡在内容宽。
- 缩放改用**下拉菜单**：即时显示当前 `%`，点开选预设（50/75/100/125/150/175/200/250/300%）或「适配页宽」；键盘 `+`/`-` 微调、`F` 适配页宽、`Esc` 先关下拉再关阅读器。
- 纯函数 `fitScaleForWidth` / `zoomContentWidth` / `MIN_SCALE` / `MAX_SCALE` 抽入 `pdfLayout.ts`，配 5 条新断言锁缩放语义。

### 6.2 点击 PDF 节点直达阅读器
**改法**：`AttachmentPanel`（附件打开按钮 + 名称）+ `PageTree`（文件树节点）在 `mime === "application/pdf"` 时改为 `usePdfReader.getState().openPdf(id, name)`，不再用默认程序打开（`opener.openPath`）。

### 6.3 验收
- [ ] 打开 PDF 后点缩放下拉选 150%/200%：页面真实放大且清晰（1:1），不再"点了没反应"或抖动。
- [ ] 缩放下拉旁「适配页宽」一键回到铺满；`+`/`-`/`F`/`Esc` 快捷键正常。
- [ ] 文件管理器/文件树里点 PDF 附件：直接打开内置阅读器（可批注），不跳默认程序。
- [ ] `tsc` / `smoke`（314）/ `pnpm build` 全绿。

---

## 7. 增量：缩放下拉重构为桌面阅读器式（v1.59.189）

参考桌面 PDF 阅读器的缩放下拉，把 v1.59.188 的简单百分比菜单升级为完整形态：

- **触发器**：`− [当前值 ▾] ＋`（左右各一个沿阶梯步进的按钮，中间是显示当前模式/百分比的下拉触发器）。
- **下拉内容**：
  - 顶部适配模式：实际大小 / 适合页面 / 适合宽度 / 适合内容 / 自定义缩放；
  - 百分比阶梯 **6400% → 8.33%**（含 150/125 等常用档），当前值打勾（✓）；
  - 底部「设置默认缩放比例」（存 `localStorage`，下次打开默认用它；未设置则适合宽度）。
