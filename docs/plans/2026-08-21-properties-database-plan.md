...
| checkbox | `"true"` / `"false"` |
| select | 选项名（单个） |
| multi | 逗号分隔文本 `a, b`（实际实现，偏离原 JSON 数组约定） |
| tag | **不存 `page_props`**——作为 `page_tags` 的读写视图，`value` = 该页标签名逗号拼接 |
| ref | 目标 page_id / block_id（前缀 `p:` / `b:`） |

> 用 `page_props` 的 `value` 统一存 TEXT，牺牲少量查询性能换「无 schema 迁移成本」；排序/过滤时按 type 解释。
> `tag` 类型特例：写入时经 `sync_page_tags` 落到 `page_tags`（get-or-create 标签 + 替换关联），读取时从 `page_tags` 动态拼接，故与标签面板/侧栏筛选/关系图天然一致。
...
- **M3（数据库看板视图 + 看板泛化）** ✅：现有 BoardView 泛化为数据库看板（分组字段可选任意 select 属性）；画廊视图可选。验收：看板能按任意 select 属性分列。
- **M4（整合收尾）** ✅：关系图按属性过滤/着色、搜索 `prop:属性=值` 语法、命令面板入口。验收：跨视图属性语义一致。

## 9. 测试与验收标准

- [x] 给页面添加/编辑/删除自定义属性后，重载仍保持。
- [x] 属性类型正确渲染（数字/日期/布尔/单选下拉/多选/标签/引用，多选与标签为逗号分隔文本）。
- [x] 数据库表格能加列、删列、排序、过滤、单元格内联编辑、select/multi 选项管理。
- [x] 看板能按任意 select 属性分列，且与旧「按标签分列」行为一致（回归）。
- [x] 标签作为 `tag` 属性与标签系统互通（写入落到 `page_tags`，读取动态拼接），在属性面板 / 看板 / 关系图中语义一致。
- [x] 关系图能按标签或 select 属性过滤与着色。
- [x] 数据库支持 表格 / 画廊 / 看板 三种视图。
- [ ] 同步 / 版本历史 / 回收站不受影响（属性随页面走；`tag` 类型作为派生视图不随 `content_json`）。
