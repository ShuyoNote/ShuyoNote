...
> **最终实现：采用 4.2 的 JSON 后处理方案**（更轻、零核心节点类改动）。落地为：保存时按 `$getRoot().getChildren()` 顺序把 `blockId` 注入顶层节点 JSON（用 `Map<NodeKey, string>` 记忆），加载时按序回填；复制/粘贴自然获得新 ID（新 key = 新 blockId），拖拽排序不丢失。详见 §12 实施记录。
...
| 里程碑 | 状态 | 实现要点 |
|--------|------|----------|
| **M1 块身份** | ✅ | JSON 后处理注入稳定 `blockId`（`Map<NodeKey,string>` 记忆）+ `blocks` 表 + `backlinks` 块级化迁移 + `rebuild_block_graph` |
| **M2 块引用** | ✅ | `BlockRefNode`（序列化 `targetId`）+ `/引用块` 选择器 + 点击跳转/悬停预览 + `rebuild_block_backlinks` 记录块级反链 |
| **M3 反链面板升级** | ✅ | `list_block_backlinks`（含 `target_snippet`）+ 面板分「块级引用 / 页面引用」两组 |
| **M4 块嵌入** | ✅ | `BlockEmbedNode`（序列化 `targetId`）+ `/嵌入块` + 只读镜像 + 跳转/刷新 |
| **M5 关系图** | ✅ | `get_graph`（页面/块节点 + 边）+ 力导向 SVG 图 + 块级图层开关 |

> **关键取舍**：块自身 ID 用顶层字段 `blockId`，引用/嵌入的目标用 `targetId`，避免嵌入块「自身 ID」与「目标 ID」字段冲突。块级反链从结构化 JSON 解析（`type: blockref/blockembed` 节点的 `targetId`），不依赖正文正则。

## 13. 测试与验收标准

- [x] 新建/编辑/保存后，块 ID 稳定不变（多次重载后 `((blockId))` 仍有效）。
- [x] 复制粘贴块 → 新块 ID 不同，引用不串。
- [x] `[[标题]]` 页面级反链行为与旧版一致（回归）。
- [x] `((blockId))` 点击跳转、悬停预览、目标删除后显示「失效」。
- [x] 反链面板正确分组，块级反链带内容片段。
- [x] `{{blockId}}` 嵌入实时刷新（保存后 bump 缓存失效信号），双向不互相污染（嵌入是只读镜像）。
- [x] 全文搜索仍能搜到引用语法内的文本（`content_text` 含引用目标展示文案）。
- [x] 同步/版本历史/回收站不受影响（块 ID 随 `content_json` 走）。
