...
⚠️ **仍没做**：真机双设备验收（要人手）。
（订正 2026-09-23：原文这里还写着"块级角标"没做 —— 它已在 §8 落地并进了 dev：`3e0f5993 feat(crdt): 冲突块级角标 ＋ 定位`。
阶段 1 因此只剩"真机双设备验收"这一条要人手的事；阶段 2 的前置清单见
[阶段 2 开工清单](2026-09-23-crdt-stage2-kickoff.md)。）
...
## 13. 验收期真机上抓到的两个坑（2026-09-22，第十二段；**都已修**）

两个都**不是本片带来的**（13.1 在纯 dev 上就能复现，13.2 是 CSS 里的老魔法数），但都**直接卡住人工验收**
（一个让"同步成功"看起来是红的，一个让面板里的按钮看起来是坏的），所以就地修掉并留下读数。

### 13.1 「33 个传输失败」是假的：N 恒等于分桶目录数

真机读数（桌面端 1.91.10，空间 `test`）：面板报
`「…」同步完成（33 个传输失败）：上传 0 / 拉取 0 耗时 0.1 秒`；过一会儿再同步，同一个数字变成 **57**。

真因（`src-tauri/src/sync.rs` 第 2 步"本地 hash 清单"）：**平铺布局那一支扫描时没有排除目录**，
而分桶目录名恰好是**两个十六进制字符**（`08` / `0a` / …）⇒ 每个桶都被当成"本地待上传附件"进了
`local_set` ⇒ 进了 `up_items` ⇒ 在上传循环里过不了 `is_valid_attachment_hash` ⇒ `failed += 1`。
于是 **N 恒等于桶数**，与真实传输毫无关系——而且 0.1 秒就"失败"完：一次网络请求都没发出去。

证据（同一时刻的三份集合，**都是 56**）：

| 集合 | 值 |
|---|---|
| 桌面本地已落地（`%APPDATA%\cn.shuyo.shuyonote\attachments` 里的 64 位 hex 名） | 56 |
| 服务端 `attachment_meta`（空间 `test`） | 56 |
| 服务端 blob（`_acceptance\attachments\<space>\<hh>\`） | 56 |
| 差集 `local − remote` / `remote − local` | **0 / 0** |

⇒ 两端内容**完全一致**，红字纯属账目错。修法：这段扫描抽成 `scan_local_attachment_hashes(dir)`——
**目录只认两字符十六进制的分桶目录，其它目录一概不算附件**，`*.part` 照旧排除。
判据 `local_attachment_scan_ignores_bucket_dirs_and_part_files`：分桶里的 hash 要认、老平铺的 hash 要认、
桶名与 `.part` 都不许进集合。

⚠️ 诚实边界：真·历史遗留文件（例如 2026-09-15 那批 `9d.png`）**仍然**计入 `attachments_failed`：
它们确实传不上去，但把"存储名不是内容哈希"计成"**传输**失败"是口径不严。本次没动它——要动就得给
`SyncReport` 加一个字段，牵动 `commands.ts` / `CommandMap` / 面板文案整条链。记在这里，留给下一次口径整理。

### 13.2 同步面板把输入框画在自己背后（F3「按钮没反应」的真身）

真机现象：从同步面板里发起「新建空间 / 重命名 / 设备密钥」这类输入框 ⇒ **看不见窗口**，
用户体感就是"点了按钮没反应"。

真因（`src/App.css` 的层叠）：`.sync-popover` 写的是魔法数 **1500**（窄屏弹层 **1600**），
而输入/确认框 `.confirm-overlay` 是 **1400** ⇒ 输入框被面板盖在下面。
`:root` 的分层规范本来写着"浮层 flyout（回收站/备份/模板/**同步**）= `--z-popover`(700)"；
1500 是后来为了"别被文件预览挡住"抬上去的，但预览那层是 `--z-content-overlay`(**600**)——
**700 本来就高于它**，这次抬升只买到了这一个 bug。

修法：桌面 `.sync-popover` 与窄屏 `.sync-popover(.is-sync)` 都回到 `var(--z-popover)`(700)：
700 > 预览(600)，且 < 模态(1000) / 右键(1100) / 确认框(1400) / 命令面板(1500) / toast(1600)。
窄屏那排 1600 的其它弹层（搜索 / 回收站 / 空间切换 / 备份 / 新建菜单 / 版本历史）**不动**：
它们的父级可能是全屏模态（如设置里的备份下拉），压低会反过来被模态吃掉——只压低"自己会弹输入框"的同步面板。

**改后读数**：`sync::` Rust 用例（含新判据）见本节提交记录；`pnpm verify` 与上一节 §12 同口径。
