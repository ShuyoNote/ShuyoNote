...
### 5.2 决定：先保底，再用 spike 定路径 3

1. **路径 2 即刻成立**：档 3 做完，链路上的载荷本身就是 SM4 密文 —— **不需要任何额外开发**；
2. **立一个 spike 决定路径 3**（与 PDFium 那次同一个套路，先消灭最大的未知）：
   在 **MSVC** 上把 **Tongsuo 构建出来**，并用 `rust-openssl` 开一条 **TLCP** 连接（Tongsuo 自带测试证书即可）。
   - 成功 ⇒ 路径 3 可做，按 1–2 周排；**失败 ⇒ 用书面口径把路径 2 说明白**（数据面国密 ＋ 传输层标准 TLS），而不是硬啃构建链；
3. **风险预判**：Tongsuo 与 OpenSSL 同源，构建同样要 **Perl + Configure** —— 本项目在 Android 上**已经因为 Git-for-Windows 的精简 Perl 卡过一次**（`Cargo.toml:108-109`）。**spike 的第一关就是这个**，别等排期到后半段才发现。
...
| **传输层** | 按 §5.2 结论 | 路径 2：数据国密、传输标准 TLS；路径 3：TLCP |
...
2. **传输层走路径 2 还是路径 3** —— ✅ **已定策略**：甲方**无**国密网关 ⇒ 路径 1 排除；
   **路径 2 即刻成立**（档 3 的载荷本身就是 SM4，零额外开发），**并立 spike 决定路径 3**（先验 Tongsuo 在 MSVC 上的构建链，§5.2）；
   spike 失败 ⇒ 用 §5.3 的边界表把路径 2 的口径写清楚，不硬啃构建链；
