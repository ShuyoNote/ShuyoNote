...
| 平台 | 现状 | 要做的话是什么 | 粗估 |
|---|---|---|---|
| **Android** | 🟡 **进行中（owner 2026-09-23 拍「做」）**。此前未排、两边都建议"先不做"，owner 覆盖。系统**没有** OpenSSL ⇒ 必须**随包**一份 SM 版 | 已落（AMD，2026-09-23）：① **取库链** `scripts/build-tongsuo-android.sh`（Gitee 镜像 @ 钉死 commit `540603a3` ＋ NDK r29 ⇒ 静态 `libcrypto.a` 10,931,686 字节、SM 符号 190、**不产出 `libcrypto.so`**）＋ `scripts/check-openssl-android.mjs`（**属性判据**：静态/大小带/headers/SM 符号；sha256 只作读数 —— 实测同机同参数连编三遍是三个不同值，见该脚本文件头）；② **产物门禁** `scripts/check-android-crypto.mjs`（**三条**：包里不该有 `libcrypto.so*`；应用 `.so` 的 DT_NEEDED 里不该有它；★ **正一半** —— 应用 `.so` 里必须找得到**补丁引入的字面量** `PBKDF2_HMAC_SM3`/`HMAC_SM3`）；③ **workflow 接线**（android.yml：导出 `ANDROID_NDK_ROOT`、交叉编译、`--print-env` 进 `$GITHUB_ENV`、`cargo fetch` 前置、`sm-library-build --prepare --require-static` 打补丁、构建加 `--features sm-library`）。★★ **2026-09-25：本机真的把 APK 出出来了，并在真产物上跑了门禁**（当时 GitHub 镜像停着 ⇒ 不等 CI，自己造读数）：`app-universal-release-unsigned.apk` **52.9 MB**、`build exit=0`；`check-android-crypto` ⇒ **exit=0 三条全过**：✓ 包里没有 `libcrypto.so*`、✓ `lib/arm64-v8a/libshuyonote_lib.so`（52,847,416 字节）的 DT_NEEDED 里**没有** `libcrypto.so`、✓ 该 `.so` 里有补丁字面量 `PBKDF2_HMAC_SM3`（×3）、`HMAC_SM3`（×6）。★ **那条"待判定"已判定**：Android 走的是 crate 自带的 **vendored OpenSSL**（日志里 `openssl-sys` 自己 `./Configure … linux-aarch64` 从源码编）⇒ **自编静态前缀在 Android 上不被使用**，要害是**补丁 ＋ `--features sm-library`**。★ **2026-09-25 更正一条旧结论**：此前写的"正一半验不了，因为 release 被 `strip=true` 剥了符号"**是错的** —— strip 剥的是符号表，不是 `.rodata` 里的字符串；真产物上数得到那两个字面量，且它们在**未打补丁**的 registry 源码里出现 **0** 次（⇒ 命中即"打过补丁的那份源码被编进来了"），门禁因此补上第③条并做了变异（假 APK ⇒ exit 1 ＋ 指名"编的是原版 SQLCipher"的成因；空输入 ⇒ 全部 missing）。本机出包的配方级坑（perl 要 Unix 路径那份、make 用 NDK 自带、rustup/Gradle 换镜像、`CARGO_HOME` 必须来自 `--print-env`）见 `docs/TESTING.md` 安卓一节新增块。**仍未做**：CI 那条 job 的读数（等镜像；★ **我自己造的 CI 红有两层，都已修**：① patch 步骤原先排在**任何 `cargo fetch` 之前** ⇒ runner 上 registry 是空的、`sm-library-build` 如实拒绝（补 `cargo fetch --locked --target aarch64-linux-android`，`619c4877`）；② 补丁自 2026-09-23 起只打在 `.gm-build/` 的**私有副本**上，而两条 android job 都**没有把私有 `CARGO_HOME` 交给构建步骤** ⇒ `build.rs:229` 如实 panic「找不到 §3.1 的 SM3/SM4 provider 补丁」，因为它拿 `Cargo.lock` 的版本去**共享** registry 里找（`38db5db4`，两条 job 同名加了一句 `sm-library-build --print-env >> "$GITHUB_ENV"`）。**取证方式值得记**：这两层都是**读真 CI 日志**读出来的 —— 第一层修完，红**前移**到下一步（step 21 → step 23），第二层才露头；`web_fetch`/API 读 run 的 job/steps 比"本地复跑猜"快得多）＋ **真机验收**（人手；字面量在库里 ≠ 运行期 provider 支持 SM3，"SM provider 真生效"只能真机跑一次） | 2–3 人日（编库 0.5 ＋ 接线 0.5 ＋ 判据/真机 1–2）—— 取库链、判据与**本机真产物读数**都已在 2026-09-23/25 落完 |
