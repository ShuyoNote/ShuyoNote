...
> 关联方案：[跨平台适配](2026-08-24-cross-platform-plan.md)（M16 全平台底座）与 [每空间独立存储](2026-08-22-per-workspace-storage-plan.md)（存储布局）；受益于 M16.1b 已打通的 web 真实 SQLite / 属性 / 数据库 / 版本 / 块引用 / 备份。
...
- **已实现**（2026-09-01 核实）：`persist()` 已有 `onPersistError` 钩子（catch + 不抛错 + 保留内存态）；`web.ts` 设置 `console.error("[web] persist failed")` + `emitPageEvent("persist-error")`；`App.tsx` 监听 `persist-error` → toast 提示（可知晓未落盘）。**验收满足**：模拟 adapter 失败不抛错打断 CRUD、控制台有明确错误、UI 有提示。

### 2.6 🥈 P1 — 大媒体内存优化（✅ 已实现[限制层]）
...
- **已实现**（2026-09-01 核实）：文件选择（`web.ts` 文件对话框）对单项 >50MB 直接跳过并用 `window.alert` 明确提示「请用桌面版」，避免大文件 OOM；视频缩略占位仍待做。

### 2.7 🥉 P2 — 自动化测试补强（✅ 已覆盖）
- **问题**：拖拽、看板、错误路径无自动化。
- **做法**：把拖拽的 `computeReorder`、`move_page` 语义抽成纯函数单测；为主流程补 `node:test`/vitest；给 `search` 打分排序加用例。
- **已覆盖**（2026-09-01 核实）：`scripts/smoke-web.mjs` 已断言 `computeReorder`（treeReorder）、**search 打分排序**（TF 在前）、**`move_page` 防环**（自下/自身均被拒）。
...
> 其中「同步」这条边界的完整推理（四层原因 + 代码出处 + 若要开启的 W1–W4 分阶段路线）见 [Web 同步能力边界](../web-sync-boundary.md)。
