...
- 栏数在「+」插入菜单的**分栏行右侧展开的二级子菜单**中选择：**光标移到「分栏」行即自动展开** `.insert-columns-sub`（2/3/4 栏缩略图，无需点击），移开自动收起（带 linger）；选中后 `$createColumnsNode(count)` **直接生成对应列数并插入**——不再有插入后的浮层选择器。子菜单以 `position:fixed` 锚定在行右缘，因此不受父面板 `overflow:hidden` 裁剪。
...
- [x] `/分栏` 与「+」插入「分栏」块；「+」分栏行**光标移上右侧自动展开二级子菜单**选 2/3/4 栏（无需点击），选中直接插入对应列数。
