...
> 目标：把 ShuyoNote 从「**单库 + 全局附件，空间逻辑隔离（workspace_id）**」升级为「**每个工作空间独立的 SQLite 数据库文件 + 独立的附件目录（物理隔离）**」——让每个空间可**单独搬移 / 单独备份 / 单独加密 / 独立删除**，数据主权按空间颗粒度。
...
| 附件 | 全局内容寻址 `attachments/`（跨空间去重） | 每空间 `spaces/<ws_id>/attachments/` |
| 隔离 | 逻辑（`pages.workspace_id` + 查询 scope） | 物理（独立文件） |
| 单空间搬移/备份 | 难（`export_workspace` 未实现） | **易**（拷贝库+附件目录） |
| 单空间加密 | wire 层、同文件 | 可**按文件**整体加密 |
| 删除空间 | 软删 + 集中物理清理 | 删一个目录（干净） |
| 单空间损坏 | 影响整库 | 只影响该空间 |
...
└── spaces/
    ...
    │   │   └── changes(该空间 outbox) / attachments(该空间文件表)
    │   └── attachments/       # 该空间附件（内容寻址，空间内去重）
    └── <workspace_id2>/ ...
```
...
| 附件**字节** | 每空间目录 | 内容寻址，**空间内**去重 |

> **代价（诚实标注）**：
> - `attr_defs` 变为**每空间一份**——「同名属性跨库语义一致」的哲学在物理隔离下**不再成立**（每个空间有自己的属性定义）。这是物理隔离的必然取舍。
> - 附件**跨空间不再去重**——同一文件在两个空间各占一份字节。与「全局内容寻址去重」相冲突，属物理隔离的取舍。
...
2. **附件**：**每空间一个 `attachments/` 目录**（内容寻址，空间内去重）——保证「空间 = 可整体搬移的单元」。跨空间相同文件不共享字节（接受）。
...
| 能力 | 现状（单库） | 改后（跨库） |
|---|---|---|
| **全空间搜索** | 单 `page_fts` + `all_spaces` | 遍历所有空间库，各自 FTS 检索后**前端合并**（或 Rust 层聚合） |
| **跨空间复制页面** | 单 `copy_page_to_workspace` | 打开目标空间库，跨库插行 + 将附件字节拷入目标空间附件目录（目标内去重），重新映射块引用 |
| **共享模板** | `templates`（全局） | 不变（meta.db 全局） |
| **移动附件（move_attachment）** | 单库改 `page_id` | 仅空间内；跨空间移动需拷贝字节到目标目录 |
| **空间清理 / 统计** | 单库统计 + 全局零引用 | 每空间库分别统计；孤儿附件按**各自空间**判断（不再跨空间） |
...
2. **阶段 1（拆库器，幂等+可续）**：对单库中**每个非删除空间**，建 `spaces/<ws_id>/shuyonote.db`，把该空间的
   `pages/props/attr_defs/tags/page_tags/backlinks/blocks/database_columns/db_views/db_rule/page_versions/page_fts/changes` **按 `workspace_id` 过滤**复制过去（build 其 FTS/块图）；把该空间页面引用的**附件字节子集**拷到 `spaces/<ws_id>/attachments/`。
   - **幂等**：记录已迁移空间 id，重跑跳过；**可续**：失败重试不丢进度。
   - **校验**：每个空间库行数 == 原库该空间行数；FTS 行数 == pages 行数。
3. **阶段 2（命令层改造）**：所有 Rust 命令改为「解析 active → `conn_for(space)`」；查询去掉 `workspace_id` 过滤（每库单空间）；`active_workspace_id` 改读 `meta.db`。逐个回归 list/tags/trash/search/graph/backlinks/db/属性/版本。
4. **阶段 3（跨空间适配）**：全空间搜索跨库合并、跨空间复制跨库实现、备份/导出改为
   - `export_workspace`：`spaces/<id>/shuyonote.db` + `spaces/<id>/attachments/` 打成一个可独立导入的 zip（**单空间搬移**）。
...
- **M15.1 拆库迁移器**：单库 → 每空间库 + 附件子集（幂等/可续/校验/回滚）。
- **M15.2 命令层改造**：所有内容命令走 `conn_for(space)`，去掉 `workspace_id` 过滤；回归各内容查询。
- **M15.3 单空间备份/导出**：`export_workspace` / `import_workspace`（空间 = 库 + 附件 zip）。
- **M15.4 跨空间适配**：全空间搜索跨库合并、跨空间复制跨库 + 附件拷贝。
- **M15.5 验收 + 清理**：全功能回归；归档原单库。

## 8. 测试与验收标准

- [ ] 每个空间有独立 `shuyonote.db` 与 `attachments/`；`meta.db` 只含 workspaces/sync/templates/plugin。
- [ ] 迁移后每空间行数/FTS 行数与原库一致；原库保留且可回滚。
- [ ] 切换空间 = 打开另一空间库；list/tags/trash/search/graph/backlinks/db/属性/版本 行为不回归（每库单空间）。
- [ ] `export_workspace` 产出的包可独立导入到另一设备（库 + 附件一起），空间内容完整。
- [ ] 全空间搜索能跨库返回各空间结果并标注空间名。
- [ ] 跨空间复制页面：字节拷入目标空间附件目录，目标内去重。
- [ ] 空间清理按各自空间统计、孤儿附件按各自空间判断。
- [ ] 每空间 `changes`（outbox）独立，同步互不影响；每空间同步游标独立。

## 9. 风险与取舍（诚实标注）

- **高成本 / 高风险**：命令层 + 连接托管全面重构；迁移是破坏性的（但设计了「新增+校验+切换」+ 原库保留回滚）。
- **去重失效**：附件跨空间不再去重，跨空间相同文件各占一份（换取可独立搬移）。
- **跨库一致性**：目前全空间搜索/复制/统计从单事务变成跨库操作，需在 Rust 层聚合/拷贝，无跨库强事务。
...
「每空间独立存储（物理隔离）」能带来**可搬移 / 可单独备份 / 可单独加密 / 故障隔离**的真实价值，但代价是**跨库一致性、附件去重失效、attr_defs 空间化、命令层大规模重构与破坏性迁移**。方案按 **M15.0 → M15.5 分阶段**推进，迁移采用「**新增 + 校验 + 切换指针 + 原库保留可回滚**」的安全策略；建议**先做 M15.0–15.2（底座+拆库+命令改造）并充分验证**，再评估是否继续跨空间与导出。配套取舍见[设计哲学](design-philosophy.md)「空间是隔离容器」。
