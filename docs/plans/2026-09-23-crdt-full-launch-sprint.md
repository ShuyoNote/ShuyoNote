...
> owner 2026-09-23 口述口径：**「不考虑向后兼容，冲刺 CRDT 全上线」**。
> （⚠️ 该口径**不含**"现在没有正式客户"这个前提 —— 早先的转述里带过它，2026-09-23 已按 owner 的
> 口述**更正**：不向后兼容是**直接的要求**，不是"因为现在没人用所以可以随便"。见 §0 的两处推论。）
...
> ★★ **现状指针（先读这一行，再读下面的历史记录）**：本文是**逐轮追加**的记录 ——
> §2 的切片表、§9/§10 的清单与读数**都是各自当时**的状态（例如 §2 里 S9 写着"待发版／桌面侧 claim 待补"、
> §10.1 写着"未部署"、§10.3 列着七条缺口）。**这些都已过期**。
> **实况以 §11–§15 为准**（另见 [S5 决策稿](2026-09-23-s5-server-merge-decision.md)、
...
| 前提 2「混版本降级」（老客户端只写 JSON） | ❌ **作废**：口径是**不考虑向后兼容** ⇒ 不保混版本、所有人一起升（**理由就是这条口径本身**，不再挂在"没有正式客户"上） |
| §0.6「发布版一律恒关」 | ❌ **作废**：不要求"关着逐字节不变"当上线门槛；开关只为开发期对照保留 |
| 前提 4「全库补种块身份」（只服务"持久化 ydoc 状态"那条路） | ✅ **重新变成必须**（因为那条路就是全上线唯一可走的路）—— 但补种时机由 S1/S2 定 |
| 面 5「E2EE 加密快照」 | ⏸ **不阻塞**：今天服务端存明文是可接受的临时态；面 5 落地时另做迁移（**不考虑向后兼容** ⇒ 届时可以要求一起升，迁移成本可控）。★ **按空间分道（2026-09-23 总口径）**：服务端存明文说的是**团队空间**；**个人空间的服务端只落密文**（E2EE），因此**面 5 只服务个人空间**，与团队空间的合并/检索/AI 是两股道 —— 见 [数据可见边界](../sync-server-data-boundary.md) §0.5 |

**仍然有效的纪律（不因「冲刺全上线」而放弃）**：不静默失败（该抛就抛）、块身份不许丢/不许重、
...
★ **已治（2026-09-23 第 49 轮）**：那条判据改为「**读数**（跑 3 次打印最快值，闲时实测 30–34ms）
＋ **1500ms 数量级兜底**」，**没有**去调大 250；并带**变异实测**（`settle` 每帧塞 ~15ms ⇒ 红 1622ms；
还原 ⇒ 绿 30/32/31ms）证明兜底仍咬人。细节见 `docs/TESTING.md` 的「计时类判据」条。
...
1. **投影列的滞后**：打开页面时合并/承接只写**状态**（＋hydration 把内容落进编辑器），
   ...
   **不是**新引入的静默 —— 但确实是滞后，记在这里。
   ⇒ **方案稿已出**（只出稿、未实现）：[投影与派生滞后：方案稿](2026-09-23-projection-lag-closure.md)
...
2. **`pendingSkipped > 0` 现在只到 `console.warn`**：没有走 S8 那条"冲突可见 + 用户可裁决"的出口
   ...
   ⇒ **方案稿已出**（只出稿、未实现）：[血统冲突的「可裁决」那一半](2026-09-23-lineage-conflict-adjudication.md)
   —— 三条真实选项（留本机／用对端／**两个都要 = 一页变两页**，只有第三个不丢数据）、三条路对比
   （复用 `page_conflicts` **不可**／新表 `page_lineage_conflicts`／先做"把对端那版另存成新页"）、
   8 条承重判据与 5 条风险。**建议先做最小那一步（另存成新页），再按新表补三个显式选项。**
   ★ **第 49 轮已按"B 的骨架 ＋ C 的动作"实现**（owner 拍的是**显式按钮**；方案稿 §9 有读数与判据）：
   · 新表 `page_lineage_conflicts`（页级本地；**含对端那一版的投影快照** —— 待并状态随后会被 `clearPending`
     清掉，不留快照就无从救援）＋ 三条命令 `record/list/resolve_lineage_conflict`（两侧 schema 与语义同形）；
   · 两处记录点共用同一份指纹口径（web 当场合并 / 桌面端口），**去重＝同一对指纹只提一次**（在存储层）；
   · 小横幅 `LineageConflictBanner`（挂 `App.tsx`，与块级 `ConflictBanner` 并列）：**「另存为新页」／「保留本机」**；
   · 判据 **4（Rust）＋ 4（docContent）＋ 1（pageBinding ㉑）＋ 5（横幅）＋ 1（文本级接线）** 全绿；
     **变异实测**：短路 TS 去重闸门 ⇒「同一对指纹只提一次」**红**；还原全绿。
   · 当轮读数：Rust 全量 **555 passed / 0 failed**、vitest 全量 **2194 passed**、build 0、
     `build:web`＋`check:web-build` **9/0**、`test:sync-verify` **84/0**、
     命令 **Rust 251 / web 249 / 契约 253**、收口门禁 **562（基线 562）**。
   ⚠️ **仍未做**：**②「用对端」**（放弃本机血统改接对端）—— 要动本机血统的取舍，且必须先有"本机这版
   也不丢"的去处；今天走 ③（另存为新页）已能"两边都在"，所以 ② 的边际价值低于风险（方案稿 §9 末尾）。
...
计时判据的教训：负载下会假红；那条已在第 49 轮改成"读数 ＋ 数量级兜底"，但**结论不变**：
新判据仍然不该拿墙钟当门禁）。
...
>
> ★ **2026-09-23 总口径之后这句要改读**：那条"输入约束"已经**由作用域化解** ——
> 个人空间 E2EE（服务端只落密文）、团队空间放弃零知识（[数据可见边界](../sync-server-data-boundary.md) §0.5）
> ⇒ **服务端合并只对团队空间**，个人空间载荷永不上服务端合并；面 5（E2EE 快照）只服务个人空间。
> ⇒ 决策的实质回到"**值不值得为团队空间做**"（S5 决策稿 §3 的 ①②③）。

⇒ 本轮**不落实现**：只出稿 ＋ 留读数，等 §3 的四条触发条件之一成立再动工。

## 15. 第 46 轮：文档口径对齐 ＋ 桌面流通道设计稿（**未实现**）

### 15.1 口径对齐：把"在用的口径"改成实况（历史记录不动）

| 改在哪 | 原来写的（过期） | 现在的口径 |
|---|---|---|
| 本文开头 | 无 | 加**现状指针**：§2/§9/§10 都是各自当时的记录（S9"待发版"、§10.1"未部署"、§10.3 七条缺口**均已过期**），**实况以 §11–§15 为准** |
| `docs/SHUYONOTE_STATE.md`（"下一阶段"第 3 条） | "客户端 **Web** 全链路已通…卡在**桌面侧消费 `crdt_state`**" | 客户端**两侧**都已消费（第 43 轮）＋ claim 与两套 id 修正已接；只剩**真机验收**与**真账号探针** |
| `docs/roadmap.md`（§3.5 与 P1 行、竞品差距表、CRDT 末表） | "阶段 2 在飞（…默认关闭的平面开关）"、"**剩三件**：…S9 服务端首写者裁定（要动另一个仓，等 owner 点头）" | 阶段 2 **客户端两侧已落地**；S9 **已实现并已发版**、桌面 claim 已接、yrs 对拍与体积读数已做；只剩真机验收/真账号探针/S5（暂缓） |
| `docs/SYNC.md` §五 | "这不是 WebSocket 推送，而是**增量轮询**" | 改成**三条通道并存**（手动/轮询/**SSE 变更流**）＋ 端点与 keep-alive 形状 ＋ **SSE 仅 Web** ＋ 订阅按当前工作空间解析 ＋ "这条延迟对 CRDT 更重要" |
| `docs/JOINT-ACCEPTANCE.md` §6 | "剩下的外部件是**用户可见的平面设置项**（今天靠 `VITE_CRDT_PLANE=1`）" | 见 §15.2 —— 那件不是"待做"，而是"**该撤没撤**" |

### 15.2 ★ 对齐时撞出的一条**真发现**：该撤的平面开关**至今还在**

[边界决策](2026-09-23-crdt-plane-boundary-decision.md) §6.2 拍了"Slice B 在**磁盘边界**的那三处接线
**应当撤出**（`docContent.ts:58/83/100` 各回一行原样调用）"。第 46 轮核对代码：
**`VITE_CRDT_PLANE` 与 `throughCrdtPlane` 仍然存在**，`docContent.ts` 那三处仍包着它。

它今天**既不推进合并**（口径 1 作废后开着只是"把已落盘 JSON 归一化一次"），**也不再是任何判据的前提**
（wire 平面走的是另一套注入 `setCrdtRemoteApplier`）⇒ 建议**单独一片撤掉**：
撤的时候 `plane.path.test.ts` 那 9 条要按新边界**重写**（**不许删判据不写替代**）。
⚠️ 在此之前，`JOINT-ACCEPTANCE.md` §5 的 j6 剧本第 2 步仍照旧可用（那段没说错，只是多了一层没人需要的东西）。

### 15.3 桌面流通道：出稿，不实现

缺口＝**桌面没有推送通道**（`src/hooks/useSyncStream.ts` 对桌面直接 `return`）⇒ 桌面的"近实时"实际由
**轮询间隔**决定，而它同时就是 **CRDT 合并的可见延迟**。设计稿：
[桌面「近实时」流通道：设计稿](2026-09-23-desktop-near-realtime-stream-design.md)。

要点：服务端端点**已有**（`GET /spaces/{id}/changes-stream`，axum `Sse` ＋ `KeepAlive`），
所以这不是"新造通道"，而是"**桌面缺一个 SSE 客户端**"。形态选**"Rust 订流 ＋ 发事件、拉取仍由前端发起"**
—— 唯一理由（也是承重理由）：**自动经过既有的 C2 Wi-Fi 闸门、防重入与状态行**（绕过去就会重演
`syncGate.ts` 文件头记的那个 bug）。含七条判据、两个纯函数（帧解析/退避）、五条风险边界，
以及一条**必须写进语义的**：服务端订阅者落后时发的 `{"type":"ping"}` **意味着可能漏事件 ⇒ 立刻拉一次**，
不许当心跳忽略。
