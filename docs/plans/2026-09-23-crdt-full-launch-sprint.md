...
| **S3b-2d** | **组件接线**（下一件） | `editor/Editor.tsx`：新增 `crdtState?: Uint8Array \| null` ＋ `onStateChange?: (s: Uint8Array) => void` 两个可选 prop；挂载时按 `pageId` 开 `openPageSession({ state \| json: 保存路径的 serializer 产物, editor })`、订阅 `onLocalEdit` ⇒ `onStateChange(exportState())`、卸载 `dispose()`。`App.tsx`：打开页面时 `api.readPageState(pageId)` 传下去、`onStateChange` ⇒ `api.savePageState`；`contentJson` 改走状态的投影 | 真应用里打字 ⇒ 库里的 `page_crdt` 跟着变；切页/卸载不泄漏监听；**未建血统的页面行为不变**（首开建一次、之后只载入）—— 用现成的浏览器门禁（`check:web-build` / `smoke-web`）在**当轮 tip** 上验 |
