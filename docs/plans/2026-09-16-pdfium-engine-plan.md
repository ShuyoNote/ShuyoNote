...
## 0.3 2026-09-20：**P5 切默认之后补的"缺库回退"** ＋ 两类新样本（Windows 侧）

| # | 决定 | 理由与连带 |
|---|---|---|
| **L** | **缺省引擎要"看库在不在"：库在 ⇒ PDFium；库不在 ⇒ 回退 MuPDF**（`PdfEngine::resolve`）。⚠️ **显式** `pdfium` 时**不回退** | P5 已把默认切成 PDFium，而**只有 Windows 的包确定带了库**（§4 验收表里 Linux/macOS/Android 的"首启即可渲染"**未验**）。⚠️ **2026-09-20 更正（原先这里写"会当场渲染失败"，那是错的）**：缺库**不会**渲染失败 —— `src/components/PdfReader.tsx:122-131` 会把原生渲染的异常吞掉、**静默降级到 pdf.js**（页面照样出来、只是慢，现场只有控制台一行日志；`1.91.6` 那封函 §三 已经记过这条）。于是这条决定真正买的是：**别为"装包里没有库"降级到 JS 引擎**——回退到**同为原生**的 MuPDF（保真度、速度都更接近，且 MuPDF 本来就编在二进制里、零额外依赖），而"回滚"靠 `SHUYONOTE_PDF_ENGINE=mupdf` 对终端用户是不存在的选项（用户设不了环境变量）。**显式**要求时不回退，是因为**静默换引擎比报错更难查**（"成功 ≠ 生效"那一族）。回退发生时**只出声一次**（`eprintln!`）。⚠️ 不同意"MuPDF 回退优先于 pdf.js 回退"就把回退撤掉、只留出声，判据随之改 |
| **M** | **对拍样本新增两类，且**分两类判**：`scan.pdf`（图像 XObject）走**硬判据**；`cjk.pdf`（Type0＋预定义 CMap＋标准 CJK 字体名）**只报不判** | 报告 §六 自认的缺口 #1 就是这两类。扫描件里没有字形 ⇒ 两个引擎**应当**逐像素一致，判硬；中文**没有嵌入字体** ⇒ 两个引擎各走各的字体解析/回退路径，字形本就不同（**实测比这句更糟，见 O**），**逐像素等价在这个样本上做不到** ⇒ 它只回答"都能开、尺寸一致、都画出了东西"（非透明像素两边都 > 0），字形差异**如实报出来**。⚠️ 这不是"给红样本开后门"：`scan.pdf` 里的图像特意做成**块状 + 1:1 到设备像素**（96×96 画进 64×64 pt，`SCALE=1.5` ⇒ 正好 96 设备像素），把"插值算法的自由"从判据里排除掉 |
...
| **O** | **`cjk.pdf` 实测读数：非嵌入 CID 字体是 P5 的**已知风险项**，"只报不判"那一类的自检**不足以证明文字画出来了** | WSL2 实测（**装/不装系统 CJK 字体 `msyh`+`simsun`，数字一字不变** ⇒ 不是"本机缺字体"）：MuPDF 墨迹 28000 = 蓝矩形 27000 ＋ **文字 1000**，且这 1000 是**把 2 字节码按单字节**喂回退字体画出的拉丁乱码（`<4E2D65876D4B8BD5>` → "N-e mK"）；PDFium 墨迹 27000 **正好等于那个蓝矩形的面积 ⇒ 整行文字一个像素都没画**（墨迹包围盒顶边 = 矩形顶边）。⇒ ① "两边都画出东西"会被页面里的**矩形**满足，**证明不了文字**；② 这条路径两家都不可信、不可信的方式还不一样 ⇒ 维持"只报不判"，但**不许把它的 ✅ 读成"中文没问题"**；③ **Windows/macOS 侧尚未复核**（PDFium 在 Windows 上走系统字体映射，很可能画得出来）⇒ 复核之前，"中文 PDF（不带字体文件）能不能看"在 P5 的默认切换上属于**未验**，别写进验收表 |
...
| **P3** | **对拍**：同一批真实 PDF（含扫描件、中文、旋转页、超大文件）比较两引擎渲染结果与单页耗时 | 1 人日 | 对拍脚本 + 报告 · **✅ 脚本＋6 个自写样本**（WSL2 实测通过）；⚠️ **中文那一类只报不判，而且实测两家都没把文字画对**（§0.3-O），`scan.pdf` 走硬判据且一次通过 |
| **P4** | Windows 打包验收 → 多平台（macOS bundle ＋ 公证、Linux rpath、Android `jniLibs`）**作为独立任务** | 1 人日 + 2–3 人日 | 各平台安装包 · **🟡 只有 Windows 完成**（dll 进包、sha256 一致、变异实测）；Linux/macOS/Android **未验**（见 P3 报告 §七 第 5 行）；⚠️ **`cjk.pdf` 在 Windows/macOS 上的显示也没复核**（§0.3-O） |
