...
- [ ] **打包**：每个平台的安装包**首次启动即可渲染**（动态库随包、路径正确、macOS 过公证、Android 进 `jniLibs`）
      ...
      **2026-09-20 补**：真 deb 端到端已验（`libpdfium.so` + **随包中文字体** 都在资源目录那一层、sha256 与钉死值一致），
      并把这两件都做成了 `check-linux-bundle` 的产物级判据 ⇒ 见 §0.3-**P**。AppImage 仍**未验**（本机无 Linux 构建环境）。
