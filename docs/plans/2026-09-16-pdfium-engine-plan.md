...
| **Q** | ★ **P4 的 macOS 两格（签名顺序 ＋ 「包里的库真能开 PDF」）—— 2026-09-22，macOS 侧，读数已齐** | **① 签名链**：如实记 —— `tauri build` 产出的 .app **没有任何一步在签名**，实测只有工具链的 **linker 签名** ⇒ `codesign --verify --deep --strict ShuyoNote.app` **❌ `code has no resources but signature indicates they must be present`**。新增 **`scripts/sign-macos-app.mjs`**（Apple 要求的**由内到外**：先 nested Mach-O、最后签 bundle；`--identity -` 默认 ad-hoc，签完自动 `--verify --deep --strict`）。真产物读数：签前 **❌** ⇒ 签后 **exit=0 ✅**；`check-macos-bundle` 仍 **✅**（新增「签名状态」两格）；`spctl -a -vv` **rejected(exit 3)** ＝ **ad-hoc 的边界**（Gatekeeper 要真实身份），不是"签得不对"；公证要 owner 凭据，本侧不做。<br>**量出来三件事**：ⓐ 签名**会改字节**（库 15,219,824 B `3858ed6a…` ⇒ 15,274,928 B `e4a3a51f…`，**+55,104 B**）⇒ "包内必须与 vendor 逐字节相同"只对**未签名**产物成立 ⇒ 门禁改成两条分支（未签名 ⇒ 逐字节；已签名 ⇒ lib 严格校验 ＋ .app `--deep --strict` 都过才接受，否则照样红并指向签名脚本）；ⓑ **内容一致性的可证明时刻在签名之前**（脚本在那时先断言「包内 == vendor」，两个哈希都打印）；⚠️ `codesign --remove-signature` **不能**还原原始字节（15,176,104 ≠ 15,219,824），别拿它当证据；ⓒ ad-hoc 签名对（**文件名 ＋ 内容**）是确定的（同名同内容两次签逐字节相同），跨文件名不可比。<br>★ **顺带查出的顺序问题**：dmg（34,697,548 B）里那份 .app 是**签名之前**的拷贝（挂载实测 `--deep --strict` ❌ exit=1）⇒ **签名必须早于做 dmg**；发版那条路应当交给 Tauri 自带签名（release.yml 已搭好证书导入，矩阵里 macOS 仍"暂缓待 secrets"，bundler 顺序本就是 nested → app → dmg），本脚本管**没有身份**的那半。<br>**② 包级功能读数（不依赖凭据）**：把**包里那份已签名库**喂给真渲染路径 —— `SHUYONOTE_PDFIUM_DIR=<.app>/Contents/Frameworks cargo test --lib --features mupdf-rollback pdfium_matches_mupdf_on_fixtures`。**仪器自证（负对照）**：该变量指向假库 ⇒ 7 个样本全部 `❌ 加载 PDFium 失败（/tmp/boguspdfium/libpdfium.dylib）`；指向包里 ⇒ **1 passed / 0 failed**、RGB 差 ≤ 1、RGB 超阈 0.000%、`scan.pdf` **M9216/P9216**、**`cjk.pdf` PDFium 墨迹 29582 ＋ `non_rect_ink` M1000/P2582**（与 §0.3-O 记的 macOS 读数**逐字相同**）。⚠️ **仍未做（人手，不冒领）**：人工挂 dmg → 拖进 Applications → 首次打开（ad-hoc 会被 Gatekeeper 拦）→ GUI 里目视开 PDF。 |
...
| **P4** | Windows 打包验收 → 多平台（macOS bundle ＋ 公证、Linux rpath、Android `jniLibs`）**作为独立任务** | 1 人日 + 2–3 人日 | 各平台安装包 · **🟡 Windows ✅（dll 进包、sha256 一致、变异实测）；Linux ✅ 已验**（deb 里 `libpdfium.so` ＋ **随包中文字体**都在资源目录那层、sha256 与钉死值一致，且两件都有产物级门禁）；**Android ✅ 通路已落**（CI 上第一次走通，见 §0.3-**P**；**真机验收未做**）；**macOS ✅ 已验（2026-09-22，见 §0.3-Q）**：签名链（`scripts/sign-macos-app.mjs`，由内到外；签前 `--deep --strict` ❌ ⇒ 签后 exit=0 ✅）＋ **包级功能读数**（用包里那份已签名库跑对拍：1 passed/0 failed、`cjk.pdf` 墨迹 29582 与开发期逐字相同）；⚠️ 公证与 GUI 人工那半仍属**人手/owner 凭据**，且 dmg 里那份是**签名前**的拷贝（顺序问题已记 §0.3-Q）；⚠️ 中文那类文件的**六格读数已齐**（§0.3-O）；新增一条 **P4 拦路石**：**Linux 那份 `libpdfium.so` 没有 fontconfig 字体后端** ⇒ 非嵌入字体（含国标中文）在 Linux 上**整行不显示**，且"装系统字体"治不了 —— 施工单见信箱 `2026-09-20-pdfium-linux-font-backend-workorder`（已按路线 D 落地：随包 OFL 中文字体；读数见 §0.3-**P**） |
