...
| 编号 | 项 | 优先级 | 依赖 | 说明 |
|------|----|--------|------|------|
| E1 | ✅ **文件级 vault 加密**：口令 → Argon2id → 密钥 → 加密工作空间 DB + 附件；打开解密 / 锁定加密；默认关 | ✅ P1 | crypto（✅）+ 存储层 | §5.4 方案1；上层透明（SQLCipher 空间库 + 启动锁定/解锁门控 + 附件加密 + 双向迁移，v1.64.16；见 e1-implementation-notes.md） |
