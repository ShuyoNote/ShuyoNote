...
- ✅ **本地静置加密 E1 + 解锁 UX E2**（`security.rs` SQLCipher 空间库 + 附件加密；`src/lib/vault.ts` 状态中枢 + `LockScreen`）。
  ⚠️ **2026-09-25 更正（本行原来写的是应用级模型，那套已经整条删掉）**：
  今天加密是**按空间**的（`space_crypto.rs` ＋ 钥匙袋 `keyring.rs`：主口令 → 主密钥 → **每空间一个盒子**），
  「服务端那份同步内容用的也是同一把**会话密钥**（`security::key_if_enabled`）」这句**不再成立** ——
  应用级加密（全局一把钥匙、含解锁哨兵与读老库的兜底）已按 owner 第三轮拍板**整套删除**
  （见 [交接稿](plans/2026-09-24-crdt-privacy-handoff.md) §7.0.7）。
  **已知边界（仍然成立、更准确的说法）**：**口令是唯一钥匙 ⇒ 忘记口令，那一个空间的数据就打不开**
  （本机与云端一起），唯一救法是**开启加密之前**导出的明文备份 —— 这条已写进锁定屏与设置页。
...
| E2 | ✅ **解锁/锁定 UX + 忘记口令提醒**（口令即密钥，别无副本） | ✅ P1 | E1 | 状态中枢 `src/lib/vault.ts` + `useVault`（锁定立刻切屏、不再有三份状态副本）；**并修掉盖住它的那个必崩路径**——闸门原是 `App` 里排在 hooks 之前的早退，加密安装重启即 `Rendered fewer hooks than expected` ⇒ 崩溃屏，**锁定屏从没出现过**；现闸门与外壳拆成两个组件（锁定态外壳不挂载）。锁定屏连错 3 次自动摊开「忘记口令？」，照代码写实：没有找回流程、服务器那份也打不开、唯一出路是加密**之前**的明文备份。<br>★ **「开启加密前必须勾选『我已保管好口令』」2026-09-25 落地**（owner 拍板 A2）：`SpacePrivacySection.tsx` 的 `PASSPHRASE_NO_RECOVERY` 常量 ＋ 那个勾选框 —— **没勾上时「开启加密」是灰的**（不是弹窗），勾选按空间记；判据 `SpacePrivacySection.test.ts` 两条（没勾 ⇒ 点不动且真话在屏幕上／勾上才点得动、且按空间记）。<br>★ **真机复验（2026-09-25）**：Mate 40（Android 12）验过勾选框与灰按钮；MIX 2（Android 9 / WebView 80）验过这一节渲染正常 ＋ 没钥匙袋时的报错可操作。<br>判据 `src/vaultGate.test.ts`（7）+ `src/components/lockScreen.test.ts`（6），均做过变异验证 |
