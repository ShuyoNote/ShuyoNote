...
> **服务器地址由你在「同步」面板里填**（客户端**不内置默认地址**）；官方实例：
> **`https://sync.shuyo.cn`**（2026-09-22 起，API 挂在根路径）或 `https://shuyo.cn/sync`（老写法，同样有效），
> 也可以指向你自己部署的实例 —— 服务端的部署 / 配置 / 排错见私有仓库 `shuyonote-sync-server` 的 `docs/deploy.md`。
> ⚠️ 客户端**不会**替你补 `/sync` 后缀（只去掉末尾斜杠），**填什么就请求什么** ⇒ 见 `docs/deploy.md` §5。
>
...
> 而 `https://shuyo.cn/sync/health` 返回 **200**。**请一律使用域名形式**（2026-09-22 起推荐 `https://sync.shuyo.cn`，
> 它有一条独立的 A 记录与证书，主域换主机/上 CDN 时不受影响）。
