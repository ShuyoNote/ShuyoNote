...
| [plans/2026-08-29-pdf-continuous-scroll-plan.md](plans/2026-08-29-pdf-continuous-scroll-plan.md) | **「PDF 连续滚动（虚拟化）」落地文档（方案 B / 已实现，待发布）**：把阅读器从「单页翻页」升级为「整篇纵向连续滚动」——所有页块纵向堆叠，一次可自由滚过整篇；只挂载视口 ± 1 页缓冲的页块（其余页占位不渲染，内存可控）。**批注随页块**（每页仍是自包含 `PdfAnnotationCanvas`，工具条/撤销/选中/批注都在块内）；导航升级（侧栏/目录跳页滚到目标页顶；←/→/↑/↓ 逐页滚动取代「滚动边缘自动翻页」；F 适配页宽）；布局数学抽成纯函数 `pdfLayout.ts`（前缀和 + 视口挂载范围），首屏预取全部页尺寸使滚动轴稳定；修复暗色下透明底 PDF 不可读。**v1.59.187 已落地**；**v1.59.188** 增量：修复缩放迟钝/抖动（页块宽随 `scale` 真实放大 + 缩放后重光栅化）+ 缩放下拉（预设百分比/适配页宽）+ 点击 PDF 附件/文件树节点直达阅读器 |
...
- [CHANGELOG.md](../CHANGELOG.md) —— **版本变更日志**（Keep a Changelog 格式，`v1.6.0` 起，当前 `v1.59.188`）。
