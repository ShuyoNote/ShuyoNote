...
| [plans/2026-08-24-columns-plan.md](plans/2026-08-24-columns-plan.md) | **「分栏」功能方案（飞书式 Columns Block）**：N 列并排/每列独立输入 + 选择栏数（2/3/4）面板；对比两条路线（ElementNode 单编辑器 vs DecoratorNode+嵌套编辑器），**轻量版已落地 v1.59.168**（`ColumnsNode`/`ColumnNode`/`ColumnsPickerPlugin`），完整版（每列子编辑器）列为候选；含数据结构、入口、样式、验收与边界 |
