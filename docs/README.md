...
| **M11** | 插件 | [x]（L1；**M11.5/M11.6/M11.7 已落地**——时限与资源上限、ABI v1 + 能力注册表 + 权限与写中介、**当时 20 条**能力 + 与 AI 工具层合并（现为 **25 条**，以注册表为准），含**作者工具链**：应用内校验/热重载/`pnpm plugin:validate`/[示例插件](../examples/plugins/README.md)；**M11.8 触发面与事件、M11.9 五档（声明式视图 / 主题 token / 视图参数化 / 导入触发 / 导出）亦已落地**） | [插件方案](plans/2026-08-22-plugin-plan.md)（L1 已落地）+ [插件体系进化方案](plans/2026-09-10-plugin-evolution-plan.md)（M11.5–M11.13，规划；**定位=做第一不做更大：第一个「有权限模型 + 作用在 E2EE 数据上」的可信插件体系**，并给出**分轴预期刻度**（哪几轴对齐/超出、哪几轴低于顶级、哪一条结构性不可达）；原 M11.3 UI 型 → M11.10、原 M11.4 市场 → M11.11，均带启动闸门；新增 M11.13 隔离强度（**已判定必有**；内存炸弹由 M11.5 的"超预算 panic 限流分配器"闭合，进程级隔离为 M11.11 分发硬前置）；**分发形态见[插件分发策略](plans/2026-09-10-plugin-distribution-strategy.md)**（协议而非平台 + 贡献阶梯 + M11.11a/b/c） |
...
| **M27** | **团队版（自建协作）** | 规划 | 方案**已移入私有仓库** `shuyonote-sync-server`（`docs/plans/2026-08-30-team-edition-plan.md`）；本仓库只留 M27 的结论 |
...
| [plans/2026-08-30-team-edition-account-space-plan.md](plans/2026-08-30-team-edition-account-space-plan.md) | **团队版 M27.1 · 客户端（账号 / 空间绑定）实现方案**（[x] 客户端核心已落地 2026-09-01）：`team_*` 命令（register · login · logout · list/create space · list/invite/set-role/remove member）＋ SyncPanel 登录 / 注册 / 空间绑定 / 成员管理 UI；**仅桌面**（Web driver 一律抛「Web 版不支持团队版」）。⚠️ 文内自注两处已过时。姊妹篇《团队版总方案》**暂未公开** |
...
| [plans/2026-09-16-sm-crypto-and-pdfium-plan.md](plans/2026-09-16-sm-crypto-and-pdfium-plan.md) | **「国密（SM 系列）支持 + PDFium 替换 MuPDF」方案（待拍板，未实装）**：面向有国密/许可合规要求的政企交付。**两个前置**决定是否开工——① 客户要哪一档国密（档 1 密钥管理 ≈3 人日 / 档 2 外发数据 ≈3–5 人日 / 档 3 全链路含库级密文 ≈2–4 周，因 **SQLCipher 不支持 SM4**）；② 要的是"用国密算法"还是"**通过认证的密码产品**"（后者自研不满足）。**关键发现**：密文格式**没有版本号**（`crypto.rs:28,61` 的裸 `nonce‖ct`）⇒ 换算法前必须先做格式版本化，否则老数据无法双读；**附件内容寻址的 SHA-256 不能换**（`attachments.rs:291`，换了等于全库改名 + 同步标识失效）；Web 版无静置加密（`web.ts:2916`）⇒ 国密只需覆盖桌面 + 导出/同步载荷。PDFium 侧：**只在做闭源商业授权版时才换**（客户端已是 AGPL-3.0，与 MuPDF 同系、本来就兼容），且 native 仅做光栅化 ⇒ **不碰坐标、不碰前端契约**，≈3–5 人日 + 多平台打包。含分工（文件零重叠，`Cargo.toml` 串行）、验收、风险与待拍板 5 项 |
| [plans/2026-09-16-sm-crypto-full-plan.md](plans/2026-09-16-sm-crypto-full-plan.md) | **「国密全链路」落地方案（档 3 ＋ 国密传输，已拍板）**：四层全换（口令派生 / 应用层 AEAD / **库级静置密文** / 传输层）。**库级是核心难点**——已核实 SQLCipher 的 `cipher_hmac_algorithm`、`cipher_kdf_algorithm` 可 PRAGMA 切，但**没有 `cipher_algorithm`**（`libsqlite3-sys-0.38.2/sqlcipher/sqlite3.c` L112162-112364）⇒ **页加密算法由编译期 provider 决定、运行期切不了**，档 3 必须新增 provider；推荐 **SQLCipher ＋ Tongsuo（OpenSSL 分支，原生带 SM2/SM3/SM4）＋ 薄 provider**（与现有 vendored-openssl 构建链同源）。传输层给出三条现实路径（**甲方国密网关 / 应用层端到端国密 / 真做 GM/T 0024**）并说明 Rust 生态基本无成熟国密 TLS。含 P0–P5 任务表（**P0 密文格式版本化 1 人日、与档位无关，建议立刻做**）、验收、风险、3 项待确认 |
| [plans/2026-09-16-pdfium-engine-plan.md](plans/2026-09-16-pdfium-engine-plan.md) | **「PDFium 替换 MuPDF（桌面光栅化）」落地方案（已开工）**：**Spike 已通过**——`pdfium-render v0.9.4` 在 `x86_64-pc-windows-msvc` 编译成功（13.5s），它用**预生成绑定 ＋ `libloading`**、**构建期不跑 bindgen** ⇒ 当年 `mupdf` 高层 crate 的 MSVC `max_align_t` 坑不会重演；crate 把支持的 PDFium 钉在 **build 7881**（`pdfium_7881` feature）。范围**只换光栅化**（`PdfReader.tsx:122` 桌面取页位图；坐标/文本层仍走 pdf.js）⇒ **不碰坐标、不碰前端契约**，`parseNativePageResponse` 零改动。含 P0–P5 任务表（**P0 拿到并固定 `pdfium.dll` 7881 + 校验和**）、实现要点（BGRA→RGBA 本侧转换、缓冲自管⇒stride 问题消失、全局 init+锁 1:1 搬迁、缓冲区生命周期与 MuPDF 同一个坑）、验收、供应链与风险 |
| [plans/2026-09-17-sm-crypto-tradeoff.md](plans/2026-09-17-sm-crypto-tradeoff.md) | **「国密方案 vs 原方案」利弊与跨平台影响（决策补充）**：给「换国密」这笔账一个可引用的对比。**利**=合规准入（等保/密评门槛，**唯一硬收益**）＋叙事差异化＋Tongsuo 与现有 vendored-openssl **同源**（是多一条构建路径 vs 不是，这条削掉一部分弊）；**弊**=① **KDF 降级**（Argon2id 内存硬化 → PBKDF2-HMAC-SM3，**国密无内存硬化对应物**）② **nonce 24B → 12B**、SM4 密钥 **128 位**（管理更严）③ **自维护 provider 是长期责任**（不是一次性成本）④ 若甲方改口要「认证产品」则自研全白做 ⑤ 软件 SM4 无 AES-NI 级加速、热路径需实测。**最该谈的**=**档 2（≈3–5 人日）vs 档 3（≈8–13 人日）**——档 3 的增量只有一句口径、代价却是最贵的 P3。**跨平台**：构建面**四条链各编 Tongsuo**（Windows MSVC 与 Android NDK 的 **Perl** 是硬坑；桌面自己挑加密后端（**macOS 实际编译成 CommonCrypto，而它只有 AES**）、只有 Android 用 vendored，`Cargo.toml:25` vs `:111`）；数据面**不存在「部分设备升级」**（解密端必须会 SM4，而协议里**没有任何算法标识**，`security.rs:473-476`）；**路径 3（TLCP）会额外伤跨平台**（Android `tls_android.rs` 的 `rustls-platform-verifier` 要重做、服务端换 Tongsuo Nginx）⇒ **已定（2026-09-17）：先走路径 2、路径 3 暂缓**。含最小风险四条做法（算法随空间记录 / 老端明确拒绝 / 保留双读 / 默认关按空间开）与归属决定（**已定 2026-09-17：国密代码进开源主干**——CLA 支持双许可、Tongsuo 为 Apache-2.0 故许可面不变；连带**主干要有常开国密 CI job**、Tongsuo 按 `fetch-pdfium.mjs` 套路钉版本＋校验和、**默认算法仍 v1**；**构建门控已定：feature 门控 ＋ 一条常开国密 CI job**） |
...
| [plans/2026-09-15-pages-tag-deploy-plan.md](plans/2026-09-15-pages-tag-deploy-plan.md) | **GitHub Pages 改为随 tag 发布 + 两个 Web 入口「构建身份」对齐（规划，未实施）**：起因是 **2026-09-15 发 v1.91.2 时的实测**——线上两个 Web 入口**都在跑 tag 之后 9 个提交的未发布代码**（含未发布的 `feat(sync)` K2），而 `pnpm check:web-deploy` **全绿**。判据全部实测：Pages 最近部署 `f173052d`（#95）、`main` 领先 tag 9 个提交、**两个入口都含 tag 里不存在的 `issue-device-key`**（tag 侧 `git grep` = 0）、两入口 26 个文件逐字节相同（**是巧合不是保证**：主站手动部署戳 16:06:50 晚于 K2 落地 16:01:03）。**根因三层**：① 入口绑 `main` 而非绑发布（`pages.yml:6-9`）；② 产物只带版本号、**无构建身份**（`version.json` 只有 `{"version"}`）⇒ `check:web-deploy` 只能比版本字符串，**跑未发布代码时必然通过**；③ 手动入口构建的是**工作区**而非某个 ref（多会话并发是常态）。**推荐 A+C**：A=Pages 只在打 `v*` tag 时部署（环境策略加 `{name:"v*",type:"tag"}`、最后删 `main` 策略；实测 `build_type:"workflow"` 故支持 tag 策略）；C=给 `version.json` 加 `commit`/`builtAt` 并把 `check:web-deploy` 升级为**身份比对**（含**部署侧全量清单**以覆盖懒加载分块，tag 不存在时**计入失败而非静默跳过**）+ 手动部署前的**一行断言**。含 A-1～A-4 / C-1～C-4 逐步判据、10 条验收、边界（不改 `ci.yml`／不做 main 预览站／不动环境策略其它设置）、**待授权项**（改环境分支策略是安全面变化，按 `RELEASING.md:302-303` 须显式授权）；另记录遗留观察：`pages/builds` API 在 `build_type: workflow` 下**返回 404/空是正常的**，查 Pages 部署史须走 Actions runs |
| [plans/2026-09-15-attachment-sync-scope-plan.md](plans/2026-09-15-attachment-sync-scope-plan.md) | **附件同步「范围与刹车」方案（手机端 × 海量视频空间，规划，未实施）**：起因是一句追问——"手机被设置和**海量视频空间**同步，结果如何？"。**代码级取证结论：当前附件同步的语义 = 把整个空间在设备上做一份完整副本**（`sync.rs:1496` 的双向全集差集：`:1529` 拉远端全部哈希 → `:1572` 全部上传、`:1631-1690` 全部下载），**不是"按需取用"**，且 `docs/SYNC.md:34` 已把它写成文档 ⇒ **有意设计**；而这条语义**默认了桌面环境**（盘大 / 常插电 / 内存宽裕 / 用户坐在机器前），换到手机上**同时缺三样刹车**：**无大小与磁盘余量检查**（`:1631-1690` 循环体只有哈希校验与"本地已有则跳过"）、**无网络闸门**（`useAutoSync.ts:5,37,26` 间隔 5 分钟 + 启动 3 秒 + `App.tsx:618` 无条件调用）、**无"未下载"状态**。三条真正会伤用户的：① 填满设备（**同分区的 SQLite 也受牵连**）② **加密开启时每个文件下完整块读进内存**（`:1670-1676`）⇒ 手机上很可能**先 OOM 而非先满盘**，且 `:1657-1666` 存在**明文落盘窗口** ③ 后台静默拉视频。**外加一个地基问题**：`useSyncStatus` 的 `end()` **只在 Web 侧被调用**（`web.ts:2555,2558`），而 `SyncPanel.tsx:214` 调了 `begin()` 却**全仓无配对 `end()`** ⇒ 桌面/安卓手动同步后永远停在"正在同步…"；且**附件进度只在 Web 上报**（`web.ts:920,962`；`src-tauri/src` 零命中）⇒ 用户完全看不见。**方案分档**：**P0 修文档口径**（三处说"Web 不支持同步"与代码相反——`MOBILE.md:18` / `App.css:1221` / 社区仓核对表 #40，而 `web.ts:2429,2541,896` **确有实现**，`SyncPanel.tsx:502` 那句才准）、**P1 补结束态 + 进度**（**照抄现成范式**：`workspace_io.rs:36-49` 的 `emit(phase,done,total,bytes,message)`、`attachments.rs:539-549` 的进度回调；⚠️ `useSyncStream.ts` 是 **Web 专属 SSE**、不是桌面通道）、**P2 边流边加密**（**须先确认 `security.rs` 的格式**，否则不做"看起来流式"的假优化）、**P3 预算刹车**（余量下限 + 本次总量上限，**优雅停止并报告**，按内容寻址故**幂等可续**；顺带修 `:1660` 与 `:1646` 的错误处理不对称）、**P4 大附件不自动下载 + 「未下载」状态**（第二批；动工前须答清读路径 / 孤儿清理等四个问题）、**P5 网络闸门**（`src-tauri` 全仓**无网络类型能力**，需新依赖，可行性未评估）、**P6 按需取字节 / 每空间开关**（待拍板）。含 10 条验收、边界（**不改"会拉附件"的语义**、**不动服务端 2 TB 默认上限**）、**6 条未验证清单**（首先声明"未在真机跑过大文件同步"）、5 项待拍板；另记两条边界：服务端请求体上限**默认 2 TB**（私有仓 `main.rs:68-70`）、客户端"legacy 全局附件"分支**实际不可达**（`sync.rs:1508-1512` 被 `:1376`/`:1420` 挡住）。**2026-09-15 补**：① **修正「四条路径同病」这个说法**——只有**导出 / 另存为是无条件**的整块读（`attachments.rs:378`，与是否加密无关），导入 / 上传 / 下载**只在用户开了静态加密时**才整块读（§2.6（2）已加"触发条件"列）⇒ **默认配置下的主要风险是"填满设备"，不是 OOM**；② **新增 §4.1 执行批次**：**第一批现在就做**＝B1 口径修正 / B2 补 `end()` / **B3＝P2a** 导出改真流式 / B4 先实测重复行（**四件都不改同步语义**），**第二批（Android 上架前必做）**＝C1 预算刹车 + C2 网络闸门；**P4 建议不做**（用「每空间开关」替代）、**P6 下一轮**、**P2b 先真机实测再决定动不动加密格式**；③ 待拍板**从 7 条收敛为 2 条** |
| [plans/2026-09-15-attachment-on-demand-plan.md](plans/2026-09-15-attachment-on-demand-plan.md) | **附件同步「按需取字节」立项（P6）+ 第一步「每空间开关」施工单（已立项，未实施）**：2026-09-15 发布者裁定立项，按上游方案 §九 的建议**先只做"每空间开关"那一半**（成本低、立刻可用、不引入"未下载"状态）。**立项时核实到一个把范围显著收窄的事实**：`attachments.rs:434-436` 的注释原文写着「找不到文件时说清是『**文件不在盘上**』——这一条最常见的成因是外部把文件删了/移走了，**而数据库里那行还在**」⇒ **「行有、字节没有」已经是代码预期的状态**，所以"只同步元数据、字节按需" **= 把 `sync_attachments` 的第 3/4 步（上传与下载）关掉**，**不需要新数据库状态、不需要数据模型改动、不需要协议改动**（元数据经 `changes` 的 `attachment` 实体同步——写入 `attachments.rs:587-588`、应用 `sync.rs:1297-1313`；字节只由 `sync_attachments`（`sync.rs:1531-1690`）管）。**阶段**：P6.1 每空间开关（本次交付）→ P6.2「未下载」UX → P6.3 按需取字节（**把下载循环抽成 `download_one_attachment(hash)` 复用，不许复制第二份实现**）→ P6.4 阈值/白名单。**施工单已精确到文件**：`db.rs:324-332` 建表 + 迁移（照 `:492-499` 的 `pragma_table_info` 守卫写法）、`sync.rs:319-330` 结构体与 `PROFILE_COLS`（**必须追加在最后**，否则 `row_to_profile` 下标全要改）、⚠️ **`:403-411` 的 `set_profile_field` 把 `field` 格式化进 SQL**（今天靠注释声明"只传常量"⇒ 加字段必须同步纳入白名单，建议改成显式 allowlist）、`sync.rs:1496+` 早退、`commands.ts:69/353`、`web.ts:646/653/2540/896` 同构、`SyncPanel.tsx` 开关、`sync-regression.mjs` 断言。**6 条待拍板的设计决定**，关键两条：**只控字节不控元数据**（元数据照同步 ⇒ 对端看得见但打不开，比"文件消失"更可解释）、**关掉后不上传且可见地提示"未上传 N 个"**（静默不上传会让人以为已同步）。明确：**P4 被本立项替代**、与 C1 预算刹车**互补不替代**、**P6.1 不构成 Android 上架门槛**（C1 才是）。含 7 条验收、边界（**不删**已下载字节／**不引入**"未下载"状态／不改服务端／不复制下载实现）与动手前必须先验 3 条——其中**第 1 条「除 `sync_attachments` 外还有谁在拉附件字节」已于 2026-09-15 排查完毕，答案是"没有"**：三步穷举（Rust `/attachments` URL 构造点 5 处、`reqwest::Client::new()` 53 处、`blobStore.put` 9 处逐个归类；`attachmentByteDownload` 只有 1 个调用点）⇒ **客户端取字节只有 `sync_attachments` / `syncAttachments` 一条路**，且 SSE 通道只是触发同步、不携带字节 ⇒ **开关只需在这两处早退即可"关干净"**。⚠️ **前置依赖：上游第一批的 B2（补 `useSyncStatus.end()`）必须先做**——P6.1 要求"中途关掉开关 ⇒ 面板报告*因开关关闭而停止*"，而**没有结束态就没有地方显示停止原因**（现状是永远停在"正在同步…"，与 B2 是同一个缺口）⇒ **顺序 B2 → P6.1，不能并行**；为此立项书 §四 专设了**步骤 0**、§八 关系表也记了这条 |
| [plans/2026-09-17-knowledge-base-ai-coverage-plan.md](plans/2026-09-17-knowledge-base-ai-coverage-plan.md) | **「全库 AI 覆盖」方案（P1 大部分 + P2 前半已在 `dev` 落地；**进度总账见方案 §8.0**）**：起因是用户提问「**如何把存入 ShuyoNote 的一切**（页面附件、绘图块、公式、文件夹里的各类文档/图片/视频）都纳入 AI 分析」。**诊断结论：AI 与全文检索唯一能看到的字段是 `content_text`，而它是"页面正文的纯文本镜像"**（为搜索/反链/导出设计，不是为多模态设计）⇒ 文字类基本已覆盖，**图片 / 视频 / 附件文档 / 数据库块完全看不见**——恰好是要补的那一半。**逐块取证**：公式 ✅（`FormulaNode.tsx:3` LaTeX 源码入库）、Mermaid ✅（`MermaidNode.tsx:69`）、绘图 ⚠️**只有图上文字标签**（`DrawingNode.tsx:125`「scene text … its labels」）、页面链接 ✅ 字面 `[[标题]]`、PDF ⚠️只有「摘录 + `pdf://` 引用串」（`pdfAnnotation.ts:170`）、**图片/视频/附件引用 ❌**（三者皆 `DecoratorNode` 且**未覆写 `getTextContent`** ⇒ 默认空）、**数据库块 ❌**（`DatabaseView.tsx:842` 直接写 `content_text: ""`）；**应用自己的自述**见 `App.tsx:93`（"a page with only an image/embed/table has empty `content_text` but does contain content"）。**破局点＝不为每种模态新增 AI 工具**（现有 7 个，会爆炸；本地小模型在工具选择上不可靠），**而是把多模态降维成文本、汇进同一层可检索面**，让现有「搜索→读取→总结」自动覆盖。**三条可复用资产**：**VLM 通道已存在**（`src/lib/ai/ocrVision.ts`，其注释原文即"对中文/复杂/低清扫描件远优于 tesseract"）、pdfium/mupdf 文本层、tesseract 离线兜底。**最要紧的一步是存储决策**：派生文本进**独立 sidecar 表**（`attachment_text` / `chunks` / `chunk_embeddings`，含 DDL），**不回写 `content_text`**（否则污染正文与 FTS、并让 6000 字截断更早发生），且**只读、可重建、不进同步/备份/导出**（对齐「能重建的绝不是事实源」）。**索引层四处必改**：`EMBED_TEXT_CAP = 500`（`semanticEmbed.ts:144` / `search.rs` 同值）⇒ **分块嵌入**；`pages.get` **硬截 6000 字**且无翻页（`capabilities/frontend.ts:69`）⇒ 支持 `offset`/`limit`；检索**页级 ⇒ 块级**（附 `loc` 回链）；新增 `files.read` / `files.search`（走 `capabilities.json` 的 `ai:true` ＋ `check-capabilities.mjs` 门禁）。**分步**：**P1 抽取 + 派生表 + 接进现有检索与工具**（一次性补上缺失的一半，**不改 AI 工具集形态**，投产比最高）→ **P2 分块 + 块级嵌入 + 混合检索**（**地基**：现"每页只索引前 500 字"等于只索引封面，50 页制度靠语义找不到第 37 条）→ **P3 视频/音频/数据库块/绘图结构** → **P4 map-reduce + 强制引用**；**顺序不可颠倒**（跳过 P1/P2 直接做 P4 只会产出没有依据的漂亮话）。含抽取矩阵（**图片必须分两档**：全量 OCR 建索引 ＋ VLM 只跑候选/关键图，否则成本失控）、利弊诚实标注（**本机 6GB 显存放不下「文本模型＋嵌入＋VLM」三件常驻 ⇒ 抽取必须排队错峰，属架构约束而非可优化项**；抽取必然有损 ⇒ 必须带引用，并声明"以原件为准"）、红线（**不新增任意文件系统访问**——只抽**已导入的附件**，**不遍历用户磁盘**；外部网盘索引属另一方向须另起方案）、9 条验收、**§12 分工与协作纪律（三台机器）**：**Windows 跑不了 `cargo test`**（`0xc0000139`，出处 `sm-crypto-full-plan.md:294`）⇒ **Rust 侧改动必须由 AMD 或 Mac 复核，否则 P1 无法闭环验收（硬需求，不是"人多力量大"）**；可并行只有**两条轴**——**格式抽取器分家**（纯函数零重叠，但**接口必须先冻结**，否则三份实现合不起来）与**平台同构**（`check-web-commands.mjs` 三向门禁兜底，`files.read`/`files.search` **必须写两遍**）；**集成 + 检索链路（`db.rs`/`search.rs`/`web.ts`/`capabilities.json`）是唯一串行点、一间独占**；**P2 不分家**（耦合最紧的地基，等 P1 合了再议）；纪律抄既有两条——**禁止 `git add -A`**（并发会话，**曾误提交他人 WIP**）、**"编过了"≠"测过了"**、分家后**三份实现共用同一组夹具**。6 项待拍板（新增第 6 项：**三台机器 GPU/显存规格**——决定"全库抽取"这项算力劳动放哪台跑，属部署决策而非分工偏好） |
| [plans/2026-09-17-ai-coverage-handoff-patches.md](plans/2026-09-17-ai-coverage-handoff-patches.md) | **「全库 AI 覆盖」被"本机不能自验"卡住的三项 · 可套用补丁草案（交接件）**：上一条方案的配套件。Windows 跑不了 `cargo test`，于是 `pages.get` 分页 / `files.read`+`files.search` / `db.rs` 派生表这三项**不该由它做**；此前每次都要重新解释一遍"为什么绕开"，说明缺的是**交接件**而不是决心。含**三条实测事实**（① 注册表 22/22 条能力**全部**声明 `"rust": "cap_xxx"` 实现函数 ⇒ 加能力不可能纯 TS（⚠️ **2026-09-23 起有例外**：`coverage.report` 刻意 `host: "frontend"`、没有 Rust 实现 —— 因为它要读 TS 侧的抽取器注册表，两条实现会漂，见方案 §15.11）；② **新增一条能力要动 6 处**，门禁原文附在文件里；③ 给既有能力**加参数**只让 4 个 TS/文档生成物变化、**`capabilities_gen.rs` 不在其中**，但门禁仍报"声明了参数 offset，但 dispatch 里根本没读它" ⇒ **仍要改 Rust**；那个 dispatch 就是 `plugins.rs` 的 `let out = match cap.id {`，锚点在 `check-capabilities.mjs:162`）、四份草案（`db.rs` 前置 → `files.read`/`files.search`；`pages.get` 分页可独立）、以及一张**"我验证到什么程度 / 不能验什么"**的表（Rust 能否编过：不能；`plugins.rs` 里 `arg_*` 的准确名：只从门禁脚本注释得知）。核心口径：`files.read` **必须带覆盖度**，否则"我没抽到"会被读成"文件里没有"——与 §15.10 防的是同一件事 |
| [plans/2026-09-08-attachment-hash-bucket.md](plans/2026-09-08-attachment-hash-bucket.md) | **附件哈希前缀分桶**：附件存储从「单目录平铺 `attachments/<hash>.<ext>`」改为「按 sha256 前 2 字符分桶」，避免单目录文件过多带来的目录性能问题 |
| [plans/2026-09-09-sync-seq-lww.md](plans/2026-09-09-sync-seq-lww.md) | **同步 LWW 改为服务端 `seq` 基准（根治时钟漂移）**：把 LWW 键从「各设备本地时钟 `updated_at`」改成「服务端单调 `seq`」，时钟漂移不再导致整页覆盖丢改动 |
| [plans/2026-09-11-app-community-interactions.md](plans/2026-09-11-app-community-interactions.md) | **ShuyoNote × 数友社区：互动方案**（优先级 + 分阶段）——**方案，未实施**，本轮只有文档、无代码改动 |
| [plans/2026-09-17-three-track-progress.md](plans/2026-09-17-three-track-progress.md) | **三线整体进展（国密 / PDF / AI）**：**进度口径**（不是方案、不改任何设计），用途是让 Windows / AMD / Mac 三方看到同一份现状 |
| [plans/2026-09-18-crdt-block-id-ownership.md](plans/2026-09-18-crdt-block-id-ownership.md) | **块 ID 归属（已拍：方案 (a)）**：`blockId` 改成**声明的节点属性**；实测「同 type 子类化内建节点」在 Lexical 0.50 抛错 ⇒ 采用**新 type 自有节点**，并用「内存/CRDT 用新形态、落盘/同步用老形态」的双形态保住旧版本兼容；含四步分步计划与**接线点清单** |
...
| [plans/2026-09-19-crdt-spike-conclusions.md](plans/2026-09-19-crdt-spike-conclusions.md) | **CRDT 尖刺结论归档**（原 `spike/crdt/README.md`，**分支 ref 已删**）：问题一（`content_json` ⇄ `ydoc` 保真）＋**两条前置阻碍**（`blockId` 会被 CRDT 绑定丢掉；任何不在节点 `exportJSON` 里的字段都会丢）＋阻碍 A 的候选解法实测；问题二（正文有**两条派生实现**，7 样本 4 个不同）与 Markdown 往返等价（7/7）；问题三（E2EE 下服务端可继续"哑且盲"，13 通过 / 0 失败）；含**覆盖边界**与"没测什么" ＋ 未提交探针改动的附录 |
| [plans/2026-09-23-crdt-slice-b-workorder.md](plans/2026-09-23-crdt-slice-b-workorder.md) | **Slice B 施工单（阶段 2 第二片）**：让**保存/加载**能选走 ydoc。核心不是写代码，而是先拍三条口径 —— ①**混版本降级**（老客户端只写 JSON、新客户端写 ydoc；倾向"落盘仍是 JSON、ydoc 只当内存/传输平面"，与阶段 1 已跑通的两形态同一手法）②**空页那段的 id 由谁铸**（倾向保存路径先铸、转换层继续**不造身份**）③**派生索引在 ydoc 平面下何时重建**（阶段 1 的补算器是过渡量）。5 条判据里**开关关闭 ⇒ 逐字节等价**是承重的一条（**已全绿**：`src/lib/crdt/plane.path.test.ts` 9 条，④ 用产品自己的 `topLevelBlockIds` 读引用完整性、⑤ 派生口径如实降级成"落后有痕＋能收口"；另补第 6 条实测撞出的**存量前提**：未补种块身份的页面会让平面开着时**炸在读路径**）；边界与 Slice A 相同：不碰服务端/wire/插件契约，`yjs` 不升格 |
| [plans/2026-09-23-crdt-plane-boundary-decision.md](plans/2026-09-23-crdt-plane-boundary-decision.md) | **决策简报：CRDT 平面住在哪条边界**（只列决策，不写码）。撞出来的张力：Slice B 把平面钉在**磁盘边界**（`docContent` 两读一写，`docContent.ts:58/83/100`），而 §0.5 口径 1 说"落盘仍是 JSON、ydoc 只当内存/传输平面" —— 磁盘边界上**只有一个版本可过** ⇒ 平面**合并不了任何东西**，开着时的净效果只是把已落盘 JSON **归一化一次**；且**桌面 Rust 侧根本没接**（那一层的唯一生产 importer 是 `platform/web.ts`）⇒ 开关只改 Web、**安装包一行不变**，与"两份实现语义必须一致、判据成对"的纪律正面冲突。四个选项（客户端 wire 边界 / **服务端合并（推荐）** / 每页持久化 ydoc 状态 / 维持现状）各列改动面·判据·成本·对已发布版本的影响；**结论：只有"持久化 ydoc 状态"才需要全库补种块身份** ⇒ **先拍边界，再决定要不要迁移数据** |
| [plans/2026-09-23-crdt-full-launch-sprint.md](plans/2026-09-23-crdt-full-launch-sprint.md) | **CRDT 全上线冲刺（阶段 2 全量）**：owner 口径「**不考虑向后兼容，冲刺 CRDT 全上线**」（2026-09-23 口述；不含"没有正式客户"那个前提）。作废/放宽的旧约束写在 §0（含口径 1「落盘仍是 JSON」**作废**）。★ **S1 实测红线**：同一份 JSON **各自新建**的两份 CRDT 状态合起来 ⇒ 顶层块变 `["paragraph#blk-1","paragraph#blk-1"]`（**一块变两块且 `blockId` 重复**）⇒「每次从 JSON 新建 ydoc」这条路**不可合、不许当保存形态**；同时证了 Yjs 正例（同一血统两笔更新可合、顺序无关、不重复）。⇒ 架构定死为**每页持久化同一血统的 CRDT 状态 ＋ 服务端合并**。S1✅ → S2 每页状态落盘 → S3 编辑器绑定真 Y.Doc → S4 同步接 update → S5 服务端合并（先只存后开算）→ S6 派生/身份口径 → S7 两侧都接＋真机验收 |
| [plans/2026-09-23-crdt-stage2-kickoff.md](plans/2026-09-23-crdt-stage2-kickoff.md) | **阶段 2（正文 Yjs，路线 C）开工前置清单 ＋ 第一切片**：先**校正状态**——阶段 1 的接线/冲突提示/块级角标都已进 dev，只剩"真机双设备验收"要人手（roadmap/STATE 那句"待收口"是旧的）；再把尖刺 §6 的四条要求逐条对账（已解 2 条／仍差 yjs 路复用 `lexicalStateValid`、混版本降级设计、补算器口径），最后钉第一切片：**只做 `content_json` ⇄ `ydoc` 的唯一实现 ＋ 五类判据**（Node 侧自验、不碰协议/持久化/服务端/插件契约，`yjs` 不升格为生产依赖）与退出条件 |
| [plans/2026-09-23-yrs-interop-spike-conclusions.md](plans/2026-09-23-yrs-interop-spike-conclusions.md) | **yrs 对拍尖刺：结论**（JS `yjs` ↔ Rust `yrs`，冲刺缺口 §10.3-6）。独立尖刺 crate `spikes/yrs-interop`（**不进 app 依赖树**）＋ 尖刺判据（没有那个二进制就 `describe.skipIf` **自报跳过**）。三条读数：① **互操作成立** —— 同一批 fixtures（同血统两分支 → 合并 → 投影）两侧投影**逐字节相同**，且 Rust 认得出真实的根／`__blockId` 属性形状；② **失败形态是"卡死"不是报错**（读事务还活着时再要写事务 ⇒ 不返回，实测挂住 ≥45s／120s）；③ ⚠️ **纠正一条我读错的**：并发共享一个 `Doc` **没有**复现失败 ⇒「每页独立、请求内短命 Doc」仍是最省心的口径。⇒ S5 阶段 2 的成本从"Rust 侧从零写一份 Yjs"降到"接 `yrs` ＋ 写合并/存储" —— **能不能做已回答，值不值得做没回答** |
| [plans/2026-09-23-s5-server-merge-decision.md](plans/2026-09-23-s5-server-merge-decision.md) | **S5 阶段 2「服务端开算」要不要现在做：建议稿**。把"能不能做"（尖刺：格式层可行）与"值不值得做"分开，并给出**体积读数**（状态**线性、约 158 B/块**；200 块 31.6 KB ＜ 同页 JSON 51.1 KB；增量追加一块 **+89 B** ⇒ "存不下/传不动"不成立）。**建议暂缓**，写清四条启动条件（需要权威视图／需要服务端拒绝或收敛第二条血统／客户端覆盖不了的场景／**面 5 E2EE 落地时必须同批设计** —— 服务端要合并就得看得懂内容，与加密存储直接冲突）。含改动面与五条承重判据（幂等／不丢块／降级／并发口径／体积不发散），以及明确**不做**的三件 |
| [plans/2026-09-23-desktop-near-realtime-stream-design.md](plans/2026-09-23-desktop-near-realtime-stream-design.md) | **桌面「近实时」流通道：设计稿（第 48–49 轮已实现，判据 1–6 有本机读数）**。缺口＝桌面没有推送通道（`useSyncStream` 对桌面直接 return ⇒ "近实时"在桌面上等于轮询间隔，而它同时是 **CRDT 合并的可见延迟**）。现状事实清单（服务端 SSE 端点/`KeepAlive`/`Lagged`⇒`ping` 语义、Web 已有消费方、桌面零消费方、桌面已有 `app.emit`↔`listen` 通路）；三条候选形态 ⇒ **选"Rust 订流 ＋ 发事件、拉取仍由前端发起"**（**理由是自动经过 C2 Wi-Fi 闸门/防重入/状态行**）。实现＝`src-tauri/src/sync_stream.rs`（帧解析/退避/`stream_url` 纯函数 ＋ 订阅任务 ＋ 三条命令）＋ `useSyncStream` 桌面分支 ＋ 「近实时推送」开关；含**七条判据**（§5 表：1–6 ✅ 带读数、7 🔶 结构性已判而真行为未验）、五条风险边界（代理缓冲／`Lagged` 不许当心跳忽略）、以及"**拉取仍由前端发起**"这条承重取舍 |
...
| [plans/2026-09-23-lineage-conflict-adjudication.md](plans/2026-09-23-lineage-conflict-adjudication.md) | **血统冲突的「可裁决」那一半：方案稿（第 49 轮已实现"留痕 ＋ 另存为新页"，读数见 §9）**（冲刺 §13.3 第 2 条剩余部分）。先把"合并在结构上做不到"讲死（S1 红线）⇒ 真实选项只有三个：**留本机 / 用对端 / 两个都要（一页变两页）**，且**只有第三个不丢数据**。三条路对比：A 复用 `page_conflicts`（哨兵 `block_id=''`）**❌ 语义污染 ＋ 与"未决数量"那条纪律正面冲突**；B 新表 `page_lineage_conflicts`（页级、本地、可裁决）✅；C 只做"把对端那一版另存成新页"（最小）✅。**实际落地＝B 的骨架 ＋ C 的动作**（owner 选**显式按钮**）：表带**对端投影快照** ＋ 三条命令 ＋ 两处记录点（同一份指纹口径、去重＝同一对指纹只提一次）＋ `LineageConflictBanner` 两个动作。**⚠️ ②「用对端」仍未做**（要动本机血统取舍） |
| [plans/2026-09-23-projection-lag-closure.md](plans/2026-09-23-projection-lag-closure.md) | **投影与派生滞后：方案稿（第 49 轮已按 B2 实现，读数见 §7）**（冲刺 §13.3 第 1 条）。事实：桌面"打开页面"那条端口只写 `page_crdt`（不写 `pages` 那一列、不打「待重建」、不重建块图），而 web 当场合并那条**会**写投影；且**两平面反链实现不同**（桌面是物化表、web 按需扫列）⇒ "只补投影列"在桌面**还不够**。四条路：A 合并后触发一次保存 ❌（`dirty=1` ⇒ 把对端内容当本机改动推回去＝假账）；**B 新增 `write_page_projection` 平台命令（B1 只写投影列／B2 ＋重建块图）✅ 选 B2**；C 下沉进 `save_page_state` ❌（Rust 没有 Yjs，等于 B）；D 只钉边界 ❌。含 8 条承重判据（含"没变就不许写库"与数据库页排除）、5 条风险，以及**如实留着的一条边界**：依赖正文的引用（`[[标题]]`）仍要等补算器 |
| [plans/2026-09-23-encryption-scope-decision.md](plans/2026-09-23-encryption-scope-decision.md) | **加密作用域决策稿：按空间 / 默认不加密 / 同步闸门**（[总口径](sync-server-data-boundary.md) §0.5 的**落法**）。三条必须一起：**① 加密按空间**（今天 `encryption_enabled` 读 meta.db 的**应用级**标志、`encrypt_payload` 不区分空间 ⇒ 一开全都密文、一关全都明文 ⇒ 两个要求没法同时成立）；**② 空间默认不加密**（保住零门槛）；**③ 同步闸门**（个人空间未加密 ⇒ **不许加入同步**；团队空间**免检**）。密钥层级取 **(c) 主口令 → 主密钥 → 每空间随机密钥被包裹**（对比 a/b 的致命短板：N 个口令 / 分享一个空间＝交出全部）。落地顺序 **0 钥匙袋 → 1 按空间 → 2 闸门 → 3 存量一次性迁移（不双读，但失败不许静默）→ 4 Web 口径**；含 5 条风险（附件跨空间去重退化、钥匙袋是离线爆破目标、零知识＝零恢复…）与现状核对（带 `security.rs` 行号） |
| [plans/2026-09-23-keyring-step0-workorder.md](plans/2026-09-23-keyring-step0-workorder.md) | **第 0 步施工单：钥匙袋（keyring）**。公开材料＝盐 ＋ KDF 参数 ＋ 每空间一个盒子（**可以同步/可以放服务端**：不含口令也不含裸密钥）＋ 盒子与空间**绑死**（换槽位当场报错）。**0a 已落地**（`src-tauri/src/keyring.rs`，`pub mod` **不接线**、存量行为零改动；复用 `crypto` 的 AEAD/密文头，随机数收口到 `crypto::random_32`）＋ **5 条判据全绿**（含"记下的 Argon2id 参数必须就是库默认""公开材料里不许出现裸密钥""轮换只动那一个空间"）。还差 **0a-2 参数驱动派生**（动全应用派生路径，单独一片）与 **0b 公开材料可同步**（要动服务端仓）；含与第 1/2 步的接口约定与"本步不做"四条 |
| [plans/2026-09-24-crdt-privacy-handoff.md](plans/2026-09-24-crdt-privacy-handoff.md) | **交接：隐私边界落地进度**（写给下一个会话／睡醒后的 owner）。**到哪**：0a 钥匙袋 ✅ ／ 1a 按空间取钥匙 ✅ ／ 1b-1 wire 按空间 ✅ ／ 1b-2a 按空间启用禁用＋状态 ✅ ／ **第 2 步同步闸门 ✅**（提交号逐条列出 ＋ 当轮读数）。**下一步 1b-2b 动哪四处**（启动闸门改嗅库文件／`encryption_enabled` 按空间／启用禁用的命令面／UI）＋ **五件待 owner 拍或硬前置**（空间的个人/团队**分类从哪来**、KDF 参数取值、0b 要服务端仓、第 3 步迁移、第 4 步 Web 口径）＋ 本会话踩出来的**三条纪律**。⚠️ 并写明一条"没管到"：未分类空间一律放行 ⇒ **"个人空间密文/团队空间明文同时成立"今天还没真正生效**，别写成已做到 |
...
> **团队版方案**（总方案 / M27.1 账号空间绑定 / 近实时协作实现）。
> 判据：**产品 / 工程 / 给用户看的对比**留公开仓，
> **合规 · 法务 · 商业化 · 上线推广 · 运营**进私有仓。
