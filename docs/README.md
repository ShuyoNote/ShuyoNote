...
| [plans/2026-09-23-crdt-full-launch-sprint.md](plans/2026-09-23-crdt-full-launch-sprint.md) | **CRDT 全上线冲刺（阶段 2 全量）**：owner 口径「无正式客户 ⇒ 不考虑向后兼容，大胆干」。作废/放宽的旧约束写在 §0（含口径 1「落盘仍是 JSON」**作废**）。★ **S1 实测红线**：同一份 JSON **各自新建**的两份 CRDT 状态合起来 ⇒ 顶层块变 `["paragraph#blk-1","paragraph#blk-1"]`（**一块变两块且 `blockId` 重复**）⇒「每次从 JSON 新建 ydoc」这条路**不可合、不许当保存形态**；同时证了 Yjs 正例（同一血统两笔更新可合、顺序无关、不重复）。⇒ 架构定死为**每页持久化同一血统的 CRDT 状态 ＋ 服务端合并**。S1✅ → S2 每页状态落盘 → S3 编辑器绑定真 Y.Doc → S4 同步接 update → S5 服务端合并（先只存后开算）→ S6 派生/身份口径 → S7 两侧都接＋真机验收 |
