...
| [plans/2026-08-27-pdf-annotation-plan.md](plans/2026-08-27-pdf-annotation-plan.md) | **「PDF 批注」方案（M24）**：竞品天花板对标（思源=块笔记顶格、MarginNote/LiquidText=思维工作台、Notion=无/Obsidian=靠插件）+ 2026 AI 帮读前沿；差异化定位「批注即块」；按 MVP 切（阶段 1：**双引擎渲染** + 批注 overlay + 内容寻址存储 + 摘录成块进反链/搜索 + 文本层判定/OCR 兜底；阶段 2：写回 PDF/OCR 精确划词；阶段 3：AI 帮读）。**阶段 1（Web v1.59.178 + 桌面 native v1.59.179 + 阅读器重构 v1.59.180）已落地**：`pdfRender` + `pdfAnnotation` + `pdf_annotations` 持久化 + `pdfjs-dist@4` 引擎 + `PdfReader`/`PdfAnnotationCanvas`（高亮/画笔/便签/摘录成块含可点击 `pdf://` 回链/文本层精确划词/OCR 兜底）+ 全局批注检索；**桌面 native 引擎（mupdf-sys）已落地（v1.59.179）**；**思源式阅读器（近全屏+左侧目录树+右侧批注侧栏+键盘导航+适配页宽）已落地（v1.59.180）**。阶段 2/3 待做 |
...
- [CHANGELOG.md](../CHANGELOG.md) —— **版本变更日志**（Keep a Changelog 格式，`v1.6.0` 起，当前 `v1.59.180`）。
