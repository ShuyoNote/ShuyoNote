...
| [plans/2026-09-23-lineage-conflict-adjudication.md](plans/2026-09-23-lineage-conflict-adjudication.md) | **血统冲突的「可裁决」那一半：方案稿（只出稿、未实现）**（冲刺 §13.3 第 2 条剩余部分）。先把"合并在结构上做不到"讲死（S1 红线）⇒ 真实选项只有三个：**留本机 / 用对端 / 两个都要（一页变两页）**，且**只有第三个不丢数据**。三条路对比：A 复用 `page_conflicts`（哨兵 `block_id=''`）**❌ 语义污染 ＋ 与"未决数量"那条纪律正面冲突**；B 新表 `page_lineage_conflicts`（页级、本地、可裁决）✅；C 只做"把对端那一版另存成新页"（最小、且顺手解决"裁决时那一版已经不在"）✅。**建议先 C 再 B**，并给出 8 条承重判据、5 条风险（快照体积／裁决要不要标脏／反复相撞去重／救回来的页归属／web 侧没有 pending 那条路）与明确不做的四件 |
| [plans/2026-09-23-projection-lag-closure.md](plans/2026-09-23-projection-lag-closure.md) | **投影与派生滞后：方案稿（只出稿、未实现）**（冲刺 §13.3 第 1 条）。事实：桌面"打开页面"那条端口只写 `page_crdt`（不写 `pages.content_json`、不打 `text_stale`、不重建块图），而 web 当场合并那条**会**写投影；且**两平面反链实现不同**（桌面是物化表、web 按需扫列）⇒ "只补投影列"在桌面**还不够**。四条路：A 合并后触发一次保存 ❌（`dirty=1` ⇒ 把对端内容当本机改动推回去＝假账）；**B 新增 `write_page_projection` 平台命令（B1 只写投影列／B2 ＋重建块图）✅ 推荐 B2**；C 下沉进 `save_page_state` ❌（Rust 没有 Yjs，等于 B）；D 只钉边界 ❌。含 8 条承重判据（含"没变就不许写库"与数据库页排除）与 5 条风险（时序／块图用的是可能旧的正文／成本／writer 交叉／不是桌面独有） |
