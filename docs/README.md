...
| [plans/2026-09-17-sm-crypto-tradeoff.md](plans/2026-09-17-sm-crypto-tradeoff.md) | **「国密方案 vs 原方案」利弊与跨平台影响（决策补充）**：给「换国密」这笔账一个可引用的对比。**利**=合规准入（等保/密评门槛，**唯一硬收益**）＋叙事差异化＋Tongsuo 与现有 vendored-openssl **同源**（是多一条构建路径 vs 不是，这条削掉一部分弊）；**弊**=① **KDF 降级**（Argon2id 内存硬化 → PBKDF2-HMAC-SM3，**国密无内存硬化对应物**）② **nonce 24B → 12B**、SM4 密钥 **128 位**（管理更严）③ **自维护 provider 是长期责任**（不是一次性成本）④ 若甲方改口要「认证产品」则自研全白做 ⑤ 软件 SM4 无 AES-NI 级加速、热路径需实测。**最该谈的**=**档 2（≈3–5 人日）vs 档 3（≈8–13 人日）**——档 3 的增量只有一句口径、代价却是最贵的 P3。**跨平台**：构建面**四条链各编 Tongsuo**（Windows MSVC 与 Android NDK 的 **Perl** 是硬坑；桌面吃系统 OpenSSL、只有 Android 用 vendored，`Cargo.toml:25` vs `:111`）；数据面**不存在「部分设备升级」**（解密端必须会 SM4，而协议里**没有任何算法标识**，`security.rs:473-476`）；**路径 3（TLCP）会额外伤跨平台**（Android `tls_android.rs` 的 `rustls-platform-verifier` 要重做、服务端换 Tongsuo Nginx）⇒ 建议先走路径 2。含最小风险四条做法（算法随空间记录 / 老端明确拒绝 / 保留双读 / 默认关按空间开）与归属决定（**已定 2026-09-17：国密代码进开源主干**——CLA 支持双许可、Tongsuo 为 Apache-2.0 故许可面不变；连带**主干要有常开国密 CI job**、Tongsuo 按 `fetch-pdfium.mjs` 套路钉版本＋校验和、**默认算法仍 v1**；仍待定：feature 门控 vs 默认编） |
