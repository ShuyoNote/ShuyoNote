...
| [plans/2026-09-23-desktop-near-realtime-stream-design.md](plans/2026-09-23-desktop-near-realtime-stream-design.md) | **桌面「近实时」流通道：设计稿（未实现）**。缺口＝桌面没有推送通道（`useSyncStream` 对桌面直接 return ⇒ "近实时"在桌面上等于轮询间隔，而它同时是 **CRDT 合并的可见延迟**）。现状事实清单（服务端 SSE 端点/`KeepAlive`/`Lagged`⇒`ping` 语义、Web 已有消费方、桌面零消费方、桌面已有 `app.emit`↔`listen` 通路）；三条候选形态 ⇒ **选"Rust 订流 ＋ 发事件、拉取仍由前端发起"**（**理由是自动经过 C2 Wi-Fi 闸门/防重入/状态行**）。含帧解析与退避两个**纯函数判据**、三条命令（**登记 web-only，与刚撤掉的 claim 相反，理由写清**）、去抖 300ms、**轮询保持不变**、开关默认值与**七条判据**、五条风险边界（代理缓冲／`Lagged` 不许当心跳忽略）、以及动手顺序（可与真机双设备验收一次做完） |
