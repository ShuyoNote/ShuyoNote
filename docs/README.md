...
| **M6** | 移动端适配 | [执行中]（**Tauri 原生壳**；Android CI 已跑通，未签名 arm64 APK **53.41 MiB**） | **[移动端上线计划（Android 优先）](plans/2026-09-13-android-launch-plan.md)** + [移动端](MOBILE.md)（路线与各平台对照）+ [跨平台方案](plans/2026-08-24-cross-platform-plan.md)（升级为 M16 全平台通吃） |
...
| [plans/2026-09-13-android-launch-plan.md](plans/2026-09-13-android-launch-plan.md) | **移动端上线计划（Android 优先，规划 → 执行中）· 工程侧**（合规 / 商店 / 推广已移入私有仓库）：目标是**上架 Android**，第一批用户走**官网 APK + 酷安**，商店与软著并行后补。含**现状审计**（UI 移动端布局与 `verify-mobile-layout` 已做、Rust `#[cfg(desktop)]` 已就位、`security.rs` 密钥**本就不落盘**所以 Android 无需 Keystore、APK 能构建但**未签名**、`gen/android` **不在 git 里**）；**体积账（已实测收口）**：arm64 **156.7 → 53.41 MiB**（`strip` + tesseract-core 白名单 + OCR 语言包改按需下载），**目标 55–70 MiB 已达成**；CI（`.github/workflows/android.yml`）**已跑通**（路上翻出两个只在 Linux 上暴露的坑：NDK 无 `aarch64-linux-android-ranlib`、mupdf-sys 的 bindgen 不带 `--target`）；**⚠️ 路线冲突**——既有 `MOBILE.md` 定的是「WebView 壳 + Web 内核」，与本计划的 **Tauri 原生（Rust 内核）** 是两个不同产品（前者无多设备同步、插件根本性不可用、数据落在会被回收的浏览器存储），已列出对照表与选型理由；五个阶段（先能装 → 手机上真的好用 → 能持续发 → 上商店，合规与工程并行）各自带**可核对判据**、按风险排序的八条风险清单，以及「上线」的定义＝**一个陌生人从官网下载到卸载重装能恢复**这条路径由本人真机走过 |
...
> **本移动端上线计划的合规 · 商店 · 推广部分**（→ 私有仓 `docs/android-launch-plan.md`，
> 本仓库只留工程侧）。判据：**产品 / 工程 / 给用户看的对比**留公开仓，
