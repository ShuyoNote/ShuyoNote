...
- **一键发布到社区**（社区互动方案 §三 第 3 条的 P0；分支 `feat/publish-to-community`，
  ...
  - **同修订重发不会多发一篇**；发布成功后在本地记一条台账，界面显示"这篇还没发过／这一版已经发过／
    上次发的是更早的一版（再发会新建一篇）"。
  - 判据：`community_publish::` **12 条**（含真起环回监听的假社区，断言 CSRF 双重提交 / Bearer /
    幂等键 / 原始字节）、对话框渲染级 **28 条**、`community::` 里面向线上真站点的活判据一条（`#[ignore]`）、
