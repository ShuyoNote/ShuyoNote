...
- **发布者公钥会被记住：换了密钥就拒绝安装**（M11.11b 第二块——发布者签名 + TOFU）。索引现在可以给每个插件声明 `signature`（对**包字节**的 minisign 分离签名）与 `publisherKey`（发布者公钥，两者必须成对出现）。安装前按它校验：不对就拒。
  ...
  - 测试：Rust 六条（指纹稳定/文件形式与裸 base64 同指纹/换一个字节就不同、签名与公钥必须成对、**包字节的签名校验（真签名通过 + 改一个字节即拒）**、首次固定 + 一致 + 换 key 判成 Changed + 别的插件不受影响 + 形状不对要报错、接受新 key 后按新的比） + 前端 10 条（三种签名状态各说各的话、公钥变了要同时摆出两个指纹、按钮变成「信任新密钥并安装」、确认框里写明这是替换信任对象、以及只有确认后才带 `trustNewKey`）。变异验证：让"与固定的 key 一致"永远成立 → 两条转红。
