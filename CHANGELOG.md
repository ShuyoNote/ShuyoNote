...
- **PDF 阅读护眼模式**：阅读器头部新增「眼睛」开关（本地持久化 `shuyonote.pdf.eyecare`）。开启后阅读区背景由白色换为**护眼米黄**（暗色主题下为昏暗暖褐），并给 PDF 页面图像叠**温和减蓝/柔光滤镜**（`sepia(0.16) saturate(0.9)` 等，只作用于页图、不影响高亮/便签等批注 SVG），降低长时间阅读对眼睛的刺激。`.pdf-reader.eye-care` 一组 CSS 覆盖实现，独立于明暗主题。
