...
- **扫描版电子书「一键 AI 生成目录」**：扫描版 PDF 没有内置目录，现可在左侧目录面板（空态）点「AI 生成目录（本段）」——从**当前打开的页往后 60 页**逐页 OCR（复用同一 tesseract worker，避免每页重载模型），把每页文本片段（带 `[第 N 页]`）喂给已配置的 LLM，让它输出章节 JSON `[{"title","page"}]`，解析后填入目录、点击可跳转。新增 `src/lib/pdfOutlineGen.ts`（纯函数：组装提示/解析 JSON 容错/换算 `OutlineItem`，已单测）、`src/lib/aiOutline.ts`（OCR+LLM 编排、进度上报、`AbortController` 可取消）、`PdfOutline` 空态入口 + 进度/取消按钮，`PdfReader` 接入 `generateAiOutline`；`ocr.ts` 重构支持 `createOcrWorker` 批量复用。
