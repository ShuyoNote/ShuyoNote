...
### 修改
- **「关于」弹窗瘦身**：去掉中文名「数友笔记」（只留 `ShuyoNote`）与那行产品说明（"本地优先 · 类 Notion 的知识管理笔记应用…"）。随之把只服务于这两行的 `APP_NAME_ZH` / `APP_DESCRIPTION` 常量与 `.about-desc` 样式一并删掉，不留没人用的死代码。窗口标题（桌面 `lib.rs` / Web `main.tsx` / `index.html` 静态标题）里的中文名**未动**——那是另一个界面。
