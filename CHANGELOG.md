...
- **上下箭头/翻页平滑滚动**：键盘 ↑/↓ 与页面导航箭头此前是瞬间跳到目标页顶（翻页跳跃）。现改为平滑滚动——↑/↓ 像滚轮一样逐帧平滑滚动一屏（90% 视口高），←/→ 与「上一页/下一页」按钮平滑滚动到相邻页顶；目录/侧栏跳到远页仍保持快速瞬时到达（兼顾「丝滑」与「远距跳转快」）。↑/↓ 步进进一步复刻滚轮：固定 ~100px/格、平滑逐格累积（连续按像滚轮连续滚动）。
