...
- **PDF 阅读护眼模式（多档位）**：阅读器头部「眼睛」按钮改为**护眼档位下拉**（本地持久化 `shuyonote.pdf.eyecare`）：**关闭 / 柔光 / 暖黄 / 夜间 / 淡绿**。开启后阅读区背景换为对应暖色纸底（浅色下护眼米黄/暖黄/淡绿，暗色下昏暗暖褐），并给 PDF 页面图像叠不同强度的**降蓝/柔光滤镜**（`sepia`/`hue-rotate` 等，只作用于页图、不影响高亮/便签等批注 SVG）。各档位用 CSS 变量定义、共享规则应用；暖黄/夜间比原单一档更明显。顶部 `眼睛` 打开下拉选档位，点击外部关闭。
