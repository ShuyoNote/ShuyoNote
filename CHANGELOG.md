...
> **M11.8：命令参数（宿主渲染表单）+ 事件钩子**——插件此前只能被用户手动触发、且只能无参执行。

### 新增
- **插件设置（M11.8 收口）**：manifest 声明 `settings`，宿主在「插件管理 → 设置」渲染表单；插件用新能力 `api.settings.get(key)` **只读**。
  - **写只发生在宿主界面**：插件对 `setting:` 命名空间是只读的（`kv.set`／`kv.remove` 会被后端拒，有测试）。否则用户看到的配置就不再是他亲手设的那个——而设置里往往是「归档到哪个文件夹」这类**只有用户本人该做**的决定。
  - **值由宿主校验**：`number` 一定是数字、`boolean` 一定是 true/false、`select` 一定在候选项里，所以插件不必防御「用户填了乱七八糟的东西」；`select` 声明了却没有 `options` 会在校验阶段直接报错。
  - **scope 由声明决定、不由插件选**：`"space"`（默认）随空间 SQLCipher 加密，`"app"` 落 `meta.db`（**明文**，设置面板会就此给出提醒）。落库复用 `plugin_data` 同一张表、不同命名空间，没有新增存储。
  - **没设过返回 `null`**（据此退回插件自己的默认值）；**没声明的 key 直接报错**（key 名字写错是最常见的低级错误，静默返回 null 会让人查很久）。
  - 顺带把「按声明渲染输入控件」抽成共用组件（命令参数表单与设置表单共用），避免将来加一种字段类型时只改到一处。
- **事件真正发出来了（M11.8 补齐）**：此前注册表里宣传 7 个事件、只有 `page.saved` 会触发——**说了做不到**。现在：
  - 已接的 5 个：`app.started`（启动，且**插件列表先加载完再播报**，否则插件还没被认出来就错过了）、`page.opened`／`page.deleted`（`notes` store 的收口处）、`space.switched`（切换成功后）、`page.saved`（原有）。
  - **没接的 2 个（`import.finished`／`sync.completed`）在注册表里标为 `hosted: false`**：不进类型包（写它会编译报错），校验器与插件日志也会明确告知「宿主还没开始发」——而不是让作者写对了名字、启用了插件、却永远收不到。
  - **零依赖的事件公告口** `src/lib/pluginEvents.ts`：`notes` / `space` / `App` 只「播报事实」，不 import 插件层（否则立刻绕成循环依赖：插件 → 草稿确认 → `applyDraftAndRefresh` → notes → 插件）。谁听、怎么听由插件层在启动时注册。
  - **没人订阅就不发 IPC**：`page.opened` 这类事件每次切页都会播报，插件列表本来就在内存里，先判一下有没有「启用中且订阅了它」的插件再决定是否跨进程（有测试钉住「什么时候该发、什么时候不该发」）。
- **触发面 v1：编辑器 `/` 菜单（M11.8）**：`register({ menus: ["slash"] })` 声明后，命令出现在编辑器里输入 `/` 弹出的菜单中（分组「插件」）——插件命令不再只能靠 `Ctrl+K` 搜索才发现。
  - 合法入口进注册表单一事实源（`capabilities/capabilities.json` 的 `menus`），**类型包只广告已实现的入口**（`PluginMenuName` 现在只有 `slash`，写别的会编译报错），而校验器对「入口存在但宿主还没接」的值**如实告知**（`menu_not_hosted`），不静默丢掉作者的声明。
  - **带参数的命令被选中时转交给命令面板**并用标题预填查询（参数表单只实现一份，不重复造对话框）；为此把命令面板的开关与查询词从组件内部 `useState` 提升到 `src/store/palette.ts`。
  - 插件命令的**执行与结果处理抽成共用链路** `src/lib/pluginRun.ts`（结果消息、`__toast`、插入文本、**待确认草稿**）——触发方式不该改变行为，尤其是草稿确认这条：任何一个入口自己实现一遍，都会出现"某入口忘了处理草稿"＝绕过写中介。
- **事件钩子 v1（M11.8）**：插件可以订阅宿主事件（`app.started` / `space.switched` / `page.opened` / `page.saved` / `page.deleted` / `import.finished` / `sync.completed`），在用户**没点任何命令**时也能跑。两处都要写、缺一不可：manifest `events: [{ on, reason }]` 是给用户看的授权面（启用前展示，和权限同样对待），JS 里 `on(name, handler)` 才是实现——只声明不注册什么都不会发生，只注册不声明则收不到事件。
  - **只有启用中的插件会收到事件**；事件名不认识就忽略 + 警告（前向兼容，与权限同一套判定）。
  - **与权限不同：没有基线授权**——没写 `events` 的插件（含所有老插件）**一个事件都收不到**。在后台跑代码更不能默认给。
  - **写操作仍然要用户确认**：事件里产出的草稿会汇总成**一次**确认（而不是静默写入笔记）。这是本体系最容易被绕过的缝隙——事件触发时用户正在编辑，没在看确认框——所以草稿确认抽成共用链路并有测试钉住。
  - **事件里的 `insert` 被忽略**：触发时没人在等你插入文本，凭空冒出文字比不支持更糟。
  - 事件处理器有**更短的墙钟预算**（2s，保存路径上不该有慢活）；一个处理器抛错或权限被拒不会吃掉其它处理器，错误随结果回传并写进插件日志；事件里的「当前页」= payload 的 `pageId`，所以作者省略 `pageId` 时写的就是刚保存的那一页。
- **事件名进注册表单一事实源**：`capabilities/capabilities.json` 新增 `events`，由此生成 Rust 事件表、类型包 `PluginEventName` 与作者文档 §4.6；应用内「校验」与 `pnpm plugin:validate` 都会检查 `events`（未知事件名、缺 `reason`、重复声明）。

### 已知缺口（必须记下来，不能装作没有）
- **插件更新后新增 `events`/权限时不会重新征求同意**：用户当初是在没有事件的情况下启用它的，更新后它就开始在后台运行。修复方向：把「授权快照」记进 `plugin_install`，发现声明扩张时**自动禁用并提示重新确认**。这属于 M11.11b 的治理面（插件更新与信任），登记在路线图。

### 新增
- **命令参数（M11.8 第一档）**：`register({ params: [...] })` 声明参数后，命令面板会**照着声明**渲染表单（文本框 / 数字框 / 复选框 / 下拉框），填完才执行，`run(args)` 收到整理好的对象。声明与渲染只有一处来源，所以不存在"表单和实现对不上"。
  - 规则都可预期：必填留空**拒绝提交**（不传空串——空串往往是有效值）；非必填留空**不传这个键**（插件的默认值才生效）；`number` 转数字且 `0` 有效；`boolean` 总是传布尔值；`type` 写错按 `string` 处理（宁可给文本框，也不让命令从面板消失）。
  - 宿主侧限了参数体积并要求是 **JSON 对象**（不是数组/标量）；不做第二套 schema 校验——参数只流进插件自己的 JS，真正碰数据的是 `api.*`，那一步宿主逐次校验权限与参数，多一套实现只会多一处漂移。
  - 转换规则抽成纯函数（`src/lib/pluginParams.ts`）并有 9 个单测：这三种"错了也不崩、只是悄悄行为不对"的情况全部钉住。
- **结构化返回**：`run()` 除字符串外还可返回 `{ message, insert, toasts }`，等价于调用对应的宿主原语（`insert` 仍受 `write:page.current` 权限约束——结构化返回不是绕过权限的后门，有测试）。

### 测试
- Rust 新增 2 个（触发面原样透传与去重限量、未实现入口被如实指出）；前端新增 4 个（`/` 菜单里插件命令的三条筛选规则）。
- Rust 新增 5 个（参数声明归一化与透传、无参命令仍可用、结构化返回落到对应通道、结构化返回仍受权限约束、非具名参数退回空对象）。
- 前端新增 9 个（参数转换规则与表单初值）。

### 文档
- `docs/plugin-api.md`（生成）新增 **§4.5 命令参数**；类型包补 `PluginCommandParam` / `PluginCommandResult` / `PluginCommandArgs`（并写明**参数值不做静态类型推导**的理由：TS 在这里对字面量类型的推断不可靠，给出一个看着精确、其实会撒谎的类型不如诚实给 `any`）；示例插件 `daily-note` 增一个带参数的命令。

### 已知限制（写作示例时发现的，已记入 M11.8 待办）
- **草稿落库后插件拿不到新页面的 id**：`api.pages.create()` 产出的是草稿，页面要等用户确认后才存在，而确认发生在插件这次运行之外。所以"建一个页面，之后往它里面追加"这类多步插件目前做不到（示例已如实标注，不假装能做到）。

> **M11.6 收口：插件作者工具链**——插件写完之后「怎么知道对不对、怎么不用重启就看到效果」
> 这一环此前是空的（文档承诺了校验与热重载，仓库里没有）。另修掉两处文档与代码不一致。

### 新增
- **作者校验（应用内 + 命令行，同一套规则）**：
  - **应用内「校验」**（`plugin_validate`）：一次列出插件的**全部**问题而不是只报第一个——manifest 字段、权限与理由、**JS 语法（用宿主同一个 Boa 引擎解析）**、能不能注册出命令；并回传命令清单与**实际授予**的权限。它与加载器**同源**（同一份 `read_manifest` + 同一次 Boa discovery），并且有一道兜底：只要加载器会拒载，报告里必然至少有一条 error——「校验说没问题、应用却装不上」在结构上不会发生。
  - **命令行 `pnpm plugin:validate <目录>`**：给编辑器/CI 用的对照检查（含 `--json`），权限清单与风险等级、理由一并打印；权限与 API 版本取自 `capabilities/capabilities.json`（与 Rust 侧同一个源），所以这几项不会两边打架。
- **热重载**：插件面板打开期间低频轮询插件目录指纹（文件名 + 大小 + mtime，无新依赖），文件一改就自动重新扫描——命令面板里的命令与行为随即更新，面板标题显示「已自动重新扫描 …」。作者的循环变成改文件 → 自动重扫 → 看日志，不必重启应用。
- **示例插件 `examples/plugins/`**（3 个，可直接抄）：只读（空间概览）、**写能力走草稿确认 + 插件私有数据**（每日笔记）、先读后写且省略 `pageId` 作用于当前页（日期标签）。它们同时是回归测试：`cargo test` 用**与加载器同一条路径**断言「能加载、能注册命令、权限都认识且都写了 reason」，CI 再用作者 CLI 与 `tsc` 各验一遍。
- **类型包补齐脚本式插件的全局声明** `packages/plugin-types/globals.d.ts`：插件是脚本不是模块（没有 `import`），此前 `index.d.ts` 是模块，`// @ts-check` 的插件文件根本看不见 `api` 与 `register`——「有类型包但没补全」。现在编辑器与 `tsc` 都能对脚本式插件生效。
- **类型包的返回类型精确化**：13 条能力的返回值由 `unknown` 改为真实类型（依据 Rust 实现实际返回的字段逐个核对，不是照抄描述），例如 `api.pages.list()` 是 `{ id; title; updated_at }[]`、写能力返回 `{ drafted: true; summary }`——一眼看出「这是草稿，不是已写入」。
...
### 测试
- Rust 新增 12 个校验器测试（含示例插件的端到端断言、以及 id 不安全 / 主版本不支持 / Boa 语法错 / 入口缺失 / 未知权限 / 无命令 / **加载器会拒时报告必然有 error** 等负例）。
- CI 新增「示例插件（类型检查 + 作者 CLI 校验）」步骤。

### 文档
- `docs/plugin-api.md`（生成）新增 **§9 开发循环：写 → 校验 → 看日志**（自动重扫、校验、日志/活动、命令行对照检查、编辑器类型配置），并在快速开始处给出指引。
