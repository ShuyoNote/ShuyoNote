...
/**
 * 端到端那一组：每条都**真的 spawn 一次脚本**（带着假 PATH / 假 dpkg），
 * 而 vitest 默认单测超时是 **5000ms**——Windows 上这一趟实测 **~6.2s**
 *（进程启动 + 假 dpkg 转发 + 工具链探针），于是同一份代码在**边界上抖**：
 * 2026-09-22 这台机器上跑同一份测试，三次分别红 5 / 2 / 3 条，而且**红的不是同一批**
 *（"本仓默认跑" ✗、"变异②" ✗、"变异④" ✗、"--json 可解析" ✗ 都出现过）。
 * 判据本身没问题（脚本单独跑 `node scripts/check-sys-deps.mjs` 是 exit 0，1 秒内跑完），
 * 只是"给一个原本就要 6 秒的端到端留 5 秒"。这里显式放宽到 30s：
 * **它不改任何断言的语义**，只是让慢机器/冷启动不再把"超时"记成"缺包"。
 * ⚠️ 真出了功能问题仍然会红——红的是断言，不是超时。
 */
describe("check-sys-deps 端到端（真脚本、真锁文件、假 dpkg）", { timeout: 30_000 }, () => {
