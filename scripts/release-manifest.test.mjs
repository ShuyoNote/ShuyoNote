...
//
// 做法（不改生产代码、不联网、跑完就清）：
//   ① 在 bundle 目录里放合成产物（版本取 package.json，dmg 用 `universal`、另加 `.app.tar.gz`）；
//   ② `--dry-run --no-build --no-plugins --no-android --no-web --skip-sig-verify`
//      （不发布、不构建、不需要发布令牌；签名校验跳过，因为这是合成文件）；
//   ③ 用 `SHUYONOTE_PREV_MANIFEST_JSON` 注入一份"线上清单"当比较基准 ⇒ **不访问网络**；
//   ④ 断言 darwin **两个键**都指向 `.app.tar.gz`，windows→exe、linux→deb，且 dmg 不进清单；
//   ⑤ `afterAll` 把合成的产物与 `latest.json` 删干净（**别把假产物留给下一次真发布**）。
//
...
describe("发布清单端到端（真跑 release.mjs --dry-run + 合成产物）", () => {
  it("universal 构建：darwin 两个键都指向 .app.tar.gz，dmg 只发布、不进清单", () => {
    ...
    // 注入"线上清单"当比较基准：这样覆盖检查不需要联网（生产发布**绝不能**设这个变量）
    writeFileSync(
      prevManifestPath,
      JSON.stringify({
        version,
    ...
    const url = (k) => manifest.platforms[k]?.url ?? "";
    const name = (k) => url(k).split("/").pop();
