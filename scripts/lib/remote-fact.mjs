...
//
// ## 同一条纪律已经落在另外两处（不是这里发明的）
//
// "**未实查 ≠ 红**"在本仓是同一条口径的三次落地，改这一处时值得一起看：
//   · `scripts/check-crypto-backend.mjs`：旧产物 ⇒ **未实查**（不判红）；
//   · `rust-sm-wired` 在 win32 上：装载期失败 ⇒ **未实查**（并打印原因）；
//   · 这里：远端取不到 / 401 / 5xx ⇒ **未实查**。
// 共同理由一句话：**假红比没有红更坏** —— 它会训练人忽略红。
