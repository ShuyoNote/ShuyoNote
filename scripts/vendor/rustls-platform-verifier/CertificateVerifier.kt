    ...
    private fun verifyCertificateChain(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            val revocationChecker = validator.revocationChecker as PKIXRevocationChecker
                        ...
                        // ------------------------------------------------------------------
            // 【ShuyoNote 本地补丁 2026-09-13】默认改成"优先 CRL、不回退 OCSP"。
            //
            // 为什么必须打这个补丁：Let's Encrypt 从 2025-08 起**取消了 OCSP**，新证书的
            // AIA 里没有 OCSP 地址、只有 CRL 分发点。而 Android 的吊销检查器**默认先走
            // OCSP**，遇到"证书没有 OCSP 地址"就抛
            //   CertPathValidatorException: Certificate does not specify OCSP responder
            // 并被上层当成**已吊销**（fail-closed 的错）⇒ 真机上所有 LE 站点都报
            //   invalid peer certificate: Revoked
            // 实测：shuyo.cn / community.shuyo.cn / letsencrypt.org 全中招，百度（有 OCSP）正常。
