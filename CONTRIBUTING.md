...
| **贡献许可（CLA）** | [CLA.md](CLA.md) —— 提 PR 前需签核（见下方「License 与贡献许可」） |
...
- **CI 会跑这些**（push/PR 到 `main` / `dev` 时自动触发，见 `.github/workflows/ci.yml` 的 `on.push.branches`）——但本地先跑一遍能省一轮往返：
...
- ShuyoNote 客户端以 **AGPL-3.0** 开源：任何人都能读、改、自建，**这一点不会变**；
  你贡献的代码同样会以 AGPL-3.0 对所有人开放。
- **提交贡献前，请确认同意 [贡献者许可协议（CLA）](CLA.md)**。它在你的版权之外，
  额外授予维护者一项**再许可权**（含以商业授权等非开源条款分发）——
  这是「开源版 + 商业授权」双轨并存的前提。**没有它，任何一次外部贡献都会把整个仓库永久锁在 AGPL 上。**
- **签署方式**：在 PR 描述里加一行
  `贡献者许可协议：我已阅读并同意 CLA.md，并有权提交本贡献。`（一次签核长期有效）
- **你不转让版权**：你仍是你贡献的版权人，可自由用于其他项目。
- 自托管 / 团队同步的 `shuyonote-sync-server` 为独立商业组件，不适用本客户端仓库。
