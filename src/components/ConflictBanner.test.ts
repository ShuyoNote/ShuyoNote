// 阶段 1 · **冲突提示条的判据**（happy-dom；数据/命令都打桩，只验"看得见 + 按得动"）。
//
...
describe("ConflictBanner（阶段 1 · 冲突提示条）", () => {
  ...
  it("② 有冲突 ⇒ 两侧原文都在，两个按钮都在", async () => {
    ...
    expect(buttons).toEqual(["conflicts.keepLocal", "conflicts.useRemote"]);
  });
