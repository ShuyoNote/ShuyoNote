...
import { resolveWorkspaceSyncScope, type ClaimScopeRow } from "./claimScope";
...
describe("S9 · claim 发给谁（页所属工作空间 ⇒ 远端空间 id）", () => {
  it("① ★ 承重：页属于 W ⇒ 发给 **W 绑定的服务器与远端 space_id**（**不是** W 自己）", () => {
    const scope = resolveWorkspaceSyncScope([row({ ws_id: "ws-1", space_id: "REMOTE-SPACE" })], "ws-1");
  ...
  it("② 没有档案／只填了服务器／只填了空间 ⇒ 一律 `null`（**不发请求**：发出去只会换 403）", () => {
    expect(resolveWorkspaceSyncScope([], "ws-1")).toBeNull();
    expect(resolveWorkspaceSyncScope([row({ ws_id: "ws-other" })], "ws-1")).toBeNull();
    // 登录了但还没选空间（保存地址与选空间是两步）—— 正常中间态，不是错误
    expect(resolveWorkspaceSyncScope([row({ space_id: "" })], "ws-1")).toBeNull();
    expect(resolveWorkspaceSyncScope([row({ space_id: "   " })], "ws-1")).toBeNull();
    // 解绑后只留地址：同理不发
    expect(resolveWorkspaceSyncScope([row({ server_url: "" })], "ws-1")).toBeNull();
  });
  ...
  it("③ 多个工作空间各绑各的 ⇒ 必须选**这一页那一个**（不是「排序第一个」）", () => {
    ...
    const scope = resolveWorkspaceSyncScope(rows, "ws-1");
