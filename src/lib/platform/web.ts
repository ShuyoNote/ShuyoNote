...
export function makeInvoke(store: SqliteStore) {
  ...
  return async <T,>(cmd: string, args?: Record<string, unknown>): Promise<T> => {
    ...
    if (cmd === "email_save_as_note" || cmd === "email_fetch_inbox" || cmd === "email_fetch_all" || cmd === "email_fetch_all_months" || cmd === "email_save_uid" || cmd === "email_get_body" || cmd === "email_get_html" || cmd === "email_get_message" || cmd === "email_get_attachments" || cmd === "email_save_account" || cmd === "email_get_account" || cmd === "email_list_accounts" || cmd === "email_remove_account" || cmd === "email_unseen_count" || cmd === "email_list_folders" || cmd === "email_list_months" || cmd === "email_set_flag" || cmd === "email_mark_read" || cmd === "email_mark_many_read" || cmd === "email_move_to_trash" || cmd === "email_move_many_to_trash" || cmd === "email_send" || cmd === "email_test_connection") {
    ...
    if (cmd === "encryption_status") {
      return {
        ...
        // Web 形态没有静态加密 ⇒ 空间级标识恒为"未记录"（不要写 1：那会谎称"这个空间是 v1"）
        space_format: 0,
        space_algorithm: "",
      } as T;
    }
    ...
    if (cmd === "show_window_menu") return undefined as T;
    if (cmd === "set_mica_effect") return undefined as T;
