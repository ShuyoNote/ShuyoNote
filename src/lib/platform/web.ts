...
function makeInvoke(store: SqliteStore) {
  ...
  return async <T,>(cmd: string, args?: Record<string, unknown>): Promise<T> => {
    ...
    if (cmd === "email_save_as_note" || cmd === "email_fetch_inbox" || cmd === "email_save_uid" || cmd === "email_get_body" || cmd === "email_save_account" || cmd === "email_get_account" || cmd === "email_unseen_count" || cmd === "email_list_folders" || cmd === "email_set_flag" || cmd === "email_mark_read" || cmd === "email_move_to_trash" || cmd === "email_move_many_to_trash") {
