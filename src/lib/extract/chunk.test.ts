...
describe("分块 · 基本口径", () => {
  it("短文本 ⇒ 一块；`loc` 保留；`hash` 是内容哈希", async () => {
  ...
  it("**不超硬上限**：一份长文本切出来的每一块都 ≤ max", async () => {
  ...
  it("**块是被「填满」的，不是一段一块**：多个短段会装进同一块", async () => {
  ...
  it("**单句超上限只能硬切**（并说明：没有更好的边界可用，但硬上限必须成立）", async () => {
  ...
  it("超长**段**被切开时，只有第一片继承 loc（后几片没有更细的定位可给）", async () => {
...
describe("分块 · 重叠", () => {
  it("第二块起带上一块的**末尾**作为前缀，且不超过上限", async () => {
  ...
  it("第一块**没有**前缀（没有上一块可借）", async () => {
    const chunks = chunkSegments({ kind: "attachment", attId: "a" }, [seg(longZh(120))]);
    expect(chunks[0].text.startsWith("第0段")).toBe(true);
  });

  it("重叠前缀尽量从边界起（不以半个词开头）", async () => {
  ...
  it("overlap = 0 时行为退化：无前缀（策略是可设的，不是写死的）", async () => {
...
describe("分块 · 确定性（缓存与回链都依赖它）", () => {
  ...
  it("**id 随 ord 稳定** ⇒ 源文本没变时 id 不变（块嵌入因此能复用）", async () => {
  ...
  it("内容变了 hash 就变（这就是「要不要重算嵌入」的判据）", async () => {
...
describe("分块 · 页面路径", () => {
  it("标题只挂在**第一块**（每块都挂会让标题在向量里反复出现、并被摊薄）", async () => {
...
describe("块存储（真 sqlite）", () => {
  it("整体替换 + 按 ord 取回", async () => {
    ...
    const back = (await store.chunksOf({ kind: "attachment", attId: "a1" }));
  ...
  it("**重切不会留下孤儿行**：新块更少时，多出来的旧行必须被删掉", async () => {
    ...
    const many = (await store.stats()).chunks;
  ...
  it("页面块与附件块互不干扰（同一个库里两种 owner 共存）", async () => {
    ...
    expect((await store.chunksOf({ kind: "attachment", attId: "a1" })).map((c) => c.text)).toEqual(["附件文本。"]);
    expect((await store.chunksOf({ kind: "page", pageId: "p1" })).map((c) => c.text)).toEqual(["页面正文。"]);
    expect((await store.chunksOf({ kind: "attachment", attId: "p1" }))).toHaveLength(0); // att 与 page 的 id 空间不混
  });
  ...
  it("remove 清空该 owner", async () => {
    ...
    store.remove(owner);
    expect((await store.chunksOf(owner))).toHaveLength(0);
  });
});

describe("哈希口径统一（防「两处实现漂移」）", () => {
  it("分块用的 `fnv1a32` 与既有 `embedHash` **逐字一致**", async () => {
