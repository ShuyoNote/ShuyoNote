...
function pdfReader(): Block[] {
  return [
    ...
    h("批注", "h2"),
    bullet([
      "高亮 / 画笔 / 便签；自动提取文本层做精确划词，扫描件走 OCR（**首次使用需联网下载一次语言包，之后永久离线可用**）。",
...
function ai(): Block[] {
  return [
    ...
    h("PDF / 视觉", "h2"),
    bullet([
      "PDF 划选帮读总结、整篇提问；页面截图视觉识别（OCR 离线兜底；首次使用需联网下载语言包）。",
    ]),
    rule(),
    h("下一步", "h2"),
...
function faq(): Block[] {
  return [
    ...
    h("能离线用吗？", "h2"),
    para("可以。核心功能完全离线；仅 AI / 同步 / 检查更新需联网（AI 可选本地模型）。**OCR 算是半个例外**：识别过程在本机完成、不上传，但中英语言包约 30 MB 不随安装包分发，**首次使用需联网下载一次**，之后由缓存复用、永久离线可用。"),
    h("页面里的 [[...]] 能点吗？", "h2"),
