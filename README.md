...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.90.1-blue" alt="version">
...
- **同步一致性加固（seq-LWW + dirty 优先本地，v1.84.3）**：LWW 改用**服务端单调 `seq` 为权威序**，配合 **dirty 优先本地**——根治团队多人同时改同一内容时因客户端**时钟漂移**而丢改动的隐患。服务端 `changes` 自增 `seq` 全局唯一、单调；本地未提交改动标 `dirty` 优先保留，推送成功才清除。
- **附件同步**：内容寻址去重，双向增量。
...
- [ ] **Android 移动端**：Tauri 原生壳，已能构建出 APK（未签名）；差体积压缩 / 签名 / CI / 真机验收 / 上架材料，见 [移动端上线计划](docs/plans/2026-09-13-android-launch-plan.md) 与 [移动端路线](docs/MOBILE.md)。iOS 随后（那台 Mac 的工具链需先解决）。
