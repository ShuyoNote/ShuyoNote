...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.91.22-blue" alt="version">
...
**同步机制**：本地每次写操作在 `changes` 表记录 outbox 变更；同步时先 push 本地增量，再 pull 服务端增量。冲突合并采用 **LWW（服务端单调 `seq` 为权威序）+ dirty 优先本地**（v1.84.3 起）——根治团队多人同时改同一内容时因客户端时钟漂移而丢失改动的隐患：服务端 `changes` 自增 `seq` 作为全局唯一、单调的 LWW 键，本地未提交的改动标记 `dirty` 优先保留，推送成功才清除。删除走墓碑，附件按内容寻址去重传输。若开启端到端加密，push 前加密、pull 后解密（服务端仅存密文），锁定会话时同步被拒绝。
