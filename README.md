...
<p align="center">
  <strong>本地优先的类 Notion 生产力工具：知识库 + 项目管理</strong><br>
  基于 Tauri 2 + Lexical + SQLite，数据完全存储在本机，离线可用；块编辑器、数据库视图、看板拖拽、<b>原生甘特图</b>、AI 助手、语义检索，支持多设备同步与全平台（浏览器 / 桌面）运行。
</p>

<p align="center">
  <img src="https://img.shields.io/badge/version-1.91.26-blue" alt="version">
...
- **内置网盘**：文件夹即网盘——批量上传超大文件（流式）、文件管理页（类型 / 大小 / 时间）、文件引用到页面（文件卡片 / 系统打开）、同名文件历史版本；内容寻址去重存储（**按空间**去重：每个空间各存自己那份）。
...
- **可自托管、多设备同步**：多设备 / 团队同步用自建 `shuyonote-sync-server`（无云锁定，可私有部署）；outbox 增量 + 服务端 seq 基准的 LWW 合并（**同一页同时编辑按服务端顺序保留一方**，页级 LWW、非 CRDT；被覆盖一方可从**每页 50 份版本历史**取回）。⚠️ **这是「多设备一致」，不是「实时协作」。**
...
首次启动会在应用数据目录（Windows：`%APPDATA%\cn.shuyo.shuyonote\`，WAL 模式）创建：`meta.db`（应用级：空间 / 同步 / 模板 / 插件状态）+ `spaces/<ws_id>/`（每空间独立库）+ `attachments/<空间 id>/`（附件**按空间**存放，空间内内容寻址去重）。Web 版则在浏览器 IndexedDB 中持久化（`shuyonote` 存 SQLite 快照、`shuyonote-blobs` 存附件字节、`shuyonote-spaces` 存多空间 catalog + 快照）。
...
- **数据库视图**：表格 / 画廊 / 看板 / 列表 / 日历 / 时间轴 / 目录 / **甘特图** 八种视图；查询型数据库（规则收页）、保存视图 + `ref` 关联属性 + 公式列 + 跨库 rollup 聚合。
- **原生甘特图**：用日期列（开始/结束，或 4 列计划/实际两组）渲染网格填色甘特图——时间轴、网格线、可编辑日期（文本+日期选择器）、列宽可拖；数据表格一键变专业甘特图。
- **看板视图**：按标签 / 分组字段分列；**卡片跨列拖、列(分组)拖换序、插入位置竖线、未设置列固定最左**——像专业看板工具，而不只是"按字段分组"。
- **语义检索**：嵌入向量语义搜索，不只关键词——全空间、跨空间找得到"想表达的意思"，与 AI 助手联动。
...
### 邮件聚合（邮件即笔记，v1.83.0）
- **多账号聚合收件箱**：打通 QQ / 企业邮箱等**多账号 IMAP**，聚合到一个统一收件流（按时间降序、分页）；单账号连接失败跳过。
- **账号筛选**：「全部账号 + 每账号」多选下拉，选哪些账号就聚合哪些；列表新增**独立账号列**（多账号才显示），未读角标用聚合汇总。
- **按月直达**：月份选择器聚合各账号含邮件月份，所有月份可点，按邮件自身时区年月在后端过滤（不依赖服务端 `SINCE/BEFORE`）。
- **存为笔记 / 任务**：单封 / 任务 / AI 总结 / 批量四条路径统一**先写属性再跳转**，自动填发件人 / 主题 / 邮件日期 + 发件人标签，不因竞态丢失。
- **AI 总结**：阅读区一键总结，结果在弹窗展示；「存入笔记」自动填属性 + 发件人标签。
- **回复 / 转发**：转发可输入收件人、光标自动定位；回复 / 转发光标定位正文最前。
- **测试连接**：设置 → 邮箱多账号管理里「测试连接」（IMAP 登录 + 选 INBOX + 可选 SMTP 认证，不发信）。

### 数据安全
- **自动保存**：防抖写入 SQLite，无「保存」按钮。
- **版本历史**：每次保存前自动快照，可一键回滚（每页保留 50 份，自动去重）。
- **回收站**：软删除 + 恢复 + 彻底删除。
- **端到端加密**：Argon2id 密钥派生 + XChaCha20-Poly1305；**按空间**独立密钥（个人空间必须先加密才能绑同步）、同步加密 + 设置 UI + 口令解锁 / 锁定；换设备靠**公开材料**推 / 取（只凭主口令，见「多设备同步 → [加密与换设备](#2-加密与换设备)」）。
...
- **同步一致性加固（seq-LWW + dirty 优先本地，v1.84.3）**：LWW 改用**服务端单调 `seq` 为权威序**，配合 **dirty 优先本地**——根治的是**客户端时钟漂移导致整页覆盖丢改动**这个隐患。⚠️ **边界（别读成「不会丢」）**：**同一页同时编辑仍会按服务端 `seq` 保留一方**（页级 LWW、非 CRDT），被覆盖的一方可从**每页 50 份版本历史**取回；**「多端一致」≠「实时协作」**。服务端 `changes` 自增 `seq` 全局唯一、单调；本地未提交改动标 `dirty` 优先保留，推送成功才清除。
- **附件同步**：双向增量；附件**按空间**存放（`attachments/<空间 id>/`，空间内内容寻址去重）—— 钥匙按空间，字节也必须按空间，绝不跨空间混用同一份。
- **换设备**：主口令 ＋ **公开材料**（盐 / KDF 参数 / 被包裹的空间钥匙盒）—— 旧设备「推到服务器」，新设备「从服务器取回」再输一次口令即可解开自己的空间；服务端拿到的公开材料里**没有裸钥匙**，解不开任何空间。详见「[多设备同步 → 加密与换设备](#2-加密与换设备)」。
- **同步闸门**：**个人空间没加密就不许绑同步**（免得明文上服务端，而且事后加密撤不回已经落库的那份）；团队空间免检（服务端明文是它刻意换来的）。
...
**物理隔离**：每个工作空间一个独立 SQLite 库（`spaces/<ws_id>/`），应用级共享状态（workspaces / 模板 / 插件状态 / 同步配置）放 `meta.db`；附件字节**按空间**存放（`attachments/<ws_id>/`，空间内内容寻址去重 —— 加密是**按空间**的，字节必须跟着钥匙走），单空间可搬移经空间级附件子集导出实现。
...
| 附件 | 内容寻址（SHA-256，**按空间**去重）；Web 侧存 IndexedDB `blobStore` |
...
**同步机制**：本地每次写操作在 `changes` 表记录 outbox 变更；同步时先 push 本地增量，再 pull 服务端增量。冲突合并采用 **LWW（服务端单调 `seq` 为权威序）+ dirty 优先本地**（v1.84.3 起）——根治团队多人同时改同一内容时因客户端时钟漂移而丢失改动的隐患：服务端 `changes` 自增 `seq` 作为全局唯一、单调的 LWW 键，本地未提交的改动标记 `dirty` 优先保留，推送成功才清除。删除走墓碑，附件按内容寻址传输（**按空间**各自一份）。同步会先过**闸门**：**个人空间没有加密就不许绑同步**（团队空间免检）；会话锁定时同步被拒绝。若这个空间开了端到端加密，push 前加密、pull 后解密（服务端仅存密文）。

### 2. 加密与「换设备」

> ⚠️ 口径：**加密是按空间的**（不是"整个应用一个开关"）—— 个人空间密文、团队空间明文**可以同时成立**。个人空间要参与同步，**必须先按空间加密**（否则服务端就是明文，而且事后加密也撤不回已经落库的那份）。

- **开加密**：设置 →「安全」→ 空间隐私，给某个空间点「开启加密」并设一句**主口令**（至少 8 位）。之后每个空间有**自己一把随机钥匙**，被主口令派生的主密钥**包裹**成"盒子"；**主口令与钥匙都不落盘、不上传**。忘了口令＝这些空间打不开（**没有找回流程**）；锁定时（设置里「立即锁定」或重启）内容不可读、同步被拒。
- **换到第二台设备**（公开材料走服务端、口令只在你手里）：
  1. **旧设备**：设置 →「安全」→ 空间隐私 → 对应空间点「**推到服务器**」，把**公开材料**交给同步服务 —— 它是「盐 ＋ KDF 参数 ＋ 被包裹的空间钥匙盒」，**里面没有裸钥匙**，服务端拿到也**解不开**任何空间。
  2. **新设备**：先把该空间同步下来，再点「**从服务器取回**」拉回这份公开材料，然后**输一次主口令**：本机就地推出主密钥、解开盒子，拿到这个空间自己的钥匙即可读写。
  3. 「从服务器取回」**默认不覆盖**本机已有的材料（闷头覆盖可能让本机打不开自己的空间）；确实要以服务端那份为准时，勾「允许覆盖本机已有的材料」。
- **手工搬也行**：公开材料就是一份 JSON，可以自己拷到另一台设备再用主口令解开（同一份材料 ＋ 同一句口令 ⇒ 同一把主密钥）。
...
- **空间导出 / 导入**（侧边栏空间菜单 →「导出当前空间」/「导入空间包」）：把单个空间（含其引用的附件）导出为自包含包，在另一台设备**新建一个空间**导入，不覆盖现有空间。导入进来的空间按**个人空间**对待（和"本地新建"同一条口径）—— 想让它参与同步，先给它开加密。
...
- [x] 多设备同步：outbox + LWW + 附件同步（附件按空间存放）＋ 公开材料推 / 取（换设备只凭主口令）
- [x] 标签 / 反链 / 文件库 / 看板
- [x] 回收站 / 版本历史 / 整库备份
- [x] 暗色模式 / 命令面板 / 多窗口
- [x] 块拖拽排序 / 编辑器查找
...
- [x] 属性驱动仪表盘聚合
- [x] 端到端加密（Argon2id 派生 + XChaCha20-Poly1305；**按空间**独立密钥 + 每空间一把随机钥匙被主口令包裹；同步加密 + 设置 UI + 口令解锁/锁定；**同步闸门**：个人空间未加密不许绑同步；公开材料可推 / 取 ⇒ 换设备只凭主口令）
- [x] 主题自定义 + 插件启停
